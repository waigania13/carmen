...
function index(geocoder, from, to, options, callback) {
    ...
    if (!from) {
        ...
        options.output.write('{ "type": "FeatureCollection", "features": [');

        var inStream = options.input.pipe(geojsonStream.parse());
...
function update(source, docs, options, callback) {
    ...
    //Legacy API - deprecate on next major version
    //             Previously options was zoom - an integer 0-14
    if (typeof options === 'number') options = { zoom: options };
    ...
    source._geocoder.loadall(getter, 'freq', ids, function(err) {
        ...
        indexdocs(docs, freq, {
            zoom: options.zoom,
            tokens: source.geocoder_tokens,
            firstWrite: options.firstWrite,
            output: options.output
        }, updateCache);
    });
    ...
    function updateCache(err, patch) {
        ...
        q.awaitAll(function(err, res) {
            if (!options.openStream && options.output) options.output.end();
