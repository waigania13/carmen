 ...
 * @param {Tilesink} to - a {@link https://github.com/mapbox/tilelive/blob/master/API.md tilelive.Tilesink} describing the destination of the index. Supported protocols: {@link https://github.com/mapbox/node-mbtiles MBTiles} and {@link MemSource}.
...
function store(source, callback) {
    ...
    types.forEach((type) => {
        q.defer((callback) => {
            const rocksdb = source.getBaseFilename() + '.' + type + '.rocksdb';
            ...
            if (type === 'norm') {
                const dawgFile = source.getBaseFilename() + '.dawg';
                const dump = source._dictcache.dumpWithNormalizations(tmpdir);
