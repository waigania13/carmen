...
function store(source, callback) {
    ...
    ['freq','grid'].forEach(function(type) {
        q.defer(function(callback) {
            ...
            cache.pack(rocksdb, type);

            var pack = tar.pack(rocksdb);
            var bufs = [];
            pack.on('data', function(d){ bufs.push(d); });
            pack.on('end', function() {
                var buf = Buffer.concat(bufs);
                fs.removeSync(rocksdb);
                source.putGeocoderData(type, 0, buf, callback);
            });
        });
    });
    q.defer(function(callback) {
    ...
    q.awaitAll(function(err) {
        ...
        // source._geocoder.unload('freq');
        // source._geocoder.unload('grid');
        // source._geocoder.unload('stat');
        callback();
    });
}
