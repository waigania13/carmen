...
function index(source, docs, callback) {
    ...
    function indexDocs(approxdocs, freq, callback) {
        ...
        _(docs).each(putDocumentFeature);
        q.awaitAll(callback);

        function putDocumentFeature(doc) {
            q.defer(function(doc, callback) {
                source.putFeature(doc.id, doc.doc, callback);
            }, doc);
        }
        ...
        function setParts(data, type) {
            q.defer(function(data, type, callback) {
                ...
                source._geocoder.getall(source.getGeocoderData.bind(source), type, ids, function(err) {
                    if (err) return callback(err);
                    for (var i = 0; i < ids.length; i++) {
                        var id = ids[i];
...
function loadDoc(doc, freq, patch, degenerated) {
    ...
    function loadTerm(terms, x) {
        ...
        var weights = [];
        var total = 0;
        var limit = Math.floor(Math.sqrt(terms.length));
