...
function update(source, docs, zoom, callback) {
    ...
    source._geocoder.loadall(getter, 'freq', ids, function(err) {
        ...
        for (var i = 0; i < ids.length; i++) {
            ...
            if (id === 0) {
                freq[id][1] = (source._geocoder.get('freq', id) || [0,0])[1] || freq[id][1];
            }
                ...
                cache.loadall(getter, type, ids, function(err) {
                    ...
                    for (var i = 0; i < ids.length; i++) {
                        ...
                        case 'grid':
                            ...
                            cache.set('look', termops.look(id), []);
                            break;
                        }
                    }
...
function generateFrequency(docs, replacer) {
    ...
    // Uses freq[0][0] as a convention for storing total # of docs.
    // @TODO determine whether 0 can really ever be a relevant term
    // when using fnv1a.
    freq[0][0] = 0;

    // Uses freq[0][1] as a convention for storing max score.
    freq[0][1] = 0;
    ...
    for (var i = 0; i < docs.length; i++) {
        ...
        // set max score
        freq[0][1] = Math.max(freq[0][1], docs[i]._score || 0);
...
function store(source, callback) {
    ...
    ['freq','grid','look'].forEach(loadTerm);
    ...
    q.awaitAll(function(err) {
        ...
        source._geocoder.unloadall('look');
        callback();
    });
}
