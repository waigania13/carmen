        ...
        function featureLoaded(err, features) {
            ...
            for (var id in features) {
                ...
                if (coalesced[coord]) cq.defer(function(feat, callback) {
                    if (!('lon' in feat)) return callback(new Error('No lon field in data'));
                    if (!('lat' in feat)) return callback(new Error('No lat field in data'));
                    feat._extid = term.db + '.' + term.id;
                    context(geocoder, feat.lon, feat.lat, term.db, function(err, context) {
                        if (err) return callback(err);
