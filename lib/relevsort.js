...
function relevSort(indexes, types, data, geocoder, feats, grids, zooms, callback) {
    ...
    function loadResult(term, callback) {
        ...
        var cache = geocoder.indexes[term.db]._features;
        cache.getone(source.getGeocoderData.bind(source), 'feature', term.id, featureLoaded);
        function featureLoaded(err, features) {
            if (err) return callback(err);
            // @TODO determine if this needs to be more selective with
            // which contexts are loaded.
            // if (Object.keys(features).length > 1) console.log(features);
            for (var id in features) cq.defer(function(feat, callback) {
