...
function relevSort(indexes, types, query, stats, geocoder, feats, grids, zooms, callback) {
    ...
    // Disallow more than 10 of the best results at this point.
    ...
    function loadResult(term, callback) {
        ...
        function featureLoaded(err, features) {
            ...
            for (var id in features) {
                ...
                var bbox = sm.xyz([features[id]._center[0], features[id]._center[1], features[id]._center[0], features[id]._center[1]], source._geocoder.zoom);
                ...
                if (coalesced[coord]) cq.defer(function(feat, callback) {
                    ...
                    feat._extid = term.db + '.' + term.id;
                    context(geocoder, feat._center[0], feat._center[1], term.db, function(err, context) {
                        if (err) return callback(err);
