...
function spatialmatch(query, phrasematches, options, callback) {
    ...
    function coalesceLoad(stack, callback) {
        ...
        for (var i = 0; i < stacks.length; i++) {
            ...
            for (var j = 0; j < stack.length; j++) {
                ...
                console.log('### stackj', stack[j]);
                delete stack[j].phrase;
                stack[j].dawgPhrases.forEach(function (dawgPhrase) {
                    var phrase = termops.encodePhrase(dawgPhrase, true);
                    console.log('#### Phrase', phrase);
                    loadables[idx].ids.push(phrase);
    ...
    function coalesceStack(stack, callback) {
        ...
        console.log('### Coalesceing stack', stack);
        var coalesceOpts = {};
