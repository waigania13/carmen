...
function stackable(phrasematches, idx, mask, nmask, stack, relev) {
    ...
    // check if address components in a stack are orderd general-to-specific or specific-to-general
        if (stack && stack.length >= 2) {
            direction = stack[0].mask > stack[1].mask ? "ascending" : "descending"
        }
    ...
    for (var i = 0; i < phrasematches[idx].length; i++) {
        ...
        var lastMask = stack.length ? stack[stack.length - 1].mask : 0;
        var next = phrasematches[idx][i];
        targetStack.relev = relev;

        if (targetMask & next.mask) continue;
        if (targetNmask & next.nmask) continue;
        if (direction === "descending" && lastMask && lastMask > next.mask) continue;
        if (direction === "ascending" && lastMask && lastMask < next.mask) continue;
