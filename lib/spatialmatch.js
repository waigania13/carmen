...
function spatialmatch(query, phrasematches, options, callback) {
    ...
    function coalesceStack(stack, callback) {
        ...
        // console.log('\nstack:', JSON.stringify(stack, null, 2)); // both streets
        coalesce(stack, coalesceOpts, function(err, features) {
            // Include text for debugging with each matched feature.

            // console.log('\nfeatures after coalesce:', JSON.stringify(features, null, 2)); // two ids
            ...
            // console.log('\nfeatures after coalesce:', JSON.stringify(features, null, 2)); // Just fake st text, but two ids
    ...
    function coalesceFinalize(err, results) {
        ...
        // console.log('\nresults:', JSON.stringify(results, null, 2)); // Just fake st
        var combined = [];
...
function stackable(phrasematches, idx, mask, nmask, stack, relev) {
    ...
    for (var i = 0; i < phrasematches[idx].length; i++) {
        ...
        if (targetMask & next.mask) {
            continue;
        }
        // if (targetNmask & next.nmask) continue;

        // refine to allow mask numbers out of order only if nmask numbers are the same
        if (targetMask && targetMask < next.mask) {
            if (targetNmask != next.nmask) continue;
        }
        ...
        stacks.push.apply(stacks, stackable(phrasematches, idx, targetMask, targetNmask, targetStack, targetStack.relev));
    ...
    // deduplicate stacks
    var uniqueStackObject = {};
    var uniqueStacks = [];

    for (i=0; i<stacks.length; i++) {
        stacks[i].sort(sortByText);
        uniqueStackObject[JSON.stringify(stacks[i])] = stacks[i];
    }

    for (var key in uniqueStackObject) {
        uniqueStacks.push(uniqueStackObject[key]);
    }
    return uniqueStacks;
}
...
function sortByRelevLengthIdx(a, b) {
    return (b.relev - a.relev) ||
        ...
        (a[a.length-1].text - b[b.length-1].text);
}
