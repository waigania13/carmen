    ...
    function coalesceLoad() {
        ...
        for (var i = 0; i < stacks.length; i++) {
            ...
            for (var j = 0; j < stack.length; j++) {
                ...
                loadables[idx] = loadables[idx].concat(stack[j].shards);
            }
        }
        ...
        for (var s = 0; s < phrasematchResults.length; s++) {
            ...
            if (sourceLoadables) q.defer(phrasematchResult.loadallshards, phrasematchResult.getter, 'grid', uniq(sourceLoadables));
        }
