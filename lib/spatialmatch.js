   ...
   //Stores the proximity latlng as zxy at all source zoom levels
    ...
    if (proximity.length) {
        zooms.forEach(function(zoom) {
            zxyProximity[zoom] = sm.px(proximity, zoom);
        });
        var zxyResult = [];
        results.forEach(function(result) {
            result.z = indexes[result.db]._geocoder.zoom;
            result.x = Math.floor(result.tmpid / xd);
            result.y = Math.floor(result.tmpid % xd / yd);
            result.distance = Math.pow(Math.pow(zxyProximity[result.z][0] - result.x, 2) + Math.pow(zxyProximity[result.z][1] - result.y, 2), 0.5);
        });
        ...
        if (proximity.length) {
            contexts.forEach(function(context) {
                var lat1 = proximity[0],
                    lat2 = context[0]._center[1],
                    lon1 = proximity[1],
                    lon2 = context[0]._center[0];
                var phi1 = lat1 * (Math.PI / 180),
                    phi2 = lat2 * (Math.PI / 180);
                var deltaPhi = (lat2-lat1) * (Math.PI / 180),
                    deltaLambda = (lon2-lon1) * (Math.PI / 180);
                var a = Math.sin(deltaPhi/2) * Math.sin(deltaPhi/2) + Math.cos(phi1) * Math.cos(phi2) * Math.sin(deltaLambda/2) * Math.sin(deltaLambda/2);
                context[0]._distance = 6371 * 2 * Math.atan2(Math.sqrt(a), Math.sqrt(1-a));
            });
        }
