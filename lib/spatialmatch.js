...
function spatialmatch(query, stats, geocoder, feats, grids, zooms, options, callback) {
    ...
    spatialMatch(query.length, feats, grids, zooms, function(err, res) {
...
//    coalesceZooms(grids, zooms, function(err, coalesced) {
//    if (err) return callback(err);
...
//    for (var c in coalesced) {
//        var key = coalesced[c].key;
//        pushed = false;

//        // Check cache for whether this set's combination of features
...
//        rows = new Array(coalesced[c].length);
//        for (var i = 0; i < coalesced[c].length; i++) {
//            rows[i] = feats[coalesced[c][i]];
//        }
...
//            sets[rows[i].tmpid] = sets[rows[i].tmpid] || rows[i];

//            // Don't use results after the topmost index in the stack.
//            if (pushed && pushed !== rows[i].dbid) continue;
//            pushed = rows[i].dbid;
//            prev = rowMemo[rows[i].tmpid];

//            // Don't overwrite an existing result entry for the same
//            // feature that scores higher.
//            if (prev && prev.relev > (relev + addrmod[rows[i].dbid])) continue;
