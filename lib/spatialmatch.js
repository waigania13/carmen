...
function spatialmatch(query, phrasematches, options, callback) {
    ...
    for (var i = 0; i < stacks.length; i++) {
        ...
        var result = coalesce(stack, options);
        relevMax = Math.max(relevMax, result[0].relev);

        // Collect features within threshold of max relev.
        var byIdx = stackByIdx(stack);
        l = result.length;
        while (l--) {
            var feature = result[l];
...
function coalesce(stack, options) {
    ...
    for (var i = 0; i < stack.length; i++) {
        ...
        maxz = Math.max(maxz, stack[i].zoom);
    }

    // Proximity option is set.
    // Convert proximity to xy @ highest zoom level
    var centerzxy;
    if (options && options.proximity) {
        centerzxy = proximity.center2zxy(options.proximity[0], options.proximity[1], maxz);
    }
    ...
    for (var h = 0; h < stack.length; h++) {
        ...
        for (var i = 0; i < grids.length; i++) {
            ...
            if (centerzxy) {
                var proxzxy = proximity.pxy2zxy([z,grid.x,grid.y], maxz);
    ...
    if (centerzxy) {
        features.sort(sortByRelevDistance);
    } else {
        features.sort(sortByRelev);
    }

    // This will get truncated to max 40 results @ verify anyway.
