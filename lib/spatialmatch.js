...
function spatialmatch(query, phrasematches, options, callback) {
    // filter out empty phrasematches, sort by query mask
    phraseSort = _.filter(phrasematches, function (item) {
        return item.length;
    });
    phraseSort.sort(sortByMask);

    // check order of query
    var direction = phraseSort[0][0].idx < phraseSort[1][0].idx ? "descending" : "ascending";
    var stacks = stackable(phrasematches, null, null, null, null, null, direction);
...
function stackable(phrasematches, idx, mask, nmask, stack, relev, direction) {
    idx = idx || 0;
    mask = mask || 0;
    nmask = nmask || 0;
    stack = stack || [];
    ...
    stacks.push.apply(stacks, stackable(phrasematches, idx+1, mask, nmask, stack, relev, direction));
    ...
    for (var i = 0; i < phrasematches[idx].length; i++) {
        ...
        if (direction === "descending") {
            ...
            var lowMask = targetMask & (-targetMask);
            if (lowMask && lowMask > next.mask) continue;
        }
        ...
        stacks.push.apply(stacks, stackable(phrasematches, idx+1, targetMask, targetNmask, targetStack, targetStack.relev, direction));
    }
