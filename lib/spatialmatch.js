...
function spatialmatch(query, phrasematches, options, callback) {
    var stacks = stackable(phrasematches).slice(0,10);
...
function stackable(phrasematches, idx, mask, stack, relev) {
    idx = idx || 0;
    mask = mask || 0;
    stack = stack || [];
    relev = relev || 0;
    ...
    stacks = stacks.concat(stackable(phrasematches, idx+1, mask, stack, relev));
    ...
    for (var i = 0; i < phrasematches[idx].length; i++) {
        ...
        targetStack.relev += targetStack.length > 1 ? 0.01 : 0;
        ...
        stacks = stacks.concat(stackable(phrasematches, idx+1, targetMask, targetStack, targetStack.relev));
    }
...
function sortByRelevLengthIdx(a, b) {
    return (b.relev - a.relev) || (b.length - a.length) || (a[0].idx - b[0].idx);
}
