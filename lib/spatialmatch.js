...
function spatialmatch(indexes, types, query, stats, geocoder, feats, grids, zooms, proximity, callback) {
    ...
    for (var id in indexes) {
        addrmod[id] = address && (indexes[id]._geocoder.geocoder_address || indexes[id]._geocoder.geocoder_address_points) ? 0.01 : 0;
        addridx[indexes[id]._geocoder.idx] = address && (indexes[id]._geocoder.geocoder_address || indexes[id]._geocoder.geocoder_address_points);
    }
   ...
   //Stores the proximity latlng as zxy at all source zoom levels
    ...
    if (proximity) {
        ...
        for (var i = 0; i < results.length; i++) {
            results[i].z = indexes[results[i].dbid]._geocoder.zoom;
    ...
    function loadResult(term, callback) {
        ...
        function featureLoaded(err, features) {
            ...
            for (var id in features) {
                ...
                if (address && feat._cluster && source._geocoder.geocoder_address_points){
                    feat._address = address;
