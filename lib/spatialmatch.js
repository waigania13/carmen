...
function spatialmatch(query, phrasematches, options, callback) {
    var stacks = stackable(phrasematches).slice(0,40);
...
function stackable(phrasematches, idx, mask, nmask, bmask, stack, relev) {
    idx = idx || 0;
    mask = mask || 0;
    nmask = nmask || 0;
    bmask = bmask || 0;
    ...
    stacks = stacks.concat(stackable(phrasematches, idx+1, mask, nmask, bmask, stack, relev));
    ...
    for (var i = 0; i < phrasematches[idx].length; i++) {
        ...
        stacks = stacks.concat(stackable(phrasematches, idx+1, targetMask, targetNmask, targetBmask, targetStack, targetStack.relev));
    }
...
function sortByRelevLengthIdx(a, b) {
    return (b.relev - a.relev) || (a.length - b.length) || (a[a.length-1].idx - b[b.length-1].idx);
}
