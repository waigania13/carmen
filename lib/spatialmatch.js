...
function spatialmatch(query, phrasematches, options, callback) {
    ...
    function coalesceLoad(stack, callback) {
        // First make a pass where phrase shards to load are grouped by idx.
        var loadables = {};
        for (var i = 0; i < stacks.length; i++) {
            var stack = stacks[i];
            for (var j = 0; j < stack.length; j++) {
                ...
                loadables[idx] = loadables[idx] || {
                    loadallshards: stack[j].loadallshards,
                    getter: stack[j].getter,
                    shards: []
                };
                loadables[idx].shards = loadables[idx].shards.concat(stack[j].shards);
            }
        }

        // Then queue loading in a single pass deduping shards.
        var q = queue();
        for (var idx in loadables) {
            q.defer(loadables[idx].loadallshards, loadables[idx].getter, 'grid', uniq(loadables[idx].shards));
        }
