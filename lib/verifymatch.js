        ...
        feature.getFeature(source, term.id, function featureLoaded(err, features) {
            ...
            for (var id in features) {
                ...
                if (address && feat._rangetype && source._geocoder.geocoder_address) {
                    ...
                    feat._geometry = applyAddress(feat, address) || applyAddress(feat, address.replace(/\D/, ''));
                    ...
                    feat._address = address;
                    feat._geometry = addressCluster(feat, address) || addressCluster(feat, address.replace(/\D/, ''));
                    feat._center = feat._geometry && feat._geometry.coordinates;
                    checks = checks && feat._geometry;
                }
                if (checks) {
    ...
    function finalize(err, contexts) {
        ...
        for (var j = 0, cl = contexts.length; j < cl; j++) {
            ...
            for (var i = 0, cjl = contexts[j].length; i < cjl; i++) {
                ...
                // @TODO this will need to be updated once we support more housenum at the first position
                if (subsets.length === 0 && address) {
                    var relev = new Relev(sets[a._tmpid]);
                    if (relev.reason % 2 === 0 && geocoder.byidx[relev.idx]._geocoder.geocoder_address) {
                        relev.count += 1;
                        relev.reason += 1;
            ...
            contexts[j]._relevance = getSetRelevance(query.length, subsets).relevance;
