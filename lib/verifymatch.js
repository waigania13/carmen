...
function verifymatch(query, stats, geocoder, matched, options, callback) {
    ...
    function afterFeatures(err, loaded) {
        ...
        function scoreMatch(loadedFeatureScore, scoreFilter, loadedFeatureStack, loadedFeatureType) {
            var subranges = Object.keys(scoreFilter[loadedFeatureStack][loadedFeatureType]);
            var match = false;
            for (var subrange = 0; subrange < subranges.length; subrange++) {
                if (loadedFeatureScore >= subranges[subrange][0] && loadedFeatureScore <= subranges[subrange][1]) {
                    match = true;
                }
            }
            return match;
