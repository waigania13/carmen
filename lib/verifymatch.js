    ...
    function afterFeatures(err, loaded) {
        ...
        if (options.scoreFilter || options.stacks) {
            ...
            for (var j = 0; j < loaded.length; j++) {
                ...
                if (qualifyResult([options.stacks, options.scoreFilter], [{func: stackMatch, arguments: [loaded[j].properties['carmen:geocoder_stack'], options.stacks]}, {func: scoreMatch, arguments: [loaded[j].properties['carmen:score'], options.scoreFilter, loaded[j].properties['carmen:geocoder_stack'], <loadedFeatureType>]}])) {
...
function dropFeature(geocoder, options, results, callback) {
    ...
    for (var i = 0; i < results.length; i++) {
        ...
        if (qualifyResult([options.types, options.stacks], [{func: typeMatch, arguments: [options.types, source.type, Object.keys(source.scoreranges)]}, {func: stackMatch, arguments: [options.stacks, source.stack]}])) {
