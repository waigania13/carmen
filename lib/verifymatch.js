...
function verifymatch(query, stats, geocoder, matched, options, callback) {
    ...
    function loadFeature(spatialmatch, pos, callback) {
        ...
        // Currently unclear why these might be undefined.
        // For now, catch and return error to try to learn more.
        if (!cover) return callback(new Error('Misuse: cover undefined'));
...
function verifyFeatures(query, geocoder, spatial, loaded, options) {
    ...
    for (var pos = 0; pos < loaded.length; pos++) {
        ...
        if (address && feat.properties["carmen:rangetype"] && source._geocoder.geocoder_address) {
            feat.properties.address = address.addr;
            feat.geometry = applyAddress(feat, address.addr);
            feat.properties["carmen:center"] = feat.geometry && feat.geometry.coordinates;
            checks = checks && feat.geometry;
        } else if (address && feat.properties["carmen:cluster"] && source._geocoder.geocoder_address) {
            feat.properties.address = address.addr;
            feat.geometry = addressCluster(feat, address.addr);
            feat.properties["carmen:center"] = feat.geometry && feat.geometry.coordinates;
            checks = checks && feat.geometry;
        } else if (source._geocoder.geocoder_address) {
            feat.properties.address = null;
        }
        ...
        if (checks) {
            ...
            feat.properties["carmen:extid"] = source._geocoder.name + '.' + feat.id;
...
function sortFeature(a, b) {
    return (b.properties["carmen:spatialmatch"].relev - a.properties["carmen:spatialmatch"].relev) ||
        ((a.properties.address===null?1:0) - (b.properties.address===null?1:0)) ||
