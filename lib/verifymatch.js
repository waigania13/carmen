...
function verifymatch(query, stats, geocoder, matched, options, callback) {
    ...
    function loadFeature(spatialmatch, pos, callback) {
        ...
        feature.getFeature(source, cover.id, callback);
    }
    ...
    function afterFeatures(err, loaded) {
        ...
        for (var pos = 0; pos < loaded.length; pos++) {
            ...
            var features = loaded[pos];
            var spatialmatch = results[pos];
            var cover = spatialmatch[0];
            var source = geocoder.byidx[cover.idx];
            ...
            for (var id in features) {
                ...
                // check the cover zxy to make sure it is part of the feature's
                // zxy cover.
                var feat = features[id];
                var checks = feat._zxy.indexOf(zxy) >= 0;
                if (!checks) continue;
        ...
        result = result.slice(0, options.limit_verify);

        loadContexts(geocoder, result, sets, callback);
    }
...
function loadContexts(geocoder, features, sets, callback) {
    ...
    q.awaitAll(function(err, contexts) {
        if (err) return callback(err);
        callback(null, verifyContexts(contexts, geocoder.names));
    });
}

function verifyContexts(contexts, groups) {
    for (var a = 0; a < contexts.length; a++) {
        ...
        for (var c = 0; c < context.length; c++) {
            var backy = false;
            var feat = context[c];
            if (!verify[feat._tmpid]) continue;
            if (lastgroup > -1) {
                stacky = 1;
                backy = lastmask > verify[feat._tmpid].mask;
                gappy += Math.abs(groups[feat._dbidx] - lastgroup) - 1;
            }
            lastmask = verify[feat._tmpid].mask;
            lastgroup = groups[feat._dbidx];
            if (backy) {
                context._relevance += verify[feat._tmpid].relev * 0.5;
            } else {
                context._relevance += verify[feat._tmpid].relev;
            }
        }
