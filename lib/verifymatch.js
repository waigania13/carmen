...
function verifymatch(query, stats, geocoder, matched, options, callback) {
    ...
    function finalize(err, contexts) {
        ...
        for (var j = 0, cl = contexts.length; j < cl; j++) {
            ...
            // Penalize features that include a feature with an omitted address range.
            if (contexts[j][0]._geometry && contexts[j][0]._geometry.tiger_omitted) {
                contexts[j]._relevance += -0.01;
            }
        }
