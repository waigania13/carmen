...
function verifymatch(query, stats, geocoder, matched, options, callback) {
    ...
    function loadFeature(num, callback) {
        ...
        feature.getFeature(source, term.id, function featureLoaded(err, features) {
            ...
            for (var id in features) {
                ...
                var c3 = (source._geocoder.zoom * mp2_28) + (bbox.maxX * mp2_14) + (bbox.minY);
                var c4 = (source._geocoder.zoom * mp2_28) + (bbox.maxX * mp2_14) + (bbox.maxY);
                var checks = coalesced[c1] || coalesced[c2] || coalesced[c3] || coalesced[c4];
    ...
    function loadContexts(err, result) {
        ...
        require('fs').writeFileSync('/tmp/foo.json', JSON.stringify(features, null, 2));
...
function sortFeature(a, b) {
    return (b._relev - a._relev) ||
        ((b._address!==undefined?1:0) - (a._address!==undefined?1:0)) ||
...
function sortContext(a, b) {
    ...
    // if distance is the same, sort by score, idx
    if (a[0]._distance === b[0]._distance) {
        ...
        if (a._typeindex > b._typeindex) return 1;
    // if distance is different, sort by idx, distance
    } else {
        if (a._typeindex < b._typeindex) return -1;
        if (a._typeindex > b._typeindex) return 1;
