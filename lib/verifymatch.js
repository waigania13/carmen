...
function verifyContexts(contexts, sets, indexes) {
    ...
    for (var c = 0; c < contexts.length; c++) {
        peers[contexts[c].features[0].properties['carmen:tmpid']] = contexts[c].features[0];
    }

    for (var a = 0; a < contexts.length; a++) {
        var context = contexts[a];
        context._relevance = 0;
        context._typeindex = indexes[context.features[0].properties['carmen:index']].ndx;

        // Create lookup for covers by tmpid.
        var verify = {};
        var cover;
        var covers = context.features[0].properties['carmen:spatialmatch'].covers;
        for (var b = 0; b < covers.length; b++) {
            cover = covers[b];
            verify[cover.tmpid] = cover;
        }

        for (var ctx of context.features) {
...
function verifyContext(context, peers, strict, loose, indexes) {
    var addressOrder = context.features[0].properties['carmen:geocoder_address_order'];
    var gappy = 0;
    var stacky = false;
    var usedmask = 0;
    var lastmask = -1;
    ...
    if (indexes[context.features[0].properties["carmen:index"]].geocoder_inherit_score)
        squishyTarget = context.features[0].properties;

    for (var c = 0; c < context.features.length; c++) {
        var backy = false;
        var feat = context.features[c];
    ...
    if (squishy > 0)
        context.features[0].properties["carmen:scoredist"] += squishy;
    else
...
function sortContext(a, b) {
    ...
    // sort by score
    var as = a.features[0].properties['carmen:scoredist'] || 0;
    var bs = b.features[0].properties['carmen:scoredist'] || 0;
    if (as > bs) return -1;
    if (as < bs) return 1;
    ...
    if (a.features[0].properties['carmen:address'] && b.features[0].properties['carmen:address']) {
        if (a.features[0].properties['carmen:addressnumber'] && !b.features[0].properties['carmen:addressnumber']) return -1;
        if (b.features[0].properties['carmen:addressnumber'] && !a.features[0].properties['carmen:addressnumber']) return 1;
    }

    // omitted difference
    var omitted = ((a.features[0].geometry&&a.features[0].geometry.omitted?1:0) - (b.features[0].geometry&&b.features[0].geometry.omitted?1:0));
    if (omitted !== 0) return omitted;

    // sort by spatialmatch position ("stable sort")
    if (a.features[0].properties['carmen:position'] < b.features[0].properties['carmen:position']) return -1;
    if (a.features[0].properties['carmen:position'] > b.features[0].properties['carmen:position']) return 1;

    // last sort by id.
    if (a.features[0].id < b.features[0].id) return -1;
    if (a.features[0].id > b.features[0].id) return 1;
    return 0;
}
