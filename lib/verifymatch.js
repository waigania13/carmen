            ...
            if (address && (feats[0].properties['carmen:addressnumber'] || feats[0].properties['carmen:rangetype'])) {
                ...
                if (addressPoints.length) {
                    const newFeats = addressPoints.slice(0,10).map((addressPoint) => {
                        ...
                        console.warn('=========== FEATS: ', feats);
                        console.warn('=========== FEAT: ', feat);
                        console.warn('=========== FEAT GEOM: ', feat.geometry.geometries[0]);
                        console.warn('=========== ADDRESSPOINTS: ', addressPoints);
                        console.warn('=========== ADDRESSPOINT: ', addressPoint);
                        // Find the routable point if routable_points is not already set, and the addressPoint isn't interpolated
                        // TODO: Could do this check inside routablePoint if entire point object is passed in
                        // TODO: Revisit what routable_points already being set will actually look like
                        if (!feat.routable_points && !addressPoint.interpolated) {
