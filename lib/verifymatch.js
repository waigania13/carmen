...
function verifymatch(query, stats, geocoder, matched, options, callback) {
    ...
    function afterFeatures(err, loaded) {
        ...
        if (options.scoreFilter || options.stacks) {
            var filtered = {
                loaded: [],
                results: []
            };
            for (var j = 0; j < loaded.length; j++) {
                ...
                var useScoreFilter = ((loaded[j].properties['carmen:geocoder_stack'] || loaded[j].properties['carmen:source_stack']) && options.scoreFilter)
                if (qualifyResult([options.stacks, useScoreFilter], [{func: stackMatch, arguments: [loaded[j].properties['carmen:geocoder_stack'], options.stacks]}, {func: scoreMatch, arguments: [loaded[j].properties['carmen:score'], options.scoreFilter, loaded[j].properties['carmen:geocoder_stack'], loaded[j].properties['carmen:geocoder_type']]}])) {
        ...
        function scoreMatch(loadedFeatureScore, scoreFilter, loadedFeatureStack, loadedFeatureType) {
            var subrangeTypes = Object.keys(scoreFilter[loadedFeatureStack][loadedFeatureType]);
            var subranges = scoreFilter[loadedFeatureStack][loadedFeatureType];

            var match = false;
            for (var subrange = 0; subrange < subrangeTypes.length; subrange++) {
                if (loadedFeatureScore >= subranges[subrangeTypes[subrange]][0] && loadedFeatureScore <= subranges[subrangeTypes[subrange]][1]) {
                    match = true;
                }
            }
            return match;
...
function dropFeature(geocoder, options, results, callback) {
    ...
    for (var i = 0; i < results.length; i++) {
        ...
        function typeMatch(optionsTypes, sourceType, subtypes) {
            if (optionsTypes.indexOf(sourceType) !== -1) return true;

            var pass = false;
            subtypes = subtypes ? Object.keys(subtypes) : [];
            for (var st = 0; st < subtypes.length; st++) {
                if (optionsTypes.indexOf(sourceType + "." + subtypes[st]) !== -1) {
                    pass = true;
                    setScoreFilter(source, subtypes[st], sourceType);
                }
            }
            return pass;
        }

        function setScoreFilter(source, subtype, type) {
            var stack = source.stack;
            if (!options.scoreFilter) options.scoreFilter = {};
            if (!options.scoreFilter[stack]) options.scoreFilter[stack] = {};
            if (!options.scoreFilter[stack][type]) options.scoreFilter[stack][type] = {};
            if (!options.scoreFilter[stack][type][subtype]) {
                options.scoreFilter[stack][type][subtype] = [source.scoreranges[subtype][0] * source.maxscore, source.scoreranges[subtype][1] * source.maxscore];
            }
        }
        ...
        if (qualifyResult([options.types, options.stacks], [{func: typeMatch, arguments: [options.types, source.type, source.scoreranges]}, {func: stackMatch, arguments: [options.stacks, source.stack]}])) {
