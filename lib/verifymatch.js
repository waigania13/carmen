...
function verifymatch(query, stats, geocoder, matched, options, callback) {
    ...
    function loadFeature(spatialmatch, pos, callback) {
        ...
        // under certain conditions, loop through items rather than getting 0th
        ...
        // console.log('sources in loadF:', sources);
    ...
    function afterFeatures(err, loaded) {
        ...
        // still nested one depth more than expected?
...
function crossStreets(features) {
    // combines 2+ features into one feature, intersection?
    // could loop over to accomodate 2+ features, or assume === 2

    // console.log('crossing');
    console.log('features:', JSON.stringify(features, null, 2)); // returns both streets

    var combinedFeature = features[0];
    combinedFeature['properties']['carmen:text'] += ' and ' + features[1]['properties']['carmen:text'];
    // console.log(combinedFeature['properties']['carmen:text']);
...
function verifyFeatures(query, geocoder, spatial, loaded, options) {
    ...
    // loaded = loaded[0]; // that's gross
    // spatial = spatial[0];
    // console.log('verifying features:', loaded);
    // console.log('spatial', JSON.stringify(spatial, null, 2)); // one more nest

    for (var pos = 0; pos < loaded.length; pos++) {
        if (!loaded[pos]) continue;

        var feat = loaded[pos];
        // console.log('feat', JSON.stringify(feat, null, 2)); // one more nest

        var spatialmatch = spatial[pos];
        // console.log('sm:',spatialmatch);
        var cover = spatialmatch[0]; // fails out here
        // console.log('cover', JSON.stringify(cover, null, 2));
