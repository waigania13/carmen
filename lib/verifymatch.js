...
function verifymatch(query, stats, geocoder, matched, options, callback) {
    ...
    if (options.types) {
        ...
        for (var i = 0; i < results.length; i++) {
            var type = geocoder.byidx[results[i][0].idx]._geocoder.type;
...
function verifyFeatures(query, geocoder, spatial, loaded, options) {
    ...
    for (var pos = 0; pos < loaded.length; pos++) {
        ...
        if (checks) {
            ...
            feat.properties["carmen:extid"] = source._geocoder.type + '.' + feat.id;
...
function loadContexts(geocoder, features, sets, options, callback) {
    ...
    for (var i = 0; i < features.length; i++) q.defer(function(f, done) {
        ...
        context(geocoder, options, f.properties["carmen:center"][0], f.properties["carmen:center"][1], { maxidx:firstidx, matched:sets, language: options.language}, function(err, context) {
