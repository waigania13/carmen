...
function dropFeature(geocoder, options, results, callback) {
    ...
    for (var i = 0; i < results.length; i++) {
        ...
        if (options.types && !options.stacks) {
            if (typeMatch()) filtered.push(results[i]);
        } else if (options.stacks && !options.types) {
            if (stackMatch()) filtered.push(results[i]);
        } else {
            if (typeMatch() && stackMatch()) filtered.push(results[i]);
        }
    }
    return filtered;
}
