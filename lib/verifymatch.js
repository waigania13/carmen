...
module.exports.crossStreets = crossStreets;
...
function verifymatch(query, stats, geocoder, matched, options, callback) {
    ...
    function loadFeature(spatialmatch, pos, callback) {
        var match = spatialmatch[0];
        // console.log('match in loadFeature:', match);

        // TODO: Under certain conditions, loop through items rather than getting 0th
        // if idx matches for both items? For now, always loop.
        var sources = []
        for (i = 0; i < match.length; i++) {
            var cover = match[i];
            var source = geocoder.byidx[cover.idx];
            sources.push({source: source, cover: cover});
        }
        ...
        feature.getFeatureByCover(sources, callback);
    }

    function afterFeatures(err, loaded) {
        if (err) return callback(err);

        // if 2+ items in features array, flatten into one feature
        // still nested one depth more than expected? Probably because multiple levels: 
        //      country, state, city, street + street
        // need flag to tell whether it's a street combo, then combine.
        if (loaded[0].length < 1) {
            loaded = loaded[0];
        } else {
            loaded = crossStreets(loaded[0]);
        }
...
function crossStreets(features) {
    // combines 2+ features into one feature, w/ intersection?
    // Make feature collection with two streets, extra property 'intersection'?
    var featureA = features[0];
    var featureB = features[1];
    var name = featureA['properties']['carmen:text'] + ' & ' + featureB['properties']['carmen:text'];
    //var intersection = [0,0] // calculate this

    // this is actually for polygons. Linestring version? Young says it should work
    console.log('\nfinding intersection\n');
    var intersection = intersect(featureA, featureB);
    console.log('intersect', JSON.stringify(intersection, null, 2));

    if (!intersection) {
        // if no intersection found, return an empty array. Better to return first feature?
        //  would give opportunity for a single street to be a match.
        return []; 
    }

    var combinedFeature = {
      'id': 1, // take this out entirely? Not actually adding to index
      'properties': {
        'carmen:text': name,
        'carmen:center': [intersection]
      },
      'geometry': {
        'type': 'Point',
    ...
    // console.log('combined feature:', JSON.stringify(combinedFeature, null, 2));
