...
function verifymatch(query, stats, geocoder, matched, options, callback) {
    ...
    q.awaitAll(function(err) {
        ...
        for (var j = 0, cl = contexts.length; j < cl; j++) {
            ...
            //Updates the lowest reason mask & count to include the address number
            //@TODO this will need to be updated once we support more than US addresses
            if (address) {
                subsets.sort(function(a,b) { return a - b; });
                var bitmask = subsets[0].reason.toString(2);
                if (bitmask.charAt(bitmask.length-1) === '0')
                    subsets[0].count = subsets[0].count + 1;
                    subsets[0].reason = parseInt(bitmask.substr(0, bitmask.length-1) + '1', 2);
                    console.log('ADDRESS:',subsets[0].reason);
            }
