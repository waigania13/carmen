...
module.exports = function(geocoder, lon, lat, maxidx, full, callback) {
    ...
    q.awaitAll(function(err, res) {
        ...
        for (var i = 0; i < res.length; i++) {
            ...
            //If the last feature was a range and the current feature is an address point
            // replace the range with the point
            if (memo[name]) {
                if (res[i-1]._rangetype && res[i]._cluster) {
                    stack[stack.length-1] = res[i];
                } else continue;
            }
