...
module.exports = function(geocoder, lon, lat, options, callback) {
    ...
    for (var type_i = 0; type_i < types.length; type_i++) {
        ...
        if (splitType.length === 2)
            subtypes[splitType[0]] = splitType[1];
        else
            subtypes[types[type_i]] = true;
    }
    ...
    for (var index_ids_it = 0; index_ids_it < index_ids.length; index_ids_it++) {
        ...
        if (lat >= bounds[1] && lat <= bounds[3] && lon >= bounds[0] && lon <= bounds[2]) {
            ...
            if (options.full && subtypes[source.type] && source.scoreranges[subtypes[source.type]]) {
                scoreRange = JSON.parse(JSON.stringify(source.scoreranges[subtypes[source.type]]));
                scoreRange[0] = scoreRange[0] * source.maxscore;
                scoreRange[1] = scoreRange[1] * source.maxscore;
            }
            q.defer(contextVector, source, lon, lat, full, matched, language, scoreRange);
        }
    }
    ...
    q.awaitAll(function(err, res) {
        ...
        for (var i = 0; i < res.length; i++) {
            ...
            if (toFilter) {
                ...
                if (options.types && !subtypes[type]) filter = false;
            ...
            if (memo[name] && res[i].properties['carmen:geomtype'] !== 3) {
                if (res[i].properties['carmen:vtquerydist'] < memo[name].properties['carmen:vtquerydist']) {
                    ...
                    if (options.full && options.types && subtypes[type]) {
