...
module.exports = function(geocoder, lon, lat, options, callback) {
    ...
    q.awaitAll(function(err, res) {
        ...
        var queryPt = point([lon, lat]);
        for (var i = 0; i < res.length; i++) {
            if (!res[i]) continue;
            var name = geocoder.byidx[res[i].properties['carmen:dbidx']]._geocoder.name;
            if (memo[name]) {
                if (memo[name].geometry && memo[name].geometry.type === 'Point') curFeat = distance(memo[name], queryPt);
                else curFeat = distance(point(memo[name].properties['carmen:center']), queryPt);

                if (res[i].geometry && res[i].geometry.type === 'Point') newFeat = distance(res[i], queryPt);
                else newFeat = distance(point(res[i].properties['carmen:center']), queryPt);

                if (newFeat < curFeat) memo[name] = res[i];
            } else {
                memo[name] = res[i];
            }
        }
...
function loadFeature(source, feat, full, query, language, callback) {
    ...
    if (!full) {
        ...
        if (feat['carmen:center']) loaded.properties['carmen:center'] = JSON.parse(feat['carmen:center']);
        else if (feat.properties && feat.properties['carmen:center']) loaded.properties['carmen:center'] = JSON.parse(feat.properties['carmen:center']);
