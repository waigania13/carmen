...
module.exports = function(geocoder, queryOpts, lon, lat, options, callback) {
    options = options || {};
    ...
    q.awaitAll(function(err, res) {
        ...
        for (var i = 0; i < res.length; i++) {
            ...
            if (toFilter) {
                if (queryOpts.types && queryOpts.types.indexOf(type) === -1) continue;
                else toFilter = false;
            }
            ...
            if (memo[name] && res[i].properties['carmen:geomtype'] !== 'Polygon' && res[i].properties['carmen:geomtype'] !== 'MultiPolygon') {
                if (res[i].properties['carmen:vtquerydist'] < memo[name].properties['carmen:vtquerydist']) {
                    ...
                    if (options.full && queryOpts.types && queryOpts.types.indexOf(type) === -1) {
                        ...
                        if (replaceName !== name) emo[name] = res[i];
