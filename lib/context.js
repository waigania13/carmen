...
module.exports = function(geocoder, lon, lat, options, callback) {
    ...
    for (var index_ids_it = 0; index_ids_it < index_ids.length; index_ids_it++) {
        ...
        if (lat >= bounds[1] && lat <= bounds[3] && lon >= bounds[0] && lon <= bounds[2]) {
            ...
            // look for specific feature tmpid at the max index if specified
            // avoid by-reference errors; `matched` is reused in this loop
            var sendMatched = {};
            if (options.tmpid && (index_ids_it === (index_ids.length - 1))) {
                sendMatched = { _exclusive: true };
                sendMatched[options.tmpid] = true;
            }
            else {
                sendMatched = matched;
            }

            q.defer(contextVector, source, lon, lat, full, sendMatched, language, scoreRange);
        }
    }
    ...
    q.awaitAll(function(err, res) {
        ...
        for (var i = 0; i < res.length; i++) {
            ...
            if (toFilter) {
                ...
                if (options.types && !subtypeLookup[type]) filter = false;
            ...
            if (memo[name] && res[i].properties['carmen:geomtype'] !== 3) {
                if (res[i].properties['carmen:vtquerydist'] < memo[name].properties['carmen:vtquerydist']) {
                    ...
                    if (options.full && options.types && !subtypeLookup[type]) {
