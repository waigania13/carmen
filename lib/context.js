...
module.exports = function(geocoder, lon, lat, options, callback) {
    ...
    if (full && options.types) {
        // if there's a type filter, pre-filter sources
        var allowedSources = sources.filter(function(source) { return options.types.indexOf(source.type) !== -1; });
        var maxAllowedIdx = Math.max.apply(null, allowedSources.map(function(source) { return source.idx; }));

        sources = sources.filter(function(source) { return source.idx <= maxAllowedIdx; });
    }
