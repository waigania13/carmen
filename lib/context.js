    ...
    q.awaitAll(function(err, res) {
        ...
        for (var i = 0; i < res.length; i++) {
            ...
            if (memo[name] && res[i].properties['carmen:geomtype'] !== 'Polygon' && res[i].properties['carmen:geomtype'] !== 'MultiPolygon') {
                if (res[i].properties['carmen:contextdist'] < memo[name].properties['carmen:contextdist']) {
                    memo[name] = res[i];
                }
            } else {
                memo[name] = res[i];
    ...
    function query(err, vt) {
        ...
        vt.query(lon, lat, {
            ...
            for (var i = 0; i < results.length; i++) {
                ...
                attr.properties ? attr.properties['carmen:contextdist'] = results[i].distance : attr['carmen:contextdist'] = results[i].distance;
                attr.properties ? attr.properties['carmen:geomtype'] = geom.type : attr['carmen:geomtype'] = geom.type;
...
function loadFeature(source, feat, full, query, language, callback) {
    ...
    if (!full) {
        ...
        loaded.properties['carmen:contextdist'] = feat['carmen:contextdist'] || feat.properties['carmen:contextdist'];
        loaded.properties['carmen:geomtype'] = feat['carmen:geomtype'] || feat.properties['carmen:geomtype'];
    ...
    feature.getFeatureById(source, feat.id, function(err, loaded) {
        ...
        loaded.properties['carmen:tmpid'] = (dbidx*mp25) + termops.feature(feat.id);
        loaded.properties['carmen:dbidx'] = dbidx;
        loaded.properties['carmen:contextdist'] = feat.properties ? feat.properties['carmen:contextdist'] : feat['carmen:contextdist'];
        loaded.properties['carmen:geomtype'] = feat['carmen:geomtype'] || feat.properties['carmen:geomtype'];
