...
module.exports = function(geocoder, lon, lat, maxtype, callback) {
    ...
    function loadType(type, callback) {
        ...
        if (cache[ckey] && cache[ckey].open) {
            ...
            source[method](zoom, xyz.minX, xyz.minY, cache[ckey].loader(done));
        }
        function queryVector(err, zpbf) {
            ...
            zlib.inflate(zpbf, function(err, pbf) {
                ...
                var vt = new mapnik.VectorTile(zoom, xyz.minX, xyz.minY);
                vt.setData(pbf,function(err) {
                    if (err) throw err;
                    // @TODO respect geocoder_layer if set.
                    var results = vt.query(lon, lat, {tolerance:10})||[];
                    ...
                    callback(null, ops.feature(results[0]._id, type, results[0]));
                });
            });
        }
