...
function stackFeatures(geocoder, loaded, options) {
    ...
    var memo = {};
    var firstType = false;
    var reverseMode = options.reverseMode || 'distance';
...
function lightFeature(source, feat, callback) {
    ...
    var loaded = { properties: Object.assign({}, feat) };
...
function fullFeature(source, feat, query, callback) {
    feature.getFeatureById(source, feat.id, function(err, loaded) {
        ...
        if (source.geocoder_address && loaded.properties['carmen:addressnumber']) {
            var addrpt = addressCluster.reverse(loaded, query);
        }

        if (source.geocoder_address && loaded.properties['carmen:rangetype']) {
            var addritp = addressItp.reverse(loaded, query);
        }
        ...
        if (addrpt && addritp) {
            ...
            if (distance(qFeat, addritp) < distance(qFeat, addrpt) && distance(addrpt, addritp, 'kilometers') > 0.2) {
                loaded = addritp;
            } else {
                loaded = addrpt;
            }
        } else {
            loaded = addrpt ? addrpt : addritp;
        }
