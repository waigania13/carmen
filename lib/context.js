...
module.exports = function(geocoder, lon, lat, maxtype, callback) {
    ...
    q.awaitAll(function(err, res) {
        ...
        callback(err, res.length && res[0]._center ? res : []);
    });
            ...
            zlib.inflate(zpbf, function(err, pbf) {
                ...
                vt.setData(pbf,function(err) {
                    ...
                    for (var i = 0; i < results.length; i++) {
                        ...
                        attr._extid = type + '.' + results[i].id();
                        results[i] = attr;
                    }
                    if ('_center' in results[0]) results.sort(function(a, b) {
                        var ad = Math.sqrt(Math.pow(Math.abs(lon-a._center[0]),2) + Math.pow(Math.abs(lat-a._center[1]),2));
                        var bd = Math.sqrt(Math.pow(Math.abs(lon-b._center[0]),2) + Math.pow(Math.abs(lat-b._center[1]),2));
                        return ad < bd ? -1 : ad > bd ? 1 : 0;
                    });
                    var res = results[0];
                    // Massage vector tile data into usable context data.
                    if (res._text||res.name||res.search) {
        ...
        function queryGrid(err, grid) {
            ...
            if (grid) {
                ...
                for (var i = 0; i < scanDirections.length; i++) {
                    ...
                    if (res && (res._text||res.name||res.search) && (res._center||(res.lon&&res.lat))) {
                        res._extid = type + '.' + (res._id||key);
                        res._text = res._text || res.name || res.search;
                        res._center = res._center || [res.lon,res.lat];
                        if (res.bounds) res._bbox = res.bounds.split(',').map(function(v) { return parseFloat(v) });
                        delete res.lon;
                        delete res.lat;
                        delete res.bounds;
