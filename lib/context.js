...
function stackFeatures(geocoder, loaded, options) {
    ...
    for (var i = 0; i < loaded.length; i++) {
        ...
        if (!memo[type]) {
            ...
            var displaced = memo[type];
            delete memo[displaced.properties['carmen:types'][0]];
            delete memo[displaced.properties['carmen:conflict']];
...
function loadFeature(source, feat, full, query, language, callback) {
    ...
    if (!full) {
        ...
        loaded.properties['carmen:types'] = feat['carmen:types'] || [source.type];
