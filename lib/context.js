    ...
    function query(err, vt) {
        ...
        vt.query(lon, lat, {
            ...
            for (var i = 0; i < results.length; i++) {
                ...
                if (attr._text) { attr = feature.transform(attr); }
        
                // If geojson has an id in properties use that otherwise use VT id
                attr.id = attr.id || attr.id || results[i].id();
                ...
                if (attr.properties["carmen:score"] < 0 && !matched[tmpid]) continue;
...
function loadFeature(source, feat, full, query, callback) {
    ...
    if (!full) {
        ...
        loaded._tmpid = (dbidx*mp25) + termops.feature(feat._id);
        loaded._dbidx = dbidx;
        loaded._text = feat._text || feat.name || feat.search;
        return callback(null, loaded._text ? loaded : false);
    }

    feature.getFeatureById(source, feat._id, function(err, loaded) {
        if (err) return callback(err);
