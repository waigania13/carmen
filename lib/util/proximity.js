...
module.exports.scoreRelevance = scoreRelevance;
module.exports.distRelevance = distRelevance;
 ...
 *
 * @param {Number} score The prominence score of the feature.
 ...
 */
function scoredist(score, dist, zoom, radius) {
    const scoreVal = scoreRelevance(score);
    const distVal = distRelevance(dist, zoom, radius);
    return parseFloat((distVal * scoreVal).toFixed(4));
}

function distRelevance(dist, zoom, radius) {
    zoom = Math.min(zoom, 14);
    const weightedRadius = radius * (15 - zoom);
    // features at or beyond proximity radius receive no dist weighting
    if (dist >= weightedRadius) return 1;
    // limit the smallest possible dist value to 10 feet or 10 / 5280 miles
    // this limits the max distVal to (1 / (10 / 5280))
    dist = Math.max(dist, 10 / 5280);
    // the closer the normalized distance is to 0, the higher the distVal
    const distVal = 1 / (dist / weightedRadius);
    return distVal;
}

function scoreRelevance(score) {
    // normalize exponential score values along a logarithmic curve
    const scoreVal = Math.log(1 + score);
    // set a min score of 1
    if (scoreVal <= 1) return 1;
    return scoreVal;
}
