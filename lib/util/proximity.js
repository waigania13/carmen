 ...
 */
function scoredist(dist, zoom, radius, scoreConstant) {
    // ignoring the passed-in radius for now -- we can constantize this later if we like it
    let effective_radius = 0;
    if (zoom <= 6) {
        // country, region
        effective_radius = 200;
    } else if (zoom <= 12) {
    ...
    // gaussian, incorporating effective radius
    const distWeight = Math.pow(Math.E, (-1 * Math.PI * Math.pow(dist / effective_radius, 2)));

    // heuristic: the closest features can be up to 100x times the geometric mean
    return parseFloat((1.25 * scoreConstant * distWeight).toFixed(4));
}
