 ...
 */
function scoredist(score, maxScore, dist, type) {
    const scoreVal = scoreWeight(score, maxScore);
    const distVal = distWeight(dist, type);
    return parseFloat((distVal * scoreVal).toFixed(6));
}
 ...
 * @return {Number} scaled score value
 */
function distWeight(dist, type) {
    const radius = scaleRadius(type);
    // features outside the proximity radius receive no dist weighting
    if (dist >= radius) return 1;
    // set gaussVal to 0.5 when normalized distance is 1/2 of the proximity radius (e.g. 100 / 200 miles)
    const gaussVal = gauss(dist / radius, variance(0.5, 0.5));
 ...
 */
function gauss(nDist, stdsq, offset = 0) {
    return Math.exp(-0.5 * Math.pow(Math.max(0, nDist - offset), 2) / stdsq);
}
