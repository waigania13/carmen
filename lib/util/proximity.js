 ...
 * @param {String} radius The proximity radius.
...
function distWeight(dist, zoom, radius) {
    const distRatio = dist / scaleRadius(zoom, radius);
 ...
 * @param {String} zoom The vector tile zoom level of the index the feature is part of.
 * @param {String} radius The proximity radius defined on the index the feature is a part of.
 * @return {Number} scaled radius in miles
 */
function scaleRadius(zoom, radius) {
    if (radius) {
        // The proximity radius is configured on the index level
        return radius;
    } else if (zoom <= 6) {
        // country, region
        ...
        // poi, address (if address index doesn't have radius configured)
