...
function getHousenumRangeV3(doc) {
    ...
    if (doc._cluster) {
        ...
        for (var p = 0; p < props.length; p += 2) {
            ...
            var a = Array.isArray(doc[props[p]]) ? doc[props[p]] : [doc[props[p]]];
            var b = Array.isArray(doc[props[p+1]]) ? doc[props[p+1]] : [doc[props[p+1]]];
...
function permutations(terms, weights, all) {
    var masks = all ? permute.all(terms.length) : permute.continuous(terms.length);
    ...
    for (var i = 0; i < masks.length; i++) {
        ...
        if (weights) {
            ...
            for (var j = 0; j < length; j++) {
                ...
                permutation.relev = Math.round(permutation.relev*100)/100;
            }
        } else {
            for (var j = 0; j < length; j++) {
                if (!(mask & (1<<j))) continue;
...
function getIndexablePhrases(tokens, freq) {
    ...
    for (var i = 0; i < perms.length && i < 5; i++) {
        if (perms[i].relev < 0.8) break;
        perms[i].relev = Math.round(perms[i].relev * 5) / 5;
    ...
    if (numTokens) {
        while (i--) {
            if (weights[i] === -1) {
                ...
                weights[i] = Math.max(weights[i] / weightsum) * 0.99;
            }
        }
    } else {
        while (i--) {
