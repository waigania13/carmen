...
function id(geocoder, query) {
    ...
    // no indexes from shard geocoder
    if (!geocoder.bytype || !geocoder.bytype[dbname]) return false;
...
function getIndexableText(replacer, globalTokens, doc) {
    ...
    for (var langText of langTexts) getTokens(doc.properties[langText[0]].split(','), langText[1]);
