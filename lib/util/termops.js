...
function decollide(replacer, doc, subq) {
    ...
    // subq is MFS text
    // decolliding because 'main' isn't accounted for in the query
    ...
    console.log('texts in collide', texts); // possibly loop through the addressnumbers and add each to the text (brutal)
    var a = texts.length;
    var fails = 0;
    while (a--) {
