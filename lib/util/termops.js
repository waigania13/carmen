...
function getIndexableText(replacer, globalTokens, doc) {
    ...
    // get tokens for universal text
    if (doc.properties['carmen:text_universal']) {
        getTokens(doc.properties['carmen:text_universal'].split(','), 'all');
    // get tokens for default text.
    // if there aren't other translations, index across languages
        getTokens(texts, (langTexts.size > 1) ? 'default' : 'all');
    } else {
        getTokens(texts, langTexts.size ? 'default' : 'all');
    }

    // get tokens for each of the non-universal, non-default languages' text
