...
function getIndexableText(replacer, globalTokens, doc) {
    ...
    for (var i = 0; i < length; i ++) {
        ...
        if (code) {
            ...
            if (!cl.hasLanguage(language) && language != 'universal') throw new Error(code[1] + ' is an invalid language code');
            if (doc.properties[keys[i]]) langTexts.set(keys[i], language);
        }
    }

    var texts = doc.properties['carmen:text'].split(',');
    if (doc.properties['carmen:text_universal']) {
        universalTexts = doc.properties['carmen:text_universal'].split(',');
        getTokens(universalTexts, 'all');
        getTokens(texts, (langTexts.size > 1) ? 'default' : 'all');
    } else {
        getTokens(texts, langTexts.size ? 'default' : 'all');
    }
    ...
    // langTexts now has universal- will still want to de-penalize default when universal present?

    // exclude universal from decision about default
    // index universal for all alnguages, like default is

    // get tokens for default
    // getTokens(texts, langTexts.size ? 'default' : 'all');
    // get tokens for each of the languages (includes universal)
    for (var langText of langTexts) {
        language = langText[1];
        console.log('lang:', language);
        if (language != 'universal') {
            getTokens(doc.properties[langText[0]].split(','), language);
        }
    }

    function getTokens(texts, language) {
        // when indexing, language is `en`, `universal`, `default`. When querying is `all`
        // console.log('texts:', texts, language);
