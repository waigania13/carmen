...
var fnv1a = require('./dataint').fnv1a;
var encodeTerm = require('./dataint').encodeTerm;
var encodeDegen = require('./dataint').encodeDegen;
...
module.exports.rangeToToken = rangeToToken;
module.exports.rangeFromToken = rangeFromToken;
...
module.exports.termsWeighted = function(tokens, freq) {
    ...
    for (i = 0; i < tokens.length; i++) {
        terms[i] = encodeTerm(tokens[i], Math.floor(weights[i]/maxweight*15));
    }
    return terms;
};
...
function getHousenumRange(doc) {
    ...
    var max = -Infinity;
    if (doc._cluster) {
        var keys = Object.keys(doc._cluster);
        ...
        return isFinite(min) && isFinite(max) ? {type:'range', min:min, max:max} : false;
...
module.exports.getIndexableText = function(replacer, doc) {
    ...
    for (var x = 0; x < texts.length; x++) {
        ...
        if (!range) continue;
        var tokensWithRange = [rangeToToken(range)].concat(tokens);
        var keyWithRange = tokensWithRange.join(' ');
        if (uniqTexts[keyWithRange]) continue;
        uniqTexts[keyWithRange] = true;
        indexableText.push(tokensWithRange);
    }
