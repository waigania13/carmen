...
function getIndexableText(replacer, doc) {
    ...
    for (var i = 0; i < keys.length; i ++) {
        ...
        if (code && mun.hasLanguage(code[1])) getTokens(doc.properties[keys[i]].split(','));
    }
...
function decollide(replacer, doc, subq) {
    ...
    for (var i = 0; i < keys.length; i ++) {
        var code = keys[i].match(/text_(.+)/);
        if (code && mun.hasLanguage(code[1])) texts = texts.concat(token.replaceToken(replacer, doc.properties[keys[i]]).split(','));
    }
