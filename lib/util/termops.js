...
var fs = require('fs')
var CJK2char = fs.readFileSync(__dirname+'/CJK2char.txt', {encoding: 'utf8'}).split('\n')
var CJK3char = fs.readFileSync(__dirname+'/CJK3char.txt', {encoding: 'utf8'}).split('\n')
 ...
 */
module.exports.segmentWords = segmentWords
function segmentWords(pretoken) {

    var tokenized = [];
    var previous = '';

    for (var i=0; i < pretoken.length; i) {
        // keep track of the previously-added token (if any)
        if (tokenized.length > 0) {
            previous = tokenized[tokenized.length-1]
        }
        // Grab the next trigram (3-character substring)
        var substring = pretoken.slice(i,i+3)

        // Is the trigram listed in CJK3Char?
        if (CJK3char.indexOf(substring) > -1) {
            // add it as a token
            tokenized.push(substring)
            // move forward one character
            i = i+1
        }
        // If not, is the first bigram of the substring listed in CJK2char?
        else if (CJK2char.indexOf(substring.slice(0,2)) > -1) {
            // add the bigram, unless it overlaps with the previous token
            if (substring.slice(0,2) != previous.slice(1)) {
                tokenized.push(substring.slice(0,2))
            }
            // move forward two characters
            i = i+2
        }
        // If not, is the second bigram of the substring listed in CJK2char?
        else if (CJK2char.indexOf(substring.slice(1)) > -1) {
            // add the unigram at the beginning of the substring
            tokenized.push(substring[0])
            // add the second bigram
            tokenized.push(substring.slice(1))
            // move forward 3 characters
            i = i+3
        }
        // No significant ngrams
        else {
            // add the substring as a separated array of characters
            tokenized = tokenized.concat(substring.split(''))
            i = i+3
        }
    }
