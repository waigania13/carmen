...
function getHousenumRangeV3(doc) {
    ...
    if (doc.properties["carmen:addressnumber"]) {
        ...
        for (var i = 0; i < keys.length; i++) {
            ...
            var numToken = keys[i];
            // force it to leave in suite numbers, ex: '20b', so that '14th' will also survive
            // var numToken = parseSemiNumber(keys[i]);
            // if (numToken === null) continue;
            add(numTokenV3(numToken.toString()));
            // add(numToken.toString());
        }
...
function decollide(replacer, doc, subq) {
    //doc is the entire address/street feature, with array of housenums/intersections
    ...
    // add addresses associated with the feature
    var textAdds = token.replaceToken(replacer, doc.properties['carmen:addressnumber']);
    if (textAdds) {
        for (var i = 0; i < textAdds.length; i++) {
            texts = texts.concat(token.replaceToken(replacer, textAdds[i]));
        };
    };
    // console.log('texts in collide', texts);

    var a = texts.length;
    var fails = 0;

    // Pull textHash out of loop, so all characters get added
    var textHash = {
        32: true, // ' ' for spaces
    };
    
    // loop through all text on the feature, add chars to hash. Check against subquery
    while (a--) {
...
// strip off non-digit characters from potential addresses, return null if no digits
...
// Where non-numeric addresses get ditched
module.exports.numTokenize = numTokenize;
function numTokenize(text, version) {
    if (typeof text === 'string') text = tokenize(text);
    var numTokenized = [];
    for (var i = 0; i < text.length; i++) {
        var replaced = text.slice(0); // copy array
        // var num = parseSemiNumber(address(text[i])); // strip non-digits. Null if all non-digits
        var num = address(text[i]);
        if (num !== null) { // if first token is a number
            // change digits to '#'s depending on length
            replaced[i] = version >= 3 ?
                numTokenV3(num.toString()) :
                numTokenV2(num.toString());
            numTokenized.push(replaced); // push *entire array copy*
        }
    }
    return numTokenized;
}

// returns digits converted to '#'
...
function numTokenV3(str) {
    // if first character is a letter, assume it's a cross street and return un-hashed
    if (!Number(str[0])) {
        console.log('returning whole string:', str);
        return str;
    };
