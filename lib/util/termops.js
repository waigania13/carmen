...
function getIndexableText(replacer, globalTokens, doc, markVariants) {
    ...
    function getTokens(texts, language) {
        ...
        if (globalTokens) combinedReplacer = combinedReplacer.concat(globalTokens);
        if (replacer) combinedReplacer = combinedReplacer.concat(replacer);

        for (var x = 0; x < texts.length; x++) {
            // push tokens with replacements
            let replacementChoices = token.enumerateTokenReplacements(combinedReplacer, texts[x]);
            let variants = [];
            if (replacementChoices.length == 1) {
                ...
                // bail if there are too many choices -- 2^3 without marked variants, or 2^5 if so
                let maxSlots = markVariants ? 5 : 3;
                let prod = [];
                if (cartArray.length > maxSlots) {
                    prod = [Array(cartArray.length).fill(0)];
                } else {
                    // just use the first 8 if there are more
                    prod = cartProd(cartArray).slice(0, 1<<maxSlots);
                }
