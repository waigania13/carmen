...
function encodePhrase(tokens, skipEncoding) {
    ...
    if (text.length < 4) {
        var encoded = parseInt(murmur.hash128(text).hex().substr(-9), 16);
        var cluster = 0;
    } else {
        var encoded = parseInt(murmur.hash128(text).hex().substr(-9), 16);
        var cluster = parseInt(murmur.hash128(text.substr(0,4)).hex().substr(-4),16) * Math.pow(16,9);
    }
    return cluster + encoded;
};
...
function getIndexablePhrases(tokens, freq) {
    ...
    for (var i = 0; i < perms.length; i++) {
        ...
        var obj = {
            ...
            phrase: encodePhrase(perms[i].join(' '))
        };
