...
function keyToLanguageCode(key) {
    var cache = keyToLanguageCode.cache;
    if (cache[key] !== undefined) return cache[key];

    var code = key.match(keyToLanguageCode.pattern);
    if (code) {
...
keyToLanguageCode.cache = {};
keyToLanguageCode.pattern = /text_(.+)/;
...
function getIndexableText(replacer, globalTokens, doc, markVariants) {
    ...
    for (var key in doc.properties) {
        var language = keyToLanguageCode(key);
        if (language && doc.properties[key]) langTexts.set(key, language);
    }
    ...
    function getTokens(texts, language) {
        ...
        for (var x = 0; x < texts.length; x++) {
            ...
            for (let variant of variants) {
                var tokens = encodableText(tokenize(variant).join(' '));
                ...
                if (range) {
                    var l = range.length;
                    while (l--) {
                        var withHousenums = `${range[l]} ${tokens}`;
                        add(withHousenums, language, primary ? `${range[l]} ${primary}` : null);
                    }
                }
    ...
    function add(tokens, language, variantOf) {
        ...
        if (variantOf) {
            phraseVariants[variantOf] = phraseVariants[variantOf] || new Set();
            phraseVariants[variantOf].add(tokens.split(' '));
        } else {
            variantOf = tokens;
        }

        indexableText[variantOf] = indexableText[variantOf] || new Set();
        indexableText[variantOf].add(language)
    }

    var output = [];
    for (var phrase in indexableText) {
        let obj = { tokens: phrase.split(' '), languages: Array.from(indexableText[phrase]) }
        if (phraseVariants[phrase]) obj.variants = Array.from(phraseVariants[phrase]);
        output.push(obj);
    }
...
function permutations(terms, weights, all, cutoff) {
    cutoff = cutoff || 1;
    ...
    for (var i = 0; i < masks.length; i++) {
        ...
        if (weights) {
            ...
            if (permutation.relev < cutoff) continue;
        } else {
...
function getIndexablePhrases(tokens, freq) {
    var uniq = {};
    var phrases = [];
    var perms = permutations(tokens, getWeights(tokens, freq), true, 0.8);
    ...
    for (var i = 0; i < perms.length; i++) {
        ...
        var etext = perms[i].join(' ');

        // Uses the highest scored phrase via sort.
        if (uniq[etext]) continue;
...
function numTokenV2(num) {
    return ('#').repeat(num.toString().length);
}

module.exports.numTokenV3 = numTokenV3;
function numTokenV3(num) {
    if (num < 10) return '#';
    if (num < 100) return '##';
    var str = num.toString();
    return str.substr(0,2) + ('#').repeat(str.length - 2);
}
