...
function getFeatureByCover(source, cover, callback) {
    var s = shard(source._geocoder.shardlevel, cover.id);
...
function getFeatureById(source, id, callback) {
    ...
    getHash(source, hash, function(err, loaded) {
        if (err) return callback(err);
...
function putFeatures(source, docs, callback) {
    ...
    for (var i = 0; i < docs.length; i++) {
        var doc = docs[i];
        var s = shard(source._geocoder.shardlevel, termops.feature(doc._id));
        byshard[s] = byshard[s] || [];
        byshard[s].push(doc);
    }
    var q = queue(100);
    for (var s in byshard) q.defer(function(s, docs, callback) {
        source.getGeocoderData('feature', s, function(err, buffer) {
            ...
            for (var i = 0; i < docs.length; i++) {
                var doc = docs[i];
                var hash = termops.feature(doc._id);
                if (typeof current[hash] === 'string') {
                    try {
                        current[hash] = JSON.parse(current[hash]);
                    } catch(err) {
                        return callback(err);
                    }
                } else if (typeof current[hash] === 'undefined') {
                    current[hash] = {};
                }
                if (!hashesUniq[hash]) {
                    hashes.push(hash);
                    hashesUniq[hash] = true;
                }
                current[hash][doc._id] = doc;
