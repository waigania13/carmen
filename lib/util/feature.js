...
function getFeatureByCover(sources, callback) {
    var features = [];

    for (i = 0; i < sources.length; i++) {
        var source = sources[i].source;
        var cover = sources[i].cover;
    ...
    // console.log('features:', features); // Sucessfully has two features, fake and main, then hangs :( because of promise magic?
    return callback(null, features);
}
