...
function transform(feat) {
    ...
    while (l--) {
        if (keys[l] === '_id') {
            ...
            doc.properties["carmen:addressnumber"] = Object.keys(cluster).map(function(address) {
                var coord = typeof cluster[address] === 'string' ? JSON.parse(cluster[address]).coordinates : cluster[address].coordinates;
                doc.geometry.coordinates.push(coord);
                return address;
            });
        } else {
            if (keys[l].indexOf('_') === 0) {
...
function putFeatures(source, docs, callback) {
    ...
    for (var i = 0; i < docs.length; i++) {
        ...
        if (!doc.properties['carmen:zxy']) {
            tiles = cover.tiles(doc.geometry, {min_zoom: source._info.maxzoom, max_zoom: source._info.maxzoom});
