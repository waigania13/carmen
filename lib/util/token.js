...
module.exports.enumerateTokenReplacements = function(tokens, query) {
    ...
    for (let token of tokens) {
        ...
        for (var i = 0; i < Math.min(out.length, 64); i++) {
            ...
            for (var j = 0; j < occurrenceCount; j++) {
                ...
                var repFunc = function(match) {
                    ...
                    if (occurrence == j) {
                        ...
                        out = typeof token.to == 'function' ?
                            ...
                            // this replaces the '$n' params with the parnethesized matches
                ...
                if (token.named) {
                    ...
                    // not clear if it's even possible for XRegExp, but for regular ones, use an alternate
                    // non-global regex so we can do one replacement at a time
                    replaced = out[i].replace(token.from, repFunc);
                }
