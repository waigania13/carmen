...
// Load all shards for a given type/queue of ids as dicts.
Cache.prototype.dictall = function(getter, type, ids, callback) {
    var cache = this;
    var queues = Cache.shards(type, ids);
    ...
    var shards = [];
    for (var i = 0; i < allshards.length; i++) {
        if (!cache.hasDict(type, +allshards[i])) {
            shards.push(allshards[i]);
        }
    }
    ...
    q.awaitAll(function(err, buffers) {
        ...
        for (var i = 0; i < buffers.length; i++) {
            ...
            // experimental/not yet stable
            try {
                cache.loadAsDict(buffers[i], type, +shards[i]);
            } catch (e) {
                return callback(e);
            }
        }
