...
Cache.shardsort = function(level, arr) {
    arr.sort(sortMod);
    function sortMod(a,b) { return a < b ? -1 : a > b ? 1 : 0 }
};
...
Cache.prototype.getall = function(getter, type, ids, callback) {
    ...
    function loadshard(shard, queue, force, callback) {
        ...
        function cacheloadshard(err, buffer) {
            if (err) return callback(err);
            else if (!buffer) return immediate(function(){ loadshard(shard, queue, true, callback) });
            else {
                try {
                    // Sync load is used because async call is
                    // experimental/not yet stable
                    cache.loadSync(buffer, type, shard);
                    immediate(function() { loadshard(shard, queue, false, callback)});
                } catch(e) {
                    error(e);
                }
            }
...
Cache.prototype.unloadall = function(type) {
    var shardlevel = this.shardlevel;
    for (var i = 0; i < Math.pow(16, shardlevel); i++) this.unload(type, i);
    return true;
};
