...
Cache.shard = function(type, level, id) {
    // for 'look' index limit shardlevel to between 0-2
    // offset from main shardlevel by -2.
    if (type === 'look') level = Math.min(2,Math.max(0,level-2));
...
Cache.shards = function(type, level, queue) {
    // for 'look' index limit shardlevel to between 0-2
    // offset from main shardlevel by -2.
    if (type === 'look') level = Math.min(2,Math.max(0,level-2));
...
Cache.prototype.set = function(type, id, data, append) {
    var shard = Cache.shard(type, this.shardlevel, +id);
...
Cache.prototype.exists = function(type, id) {
    var shard = Cache.shard(type, this.shardlevel, +id);
    return this._exists(type, shard, +id);
};
...
Cache.prototype.loadall = function(getter, type, ids, callback) {
    ...
    var cache = this;
    var queues = Cache.shards(type, shardlevel, ids);
