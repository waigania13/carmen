...
function dedupe(features) {
    ...
    for (var i = 0; i < features.length; i++) {
        ...
        // if we've seen this place_name before
        if (previous) {
            // if the current feature has more geometry, keep the that one
            if (a[previous.index].geometry.omitted && !feature.geometry.omitted) {
                a[previous.index] = feature;
            // if the current feature isn't itp, keep it
            ...
            // otherwise save the feature and location to the 'seen' list
            by_place_name[feature.place_name] = {
                feature: feature,
                index: a.length
            };
            // save the feature to our master list
            a.push(feature);
        }
    }
