...
var fallbackDisplay = require('./fallback-display.json');
var fallbackIndexer = require('./fallback-indexer.json');

// handle case-insensitive language fallback lookups
Object.keys(fallbackDisplay).forEach(function(lang) {
    fallbackDisplay[lang.toLowerCase()] = fallbackDisplay[lang];
});
Object.keys(fallbackIndexer).forEach(function(lang) {
...
var getScoredCandidates = function(target, candidateList) {
    var munTarget = getLanguageArray(target);
    var munCandidates = candidateList.map(function(item) { return getLanguageArray(item); });
...
var closestLangLabel = function(target, candidates, prefix, languageMode) {
    ...
    var scoredCandidates = getScoredCandidates(target, regexCandidates);
...
var fallbackRanking = function(target, candidateList) {
    target = target.replace("-", "_");
    var primary = target.split('_')[0].toLowerCase(); // e.g., the `en` part of `en_US`
    ...
    var fb = fallbackIndexer[target.toLowerCase()], fb_i;
    if (fb) for (fb_i = 0; fb_i < fb.length; fb_i++) {
    ...
    // now try heuristics to get something close
    var scoredCandidates = getScoredCandidates(target, candidateList);
    if (scoredCandidates) {
        scoredCandidates.forEach(function(cand) {
            if (cand.score >= 50) {
...
var fallbackMatrix = function(candidateList) {
    var output = new Map();
    for (var candidate of candidateList) {
