 ...
 * @param {Boolean} dev Enable dev mode to expose internal properties in results
 * @return {Object} Carmen GeoJSON feature
 */
function toFeature(context, format, language, dev) {
    var feat = context[0];
    ...
    for (var key in context[0].properties) if (key.indexOf('carmen:') !== 0 || dev) {
        if (key === 'id') continue;
