...
module.exports.fixContext = fixContext;
...
function toFeature(context, format, language, languageMode, debug, geocoder) {
    ...
    for (var k = 0; k < context.features.length; k++) {
        if (languageFormat) {
            var languageText = language ? closestLang(language, context.features[k].properties, "carmen:text_") : false;
            if (languageText ||
                context.features[k].properties.language === language) {
                format = languageFormat;
                break;
            }
        }

        if (context.features[k].properties['carmen:query_text'] && geocoder) {
            var added = addMatchingText(context.features[k], geocoder, language);
    ...
    passes.forEach(function(pass) {
        ...
        if (!format || format === true || format === 1) {
            ...
            for (var i = 0; i < template.length; i++) {
                ...
                for (var x = 0; x < context.features.length; x++) {
                    var f = context.features[x];
    ...
    for (var key in context.features[0].properties) if (key.indexOf('carmen:') !== 0 || debug) {
        if (key === 'id') continue;
        feature.properties[key] = context.features[0].properties[key];
    }
    if (context.features.length > 1) {
        feature.context = {features: []};
        for (var c = 1; c < context.features.length; c++) {
            if (!context.features[c].properties['carmen:extid']) throw new Error('Feature has no carmen:extid');
            if (!featureMatchesLanguage(context.features[c], { language: language, languageMode: languageMode })) continue;

            var contextFeat = closestLang.getText(language, context.features[c].properties);
            contextFeat.id = context.features[c].properties['carmen:extid'];

            // copy over all non-'carmen:*' properties
            var propertyKeys = Object.keys(context.features[c].properties).filter(function(prop) {
                return !/^carmen:/.test(prop) && !/^id$/.test(prop);
            });

            for (var j = 0; j < propertyKeys.length; j++)
                contextFeat[propertyKeys[j]] = context.features[c].properties[propertyKeys[j]];

            if (context.features[c].matching_text) contextFeat.matching_text = context.features[c].matching_text;
            if (context.features[c].matching_language) contextFeat.matching_language = context.features[c].matching_language;

            feature.context.features.push(contextFeat);
        }
    }
...
/**
 * fixContext - Corrects context nesting needed for returning sharded results
 * TODO: fix it for reals.
 *
 * @param {Object} GeoJSON feature collection
 * @return {Object} Carmen GeoJSON feature
 */
function fixContext(featureCollection) {
    if (featureCollection.features) {
        for (var i=0; i<featureCollection.features.length; i++) {
            // if that particular feature has a context
            if (featureCollection.features[i].context) {
                var current = featureCollection.features[i].context;
                // if context is nested, unnest
                if (current.features) {
                    featureCollection.features[i].context = current.features;
                } else {
                    // if one context is already correct, they all are, so don't change it
