...
module.exports.toFeature = function(context, format) {
    ...
    if (!format || format === true || !feat._address) {
        place_name = ((feat._address ? feat._address + ' ' : '') + context.map(gettext).join(', ')).trim();
    } else {
        var contextMap = context.map(gettext);
        var name = contextMap.shift();
        format = format.replace('{num}', feat._address).replace('{name}', name).replace(/\{.+?\}/, '', 'g');
        place_name = (format + ' ' + contextMap.join(', ')).trim();
    }
