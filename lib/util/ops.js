...
module.exports.toFeature = function(context, format) {
    ...
    if (!format || format === true || format === 1) {
        ...
        // Tidy entries, remove curlies, commas
        for (var i = 0; i < template.length; i++) {
          ...
          for (var x = 0; x < context.length; x++) {
            ...
            if(templateProp == carmenProp) {
              if(templateSubprop == 'name') {
                // `name` is a hardcoded subproperty that maps to carmen:text
                val = (f.properties['carmen:text']||'').split(',')[0];
                format = format.replace('{' + templateProp + '.name}',val);
              } else if(templateSubprop == 'number') {
                // `address` is a hardcored subproperty that maps to carmen:address
                val = (f.properties['carmen:address']||'').split(',')[0];
                format = format.replace('{' + templateProp + '.number}',val);
              } else {
        ...
        format = format.replace(/\{.+?\}/g, '').replace(/ \, /g,', ').replace(/^\, /,'').replace(/, ,/,'').replace(/  /,' ').trim();
