 ...
 * @param {String} pass Set to 'matching' to enable autoc synonym matching fallback
 * @return {String} formatted place_name string
 */
function getPlaceName(context, formatString, language, languageMode, pass) {
    var feat = context[0];
    var place_name;
    ...
    languages.reduce(function(memo, language, i) {
        ...
        if (i === 0) {
            ...
            if (!formatString && geocoder) {
                for (var k = 0; k < context.length; k++) {
                    if (context[k].properties['carmen:query_text'] &&
                        addMatchingText(context[k], geocoder, language)) {
                        feature.matching_place_name = getPlaceName(context, formatString, language, languageMode, 'matching');
                        break;
                    }
                }
            }
