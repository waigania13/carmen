...
function toFeature(context, format, language, debug) {
    ...
    if (!format || format === true || format === 1) {
        place_name = ((feat.properties['carmen:address'] ? feat.properties['carmen:address'] + ' ' : '') + context.features.map(gettext).join(', ')).trim();
    } else {
    ...
    if (context.features.length > 1) {
        ...
        for (var c = 1; c < context.features.length; c++) {
            ...
            var contextFeat = _gettext(context.features[c]);
