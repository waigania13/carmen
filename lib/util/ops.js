...
module.exports.toFeature = function(context) {
    ...
    if (!feat._format || feat._format === true || !feat._address) {
        ...
        var format = feat._format.replace('{num}', feat._address).replace('{name}', name).replace(/\{.+?\}/, '', 'g');
        place_name = format + contextMap.join(', ');
    }
