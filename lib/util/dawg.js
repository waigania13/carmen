...
var WriteCache = function() {
    ...
    this.normalizationTargets = new Map();
}
...
WriteCache.prototype.setText = function(text, mapsToSelf) {
    ...
    if (text == "") {
        ...
        const first = new Buffer(text).slice(0,6).toString('hex');
...
WriteCache.prototype.setNormalization = function(nonstandard, standard) {
    ...
    const nonkey = new Buffer(nonstandard).slice(0,6).toString('hex');
    let subMap = this.normalizationMap.get(nonkey);
    if (!subMap) {
        subMap = new Map();
        this.normalizationMap.set(nonkey, subMap);
    }
    if (!subMap.has(nonstandard)) subMap.set(nonstandard, new Set());
    subMap.get(nonstandard).add(standard);

    const key = new Buffer(standard).slice(0,6).toString('hex');
    let subSet = this.normalizationTargets.get(key);
    if (!subSet) {
        subSet = new Set();
        this.normalizationTargets.set(key, subSet);
    }
    subSet.add(standard);
}
...
WriteCache.prototype.dumpWithNormalizations = function(normalizationCachePath) {
    ...
    for (var i = 0; i < prefixes.length; i++) {
        ...
        for (var j = 0; j < phrases.length; j++) {
            ...
            let first = new Buffer(phraseString).slice(0,6).toString('hex');
            let subMap = this.normalizationMap.get(first);
            let subSet = this.normalizationTargets.get(first);

            dawg.insert(phraseString);

            if ((subMap && subMap.has(phraseString)) || (subSet && subSet.has(phraseString))) {
    ...
    let normData = [];
    this.normalizationMap.forEach((subMap) => {
        for (let [key, values] of subMap) {
