...
WriteCache.prototype.setText = function(text, mapsToSelf) {
    ...
    if (text == "") {
        ...
        var first = new Buffer(text).slice(0,6).toString('hex');
...
WriteCache.prototype.setNormalization = function(nonstandard, standard) {
    ...
    var first = new Buffer(nonstandard).slice(0,6).toString('hex');
    var subMap = this.normalizationMap.get(first);
    if (!subMap) {
        subMap = new Map();
        this.normalizationMap.set(first, subMap);
    }
...
WriteCache.prototype.dumpWithNormalizations = function(normalizationCachePath) {
    ...
    for (var i = 0; i < prefixes.length; i++) {
        ...
        for (var j = 0; j < phrases.length; j++) {
            ...
            if ((subMap && subMap.has(phraseString)) || this.normalizationTargets.has(phraseString)) {
