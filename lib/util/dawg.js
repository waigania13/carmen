...
WriteCache.prototype.setNormalization = function(nonstandard, standard) {
    console.log('[carmen/lib/util/dawg.js 30] WriteCache.setNormalization() called');
...
WriteCache.prototype.dumpWithNormalizations = function(normalizationCachePath) {
    ...
    for (let [key, values] of this.normalizationMap) {
        ...
        if (mapsToSelf.has(key)) {
            // This simpler expression of the four lines below can't be used
            // because es6-native-set does not support this in its constructor
            //
            // values = new Set([...values, key]);

            let cloned = new Set();
            values.forEach(cloned.add, cloned);
            cloned.add(key);
            values = cloned;
        }

        // This simpler expression of the four lines below can't be used
        // because es6-native-set does not integrate wth Array.from()
        //
        // normData.push([
        ...
        var normValues = [];
        values.forEach((v) => { normValues.push(normalizationIndexes.get(v)); });
        normValues.sort();
        normData.push([ normalizationIndexes.get(key), normValues ]);
    }
