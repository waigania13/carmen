...
ReadCache.prototype.getPhrasesFromDegen = function(string, max) {
    var phraseIterator =  _dawgCache.CompactDawg.prototype.iterator.call(this, string);
    var phraseIteratorNext = phraseIterator.next();
    var flat = [];
    if (max) {
        while (!phraseIteratorNext.done && flat.length < max) {
            flat.push(phraseIteratorNext.value);
            phraseIteratorNext = phraseIterator.next();
        }
    } else {
        while (!phraseIteratorNext.done) {
            flat.push(phraseIteratorNext.value);
            phraseIteratorNext = phraseIterator.next();
        }
    }
...
WriteCache.prototype.prefixContinuations = function(prefix, maxDepth) {
    if (DEBUG) console.warn("calling iterator on a DAWG WriteCache is very inefficient")
    return (new ReadCache(this.dump())).prefixContinuations(prefix, maxDepth);
}
