...
WriteCache.prototype.hasPhrase = function(text, ender) {
    var msg = "calling hasPhrase on a DAWG WriteCache is very inefficient";
    if (!TESTING) throw new Error(msg);
    console.warn(msg);
    return (new ReadCache(this.dump())).hasPhrase(text, ender);
}

WriteCache.prototype.iterator = function(string) {
    var msg = "calling iterator on a DAWG WriteCache is very inefficient";
    if (!TESTING) throw new Error(msg);
    console.warn(msg);
