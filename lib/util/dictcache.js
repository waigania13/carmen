...
function Dictcache(buffer, bitSize) {
    ...
    if (Buffer.isBuffer(buffer) && buffer.length) {
        ...
        this.filepath = tmp + '/carmen-dict-' + crypto.createHash('md5').update(buffer).digest('hex');
        fs.writeFileSync(this.filepath, buffer);
    } else {
        buffer = new Buffer(this.size/8);
        buffer.fill(0);
        this.filepath = tmp + '/carmen-dict-' + crypto.createHash('md5').update(Math.random().toString()).digest('hex');
        fs.writeFileSync(this.filepath, buffer);
    }
