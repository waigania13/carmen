...
function standardize(feat){
    ...
    if (feat._rangetype !== 'tiger' ||
        !feat._geometry ||
        (feat._geometry.type !== 'LineString' &&
        feat._geometry.type !== 'MultiLineString')) return;

    return {
        lines: feat._geometry.type === 'MultiLineString' ? feat._geometry.coordinates : [feat._geometry.coordinates],
        lf: Array.isArray(feat._lfromhn) ? feat._lfromhn : feat._lfromhn ? [feat._lfromhn] : [],
        lt: Array.isArray(feat._ltohn)   ? feat._ltohn : feat._ltohn ? [feat._ltohn] : [],
        rf: Array.isArray(feat._rfromhn) ? feat._rfromhn : feat._rfromhn ? [feat._rfromhn] : [],
        rt: Array.isArray(feat._rtohn)   ? feat._rtohn : feat._rtohn ? [feat._rtohn] : [],
        lp: Array.isArray(feat._parityl) ? feat._parityl : feat._parityl ? [feat._parityl] : [],
        rp: Array.isArray(feat._parityr) ? feat._parityr : feat._parityr ? [feat._parityr] : []
    };
...
module.exports = function(feature, address) {
    ...
    for (var i = 0; i < stdFeat.lines.length; i++) {
        var lfromhn = parseSemiNumber(stdFeat.lf[i]),
            rfromhn = parseSemiNumber(stdFeat.rf[i]),
            ltohn = parseSemiNumber(stdFeat.lt[i]),
            rtohn = parseSemiNumber(stdFeat.rt[i]),
            parityl = stdFeat.lp[i] || '',
            parityr = stdFeat.rp[i] || '';
        ...
        if (typeof lfromhn === 'number' &&
            ...
            paritymask[parityl]) {
            return setPoint(address, lfromhn, ltohn, stdFeat.lines[i].slice(0));
        // strict R match
        } else if (typeof rfromhn === 'number' &&
            address >= Math.min(rfromhn,rtohn) &&
            address <= Math.max(rfromhn,rtohn) &&
            paritymask[parityr]) {
            return setPoint(address, rfromhn, rtohn, stdFeat.lines[i].slice(0));
        // loose L match (no parity check)
        } else if (typeof lfromhn === 'number' &&
            address >= Math.min(lfromhn,ltohn) &&
            address <= Math.max(lfromhn,ltohn)) {
            return setPoint(address, lfromhn, ltohn, stdFeat.lines[i].slice(0), true);
        // loose R match (no parity check)
        } else if (typeof rfromhn === 'number' &&
            address >= Math.min(rfromhn,rtohn) &&
            address <= Math.max(rfromhn,rtohn)) {
            return setPoint(address, rfromhn, rtohn, stdFeat.lines[i].slice(0), true);
        // no range match, store line endpoint if closest so far
            ...
            for (var j = 0; j < 4; j++) {
                ...
                nearest.hn = hns[j];
                nearest.pt = j % 2 === 0 ? stdFeat.lines[i][0] : stdFeat.lines[i][1];
            }
            continue;
        }
    }
...
module.exports.reverse = function(feat,query) {
    ...
    var sideBinary = det2D(potential.startLine.geometry.coordinates, potential.endLine.geometry.coordinates, query);
    var leftSideBinary = det2D(
    ...
    if (sideBinary === leftSideBinary) { //If point is on the centre
        num = matchSide(feat, "left", potential.pt.properties.index);
    } else { //If point is on the right or on the line
        num =  matchSide(feat, "right", potential);
    }

    if (num) {
        feat._text = num + " " + feat._text;
    }
...
*/
function matchSide(feat, side, point, strict) {
    var opp = (side === "left") ? "right" : "left";
    var sideVar = (side === "left") ?
        {parity: "_parityl",to: "_ltohn",from: "_lfromhn" } :
        {parity: "_parityr",to: "_rtohn",from: "_rfromhn" };

    //If side x doesn't have data, check the other side
    if (!feat[sideVar.parity][point.i] && !strict) return matchSide(feat, opp, point, true);
    if (!feat[sideVar.parity][point.i] && strict) return null;

    var distRatio = point.pt.properties.dist / turf.lineDistance(turf.linestring(feat._geometry.coordinates[point.i]), 'miles');
    var num = parseInt(feat[sideVar.from][point.i]) + Math.round((parseInt(feat[sideVar.to][point.i]) - parseInt(feat[sideVar.from][point.i])) * distRatio);

    if (num % 2 === 0 && feat[sideVar.parity][point.i] === "O") ++num;
    else if (num % 2 === 1 && feat[sideVar.parity][point.i] === "E") --num;

    return num;
}

function det2D(start, end, query) { return sign((end[0]-start[0])*(query[1]-start[1]) - (end[1]-start[1])*(query[0]-start[0])); }
