...
function standardize(feat){
    ...
    var stdFeat = {
        lines: feat.geometry.type === 'MultiLineString' ? feat.geometry.coordinates : [feat._geometry.coordinates],
...
function matchSide(feat, side, point, strict) {
    ...
    var sideVar = (side === "left") ?
        {parity: "carmen:parityl",to: "carmen:ltohn",from: "carmen:lfromhn" } :
        {parity: "carmen:parityr",to: "carmen:rtohn",from: "carmen:rfromhn" };

    //If side x doesn't have data, check the other side
    if (!feat.properties[sideVar.parity][point.i] && !strict) return matchSide(feat, opp, point, true);
    if (!feat.properties[sideVar.parity][point.i] && strict) return null;

    var distRatio = point.pt.properties.travelled / point.lineDist;
    var num = parseInt(feat.properties[sideVar.from][point.i]) + Math.round((parseInt(feat.properties[sideVar.to][point.i]) - parseInt(feat.properties[sideVar.from][point.i])) * distRatio);

    if (num % 2 === 0 && feat.properties[sideVar.parity][point.i] === "O") ++num;
    else if (num % 2 === 1 && feat.properties[sideVar.parity][point.i] === "E") --num;
