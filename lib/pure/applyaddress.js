...
module.exports.reverse = function(feat,query) {
    ...
    var sideBinary = det2D(potential.startLine.geometry.coordinates, potential.endLine.geometry.coordinates, potential.pt.geometry.coordinates);
    var leftPt = turf.destination(
        ...
        'miles');
    var leftSideBinary =  det2D(potential.startLine.geometry.coordinates, potential.endLine.geometry.coordinates, leftPt.geometry.coordinates);

    console.log('side:', sideBinary,' left:',leftSideBinary);

    feat._geometry = potential.pt.geometry;
};

//calculate sign of determinant for vector in 2 space
function det2D(start, end, query) {
    return sign((end[0]-start[0])*(query[1]-start[1]) - (end[1]-start[1])*(query[0]-start[0]));
}
