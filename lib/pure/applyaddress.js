...
module.exports = function(feature, address) {
    ...
    for (var i = 0; i < lines.length; i++) {
        ...
        if (typeof lfromhn === 'number' &&
            ...
            if (Math.abs(neighbors.nlfrom - address) > Math.abs(lfromhn - address)) {
                neighbors.nlfrom = lfromhn; 
                neighbors.nlf_pt = lines[i][0];
            }
            if (Math.abs(neighbors.nlto - address) > Math.abs(ltohn - address)) {
                neighbors.nlto = ltohn;
                neighbors.nlt_pt = lines[i][1]; 
            }

            if (Math.abs(neighbors.nrfrom - address) > Math.abs(rfromhn - address)) {
                neighbors.nrfrom = rfromhn; 
                neighbors.nrf_pt = lines[i][0];
            }
            if (Math.abs(neighbors.nrto - address) > Math.abs(rtohn - address)) {
