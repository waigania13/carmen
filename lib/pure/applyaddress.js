...
module.exports = function(feature, address) {
    ...
    for (var i = 0; i < lines.length; i++) {
        ...
        var start, end, side, paritymask, coords;
        ...
        if (typeof lfromhn === 'number' &&
            ...
            side ='left';
            coords = lines[i].slice(0);
            return setPoint(address, start, end, coords);
            ...
            if (typeof lfromhn === 'number' && Math.abs(neighbors.nlfrom - address) > Math.abs(lfromhn - address)) {
                neighbors.nlfrom = lfromhn; 
                neighbors.nlf_pt = lines[i][0];
            }
            if (typeof ltohn === 'number' && Math.abs(neighbors.nlto - address) > Math.abs(ltohn - address)) {
                neighbors.nlto = ltohn;
                neighbors.nlt_pt = lines[i][1]; 
            }

            if (typeof rfromhn === 'number' && Math.abs(neighbors.nrfrom - address) > Math.abs(rfromhn - address)) {
                neighbors.nrfrom = rfromhn; 
                neighbors.nrf_pt = lines[i][0];
            }
            if (typeof rtohn === 'number' && Math.abs(neighbors.nrto - address) > Math.abs(rtohn - address)) {
    ...
    if (Math.abs(neighbors.nlfrom - neighbors.nlto) <= 4) {
        start = neighbors.nlfrom;
        end = neighbors.nlto;
        coords = [neighbors.nlf_pt, neighbors.nlt_pt];
        return setPoint(address, start, end, coords, true);
    }
    if (Math.abs(neighbors.nrfrom - neighbors.nrto) <= 4) {
        start = neighbors.nrfrom;
        end = neighbors.nrto;
        coords = [neighbors.nrf_pt, neighbors.nrt_pt]; 
        return setPoint(address, start, end, coords, true);
    }
};
