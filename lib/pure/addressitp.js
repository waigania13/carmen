const parseSemiNumber = require('../util/termops').parseSemiNumber;
...
const turf = {
    ...
    lineDistance: require('@turf/length'),
...
function reverse(feat, query) {
    ...
    for (var f_it = 0; f_it < feat.geometry.geometries.length; f_it++) {
        ...
        while (i--) {
            ...
            var current = getReversePoint(query, currentRange.lines, {
                units: 'miles'
            });
...
function getReversePoint(pt, coords, units) {
    ...
    for (i = 0; i < coords.length - 1; i++) {
        ...
        var perpendicularPt = turf.destination(pt, 1000, direction + 90, { units: 'miles' }); // 10000 = gross
