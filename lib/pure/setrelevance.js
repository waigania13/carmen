...
module.exports = function(query, sets, address) {
    ...
    for (var i = 0; i < sets.length; i++) {
        if (lastdb === sets[i].idx) continue;

        // If the address flag is set skip any matches for the initial query
        // term (ie. reason bitmask 1).
        if (address && sets[i].reason === 1) continue;
