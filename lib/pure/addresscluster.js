...
function forward(feature, address) {
    ...
    for (var c_it = 0; c_it < cluster.length; c_it++) {
        ...
        if (a_index > -1 && feature.geometry.type === 'GeometryCollection' && feature.geometry.geometries[c_it].type === 'MultiPoint') {
            return {
                type:'Point',
                coordinates: [
...
function reverse(feat,query) {
    ...
    if (feat.geometry.type !== 'GeoemtryCollection') return false;
    ...
    for (var c_it = 0; c_it < cluster.length; c_it++) {
        ...
        while (l--) {
            ...
            if (closest === undefined || closest.distance > dist) {
                closest = {cluster_it: c_it, cluster_pos: l, distance: dist, address: cluster[l]};
            }
        }
    }
    ...
    feat.geometry = {
        type: 'Point',
        coordinates: feat.geometry.coordinates[closest.c_it][closest.cluster_pos]
    }
