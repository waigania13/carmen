    ...
    to.startWriting(function(err) {
        ...
        q.awaitAll(function(err) {
            ...
            to.stopWriting(function(err) {
                ...
                var untarQ = queue();
                var rocksDirs = {};
                var fetch = function(type, cb) {
                    to.getGeocoderData(type, 0, function(err, data) {
                        var rocksdb = tmp.tmpNameSync();
                        rocksDirs[type] = rocksdb;
                ...
                untarQ.defer(fetch, "grid");
                untarQ.defer(fetch, "freq");
                untarQ.await(function() {
