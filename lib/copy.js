        ...
        q.awaitAll(function(err) {
            ...
            to.stopWriting(function(err) {
                ...
                var copyCache = function(type, cb) {
                    fromFilename = from._original.cacheSource ? from._original.cacheSource.filename : from._original.filename;
                    toFilename = from._original.cacheSource ? from._original.cacheSource.filename : from._original.filename;

                    fromRdb = fromFilename.replace('.mbtiles', '.' + type + '.rocksdb');
                    toRdb = toFilename.replace('.mbtiles', '.' + type + '.rocksdb');

                    rocksDirs[type] = toRdb;
                    fs.copy(fromRdb, toRdb, cp);
                };
