...
function tokenize(query, lonlat) {
    ...
    while (part = split.exec(normalized)) {
        const t = part[1].toString();
        if (t.length === 0) continue;
        if (removeEmoji(t).length === 0) continue;

        const charCode = t[0].charCodeAt();
        ...
        } else if (/(\d+)-(\d+)[a-z]?/.test(t)) {
            tokens.push(t);
        } else {
            Array.prototype.push.apply(tokens, t.split('-').filter((v) => v.length > 0));
        }
    }
    return tokens;
}
