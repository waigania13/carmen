 ...
 *
 * @param {Object} tokens - Tokenized array of user's input query
 * @return {String} If tokens are encodable, returns
 *                 encoded string. Otherwise returns ''.
 */
function encodableText(query) {
    if (typeof query === 'string') throw (new Error('Bad argument type "string" for encodableText'));
    const normalized = new Array(query.tokens.length);
    for (let i = 0; i < query.tokens.length; i++) {
        normalized[i] = normalizeText(query.tokens[i]);
    }
...
function encodePhrase(tokens, skipEncoding) {
    if (skipEncoding) throw (new Error('Unsupported usage of encodePhrase'));
    console.log('Using deprecated method encodePhrase. switch to encodableText');
    return encodableText(tokens);
}
...
function tokenize(query, lonlat) {
    if (lonlat) {
        ...
        if (numeric.length === 2) {
            ...
            if (!isNaN(numeric[0]) && !isNaN(numeric[1])) {
                return { tokens: numeric, separators: [',', ''] };
            }
        }
    }
    ...
    while (part = split.exec(normalized)) {
        ...
        if (tail) {
            if (tail.s === '-' || tail.s === '/') {
                ...
                // Just strip periods out for now. We'll need to revisit Once
                // we flip the order of complext replacement and tokenization
                t = `${tail.t}${t}`;
            } else {
                tokens.push(tail.t);
                separators.push(tail.s);
    ...
    return { tokens, separators, owner };
}
...
function getIndexableText(simpleReplacer, complexReplacer, globalReplacer, doc, defaultLanguages, categories) {
    ...
    for (const entry of texts) {
        ...
        let variants = token.enumerateTokenReplacements(complexReplacer, tokenize(text));

        // we should limit how many we produce to a not-insane number
        variants = variants.slice(0, 8); // TODO raise this

        for (const variant of variants) {
            const tokenized = tokenize(variant);
            const encoded = encodableText(tokenized).normalized;
...
* getIndexablePhrases
*
* @param {Object} tokenized text ...consider just passing token array
* @param {Object} freq
*/
...
function numTokenize(text, version) {
    ...
    if (typeof text === 'string') {
        console.warn('Invoked numTokenize w/ string.');
...
function numTokenizePrefix(text, version) {
    if (typeof text === 'string') text = tokenize(text).tokens;
    const numTokenized = [];
...
function numTokenV2(str) {
    console.warn('numTokenV2 is deprecated');
    let len = str.length;
    let numToken = '';
    while (len--) numToken += '#';
    return numToken;
