...
* @param {Array} tokens
* @param {Object} freq
* @param {Number} modifier Optional modifier for sensitivity of weighting within a phrase
*/
module.exports.getIndexablePhrases = getIndexablePhrases;
function getIndexablePhrases(tokens, freq, modifier) {
    const uniq = {};
    const phrases = [];
    const perms = permutations(tokens, getWeights(tokens, freq, modifier), true);
...
function getWeights(tokens, freq, multiplier) {
    // Allow an adjustable multiplier value to allow caller to emphasize
    // the sensitivity of weighting. A higher multiplier will cause more
    // extreme weight differences between terms within a single phrase,
    // making it easier to drop less significant terms.
    ...
    while (i--) {
        if (/#/.test(tokens[i])) {
            ...
            weights[i] = Math.log(1 + (totalfreq / Math.pow(10, multiplier)) / termfreq);
            weightsum += weights[i];
        }
    }
