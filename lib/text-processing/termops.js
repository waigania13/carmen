...
function getIndexableText(replacer, globalTokens, doc, markVariants) {
    ...
    function getTokens(texts, language) {
        ...
        for (let x = 0; x < texts.length; x++) {
            ...
            // TODO: should this be revisited after removing normalization cache?  <20-06-18, boblannon> //
    ...
    Object.keys(indexableText).forEach((phrase) => {
        ...
        if (phraseVariants[phrase]) obj.variants = Array.from(phraseVariants[phrase]).map((v) => { return v.split(' '); });
        output.push(obj);
    });
