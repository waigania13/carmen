...
function tokenize(query, lonlat) {
    ...
    for (let k = 0; k < normalized.length; k++) {
        ...
        if (charCode >= 0x4E00 && charCode <= 0x9FFF) {
            // Numbers in a word that starts with a \u4E00 - \u9FFF "CJK Unified
            // Ideographs" character are split into their own token here so they
            // are proteced from the naieve CJK character by character splitting.
    ...
    for (let i = 0; i < pretokens.length; i++) {
        ...
        if (pretokens[i].length && removeEmoji(pretokens[i]).length) {
            ...
            // smallest geographical entity without delimiters.
            if (charCode >= 0x4E00 && charCode <= 0x9FFF) {
