...
const WORD_SEPARATOR = [
    ...
    // The usual suspects from \u0020 - \u007F "Basic Latin" except -.
    // !"#$%&'()*+,
    '\u0021-\u002C',
    '\/',
    // :;<=>?@
...
function tokenize(query, lonlat) {
    ...
    const normalized = query
        ...
        .replace(/[\u2018\u2019\u02BC\u02BB\uFF07'\.\^]/g, '')
        // split based on other ascii and unicode punctuation except '-' which
        // we handle separately below.
        .split(new RegExp(`[${WORD_SEPARATOR}+]`, 'g'));

    const tokens = [];

    for (let k = 0; k < normalized.length; k++) {
        if (normalized[k].length === 0) continue;
        if (removeEmoji(normalized[k]).length === 0) continue;

        const charCode = normalized[k][0].charCodeAt();
        if (charCode >= 0x4E00 && charCode <= 0x9FFF) {
            ...
            const subtoken = normalized[k].split(/([0-9０-９]+)/);
            for (let l = 0; l < subtoken.length; l++) {
                const c = subtoken[l].charCodeAt();
                if (c >= 0x4E00 && c <= 0x9FFF) {
                    Array.prototype.push.apply(tokens, [...subtoken[l]]);
                } else if (subtoken[l].length > 0) {
                    tokens.push(subtoken[l]);
                }
            }
        } else if (/(\d+)-(\d+)[a-z]?/.test(normalized[k])) {
            tokens.push(normalized[k]);
        } else {
            Array.prototype.push.apply(tokens, normalized[k].split('-').filter((v) => v.length > 0));
        }
    }
    return tokens;
}
...
function getMinimalIndexableText(simpleReplacer, complexReplacer, globalReplacer, doc) {
    ...
    for (let x = 0; x < texts.length; x++) {
        texts[x] = token.replaceToken(combinedReplacer, texts[x]).query;

        // do simple token replacements without regexes -- just look them up in a dict
        let tokens = tokenize(texts[x]);
        if (simpleReplacer && simpleReplacer.tokens) {
            tokens = tokens.map((token) => simpleReplacer.tokens.get(token) || token);
        }
        if (!tokens.length) continue;
