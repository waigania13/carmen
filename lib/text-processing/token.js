    ...
    if (Object.keys(tokens).length) {
        return [{
            ...
            to: function() {
                ...
                return `${arguments[1]}${tokens[from]}${arguments[3]}`;
            }
        }];
    } else {
        return [];
...
module.exports.categorizeTokenReplacements = function(geocoder_tokens) {
    // make some regexes
    let nonWordBoundary = "[^" + WORD_BOUNDARY.substr(1);
    let innerWordBoundary = new RegExp(nonWordBoundary + '+' + WORD_BOUNDARY + '+' + nonWordBoundary + '+');
    let outerWordBoundary = new RegExp('(^' + WORD_BOUNDARY + '|' + WORD_BOUNDARY + '$)', 'g');
