 ...
 * @returns {Object} - replacement object containing token-replaced query and boolean indicating
...
module.exports.replaceToken = function(tokens, query) {
    ...
    return {
        query: abbr,
        lastWord: lastWord(query) !== lastWord(abbr) || false
    };

    function lastWord(query) {
        return query
            .trim()
            .split(new RegExp(WORD_BOUNDARY, 'gi'))
            .filter((e) => e)
            .slice(-1)[0];
    }
};
