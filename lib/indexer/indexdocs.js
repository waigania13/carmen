...
var centroid = require('@turf/point-on-feature');
...
///edqjehfjwefbiqwjebcjbqec ljehr
    const outlier = isOutlierDetected(doc.properties['carmen:addressnumber']);
        if (outlier) {
            var props = ['carmen:lfromhn','carmen:ltohn','carmen:rfromhn','carmen:rtohn'];
            props.forEach(function(i) {
                console.log(i);
                for (var x = 0; x < doc.properties[i].length; x++) {
                    console.log(doc.properties[i][x]);
                    
                };
                // doc.properties[i].forEach(function(num) {
                //     console.log(num);
                //     if (num != null) {
                //         console.log(num.fill(null));
                //         //doc.properties[i] = num.push(num.fill(null));
                //         //console.log(doc.properties[i]);
                //     }
                });
        }
...
function isOutlierDetected(addressnumber) {
    containsAddressNumbers = [];
    addresses = [];
    var quartile1, quartile3, upperLimit, lowerLimit;

    for (var i = 0; i < addressnumber.length; i++) {
        if (addressnumber[i] != null) {
            var sortedArray = addressnumber[i].map((num) => parseInt(num)).filter((num) => !isNaN(num)).sort((a,b) => a-b);
            ...
            var containsOutliers = sortedArray.filter((x) => { return (x < lowerLimit || x > upperLimit) });
