...
function indexdocs(docs, source, options, callback) {
    ...
    console.warn('LANG_DEBUG', source.name, source.lang.has_languages);
...
function loadDoc(freq, patch, doc, source, zoom, token_replacer, globalTokens) {
    ...
    var texts = termops.getIndexableText(token_replacer, globalTokens, doc, source.lang.has_languages);
    ...
    if (!(
        phrasesByLanguage.has('all') ||
        (phrasesByLanguage.has('default') && phrasesByLanguage.size === 1)
    )) {
...
function generateFrequency(docs, replacer, globalTokens, maxScore, hasLanguages) {
    var freq = {};
    ...
    for (var i = 0; i < docs.length; i++) {
        ...
        var texts = termops.getIndexableText(replacer, globalTokens, docs[i], hasLanguages);
        for (var x = 0; x < texts.length; x++) {
