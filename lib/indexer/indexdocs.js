...
function indexdocs(docs, freq, zoom, callback) {
    for (var i = 0; i < docs.length; i++) {
        ...
        if(!doc._zxy || doc._zxy.length === 0) {
            if(typeof doc.zoom != 'number') return callback(new Error('index has no zoom on _id:'+doc.id));
            if(doc.zoom < 0) return callback(new Error('zoom must be greater than 0 --- zoom was '+zoom+' on _id:'+doc.id));
            if(doc.zoom > 14) return callback(new Error('zoom must be less than 15 --- zoom was '+zoom+' on _id:'+doc.id));
        }
    }
