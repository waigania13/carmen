...
function parseDocs(docs, settings, full) {
    ...
    for (var i = 0; i < docs.length; i++) {
        ...
        if (typeof res === 'string') { return err; };
        else docs[i] = res;
...
function loadDoc(patch, doc, freq, zoom, token_replacer) {
    ...
    if (doc.properties['carmen:addressnumber']) {
        ...
        if (doc.properties['carmen:addressnumber'] !== doc.geometry.geometries.length) {
        ...
        for (var c_it = 0; c_it < doc.properties['carmen:addressnumber'].length; c_it++) {
            ...
            if (doc.properties['carmen:addressnumber'][c_it] !== doc.geometry.geometries[c_it].length) {
