...
const CARMEN_INDEXER_WEIGHTS_MODIFIER = process.env.CARMEN_INDEXER_WEIGHTS_MODIFIER;
...
function loadDoc(freq, patch, doc, source, zoom, token_replacer, globalTokens) {
    ...
    for (let x = 0; x < texts.length; x++) {
        const phrases = termops.getIndexablePhrases(texts[x].tokens, freq, CARMEN_INDEXER_WEIGHTS_MODIFIER);
