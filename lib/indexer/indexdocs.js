...
module.exports.outlierDetection = outlierDetection;
    ...
    for (var i = 0; i < docs.length; i++) {
        ...
        if (docs[i].properties['carmen:addressnumber']) {
            for (c_it = 0; c_it < docs[i].properties['carmen:addressnumber'].length; c_it++) {
                var outlier = outlierDetection(docs[i].properties['carmen:addressnumber']);
...
function outlierDetection(addressnumber) {
    addresses = [];
    var quartile1, quartile3, upperLimit, lowerLimit;
    console.error('outliers', addressnumber);
    for (var i = 0; i < addressnumber.length; i++) {
        ...
        console.log(Math.floor(lowerLimit), Math.floor(upperLimit));
    }
}
