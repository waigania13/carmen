...
function indexdocs(docs, freq, zoom, geocoder_tokens, callback) {
    ...
    workerPatch = formerWorker(docs, {freq:freq, zoom:zoom, geocoder_tokens:geocoder_tokens});
...
function formerWorker(data, settings) {
// code copied from indexdocs-workers
// process.on('message', function(data) {
    ...
    for (var i = 0; i < data.length; i++) {
        var err = docsworker.loadDoc(patch, data[i], freq, zoom, token_replacer);
    ...
    // process.send(patch);
    // very long array of numbers, features on the end

    return patch;
};

// // when a worker exits, log the ecode
// function exit(code) {
//     if (!code) return;
