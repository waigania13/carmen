...
function parseDocs(docs, settings, full) {
    ...
    for (let i = 0; i < docs.length; i++) {
        ...
        if (docs[i].properties['carmen:addressnumber']) {
            for (let c_it = 0; c_it < docs[i].properties['carmen:addressnumber'].length; c_it++) {
                if (!docs[i].properties['carmen:addressnumber'][c_it]) continue;

                for (let addr_it = 0; addr_it < docs[i].properties['carmen:addressnumber'][c_it].length; addr_it++) {
                    const feat = point(docs[i].geometry.geometries[c_it].coordinates[addr_it], feature.storableProperties(docs[i].properties, 'vector'));
                    feat.id = docs[i].id;
        ...
        if (docs[i].properties['carmen:rangetype']) {
            for (let c_it = 0; c_it < docs[i].geometry.geometries.length; c_it++) {
                if (docs[i].geometry.geometries[c_it].type !== 'MultiLineString') continue;

                for (let addr_it = 0; addr_it < docs[i].geometry.geometries[c_it].coordinates.length; addr_it++) {
                    const feat = linestring(docs[i].geometry.geometries[c_it].coordinates[addr_it], feature.storableProperties(docs[i].properties, 'vector'));
                    feat.id = docs[i].id;
                    full.vectors.push(feat);
                }
            }
...
function loadDoc(freq, patch, doc, source, zoom, token_replacer, globalTokens) {
    ...
    let phrase, phraseObj, languages, y = new Map();
