...
function standardize(doc, zoom) {
    ...
    if (doc.properties['carmen:rangetype'] && doc.geometry.type === 'GeometryCollection') {
        if (isOutlierDetected(doc)) {
 ...
 */
function isOutlierDetected(doc) {
    const props = {
        left: {
            parity: 'carmen:parityl',
            from: 'carmen:lfromhn',
            to: 'carmen:ltohn'
        },
        right: {
            parity: 'carmen:parityr',
            from: 'carmen:rfromhn',
            to: 'carmen:rtohn'
        }
    ...
    // Determine the index and length of the interpolation data in the combined feature.
    let idx, len;
    for (let i = 0; i < doc.properties['carmen:parityl'].length; i++) {
        if (doc.properties['carmen:parityl'][i] && doc.properties['carmen:parityl'][i].length > 0) {
            len = doc.properties['carmen:parityl'][i].length;
            idx = i;
            break;
        }
    }

    // Look at interpolation ranges on each side, and detect outliers
    for (const side of ['left', 'right']) {
        const ranges = new Array(len);
        for (let i = 0; i < len; i++) {
            if (doc.properties[props[side].to][idx][i] === null) continue;
            ranges[i] = Math.abs(doc.properties[props[side].to][idx][i] - doc.properties[props[side].from][idx][i]);
        }

        // Make a shallow copy of the ranges to sort and use for quartile calculations.
        const values = ranges.slice(0).filter((v) => v);
        values.sort((a, b) => a - b);

        // Calculates the two quartiles needed for the formula: {[Q1-k(Q3-Q1),Q3+k(Q3-Q1)]} using percentiles
        const quartile1 = Math.floor(percentile(values, 0.25));
        const quartile3 = Math.floor(percentile(values, 0.75));

        // The value of k in the equation is usually 1.5, can modify if this seems too aggressive
        const lowerLimit = quartile1 - (1.5 * (quartile3 - quartile1));
        const upperLimit = quartile3 + (1.5 * (quartile3 - quartile1));

        for (let i = 0; i < values.length; i++) {
            if (values[i] < lowerLimit || values[i] > upperLimit || values[i] > 1e5) {
                return true;
            }
        }
    }
