...
function parseDocs(docs, settings, full) {
    ...
    for (var i = 0; i < docs.length; i++) {
        var res = loadDoc(full, docs[i], freq, zoom, token_replacer);
        ...
        if (docs[i].properties['carmen:rangetype']) {
            for (var c_it = 0; c_it < docs[i].properties['carmen:addressnumber'].length; c_it++) {
                for (var addr_it = 0; addr_it < docs[i].geometry.coordinates[c_it].length; addr_it++) {
                    var feat = JSON.parse(JSON.stringify(docs[i]));
                    if (feat.properties['carmen:parityl']) feat.properties['carmen:parityl'][c_it] = [feat.properties['carmen:parityl'][addr_it]]
                    if (feat.properties['carmen:parityr']) feat.properties['carmen:parityr'][c_it] = [feat.properties['carmen:parityr'][addr_it]]
                    if (feat.properties['carmen:lfromhn']) feat.properties['carmen:lfromhn'][c_it] = [feat.properties['carmen:lfromhn'][addr_it]]
                    if (feat.properties['carmen:rfromhn']) feat.properties['carmen:rfromhn'][c_it] = [feat.properties['carmen:rfromhn'][addr_it]]
                    if (feat.properties['carmen:ltohn']) feat.properties['carmen:ltohn'][c_it] = [feat.properties['carmen:ltohn'][addr_it]]
                    if (feat.properties['carmen:rtohn']) feat.properties['carmen:rtohn'][c_it] = [feat.properties['carmen:rtohn'][addr_it]]
