...
function indexdocs(docs, source, options, callback) {
    ...
    if (TIMER) console.time('update:parse');
    try {
        parseDocs(docs, settings, full);
    } catch (err) {
        return callback(err);
    }
    if (TIMER) console.timeEnd('update:parse');
...
function loadDoc(freq, patch, doc, source, zoom, token_replacer, globalTokens) {
    ...
    for (var x = 0; x < texts.length; x++) {
        ...
        if (texts[x].variants) {
            for (y = 0; y < texts[x].variants.length; y++) {
                variants.set(texts[x].variants[y].join(' '), texts[x].tokens.join(' '));
            }
        }
    }
    ...
    for (var phraseItem of allPhrases) {
        ...
        var langList = languages.size === 1 ? Array.from(languages)[0] : Array.from(languages).sort().join(",");
        ...
        var gridPatchByLang = gridPatch.get(langList);
        ...
        while (l--) {
            ...
            if (encoded) gridPatchByLang.push(encoded);
        }
