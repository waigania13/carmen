...
function indexdocs(docs, source, options, callback) {
    ...
    let freq;
    try {
        freq = generateFrequency(docs, source.simple_replacer.concat(source.complex_query_replacer), options.tokens, parseInt(source.maxscore), source.lang.has_languages);
    } catch (err) {
        return callback(err);
    }
