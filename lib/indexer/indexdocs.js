...
function indexdocs(docs, source, options, callback) {
    ...
    var freq;
    try {
        freq = generateFrequency(docs, source.token_replacer, options.tokens);
    } catch (err) {
        return callback(err);
    }
            ...
            while (l--) {
                ...
                try {
                    encoded = grid.encode({
                        ...
                        score: doc.properties["carmen:score"] === 0 ? 0 : Math.ceil(7*(Math.log(doc.properties["carmen:score"]) || 0)/(Math.log(maxScore)||1))
                    });
                } catch (err) {
...
function generateFrequency(docs, replacer, globalTokens) {
    var freq = {};
