 ...
 * This is a full outer join of the features, using their shard value as the key.
 *
 * @see {@link merge} for detailed explanation.
 *
        ...
        nextq.defer((cb) => {
            fetchq.awaitAll((err) => {
                ...
                if (nexts[0].done && nexts[1].done) {
                    ...
                    advance(0);
                    advance(1);
                    // console.log({ shard: out1.value.shard, data1: out1.value.data, data2: out2.value.data });
 ...
 * necessary. The join is a {@link https://www.w3schools.com/sql/sql_join_full.asp full outer join},
 ...
 *
 * see {@link https://github.com/mapbox/carmen-cache/blob/master/API.md#rocksdbcache RocksDBCache#merge}.
 *
 * #### DAWG merge
 *
...
/**
 * Used by merge-worker to build the configuration needed to initialize the output Carmen instance
 *
 * @param {string} filename - path where the merge result will be written
 * @param {object} options - options
 * @param {function} callback - a callback function
 * @returns {object} a configuration for instantiating a geocoder
 */
function getOutputConf(filename, options, callback) {
    ...
    const tmp = Carmen.auto(filename, () => {
        ...
        /**
         * Write metadata to output source
         *
...
function multimerge(fromFiles, toFile, options, callback) {
    ...
    const q = queue(4);
    /**
     * Use merge-worker to distribute merge operations.
     *
     */
    const queueJobs = function() {
        while (toMerge.length >= 2) {
            ...
            /**
             * Fork the current process and perform merge operation on two of the sources
             *
             * @param {string} from1File - path to an mbtiles file
             * @param {string} from2File - path to an mbtiles file
             * @param {string} mergeToFile - path to the output. If this is not the final merge, this is a randomly generated location in `tmpDir`.
             * @param {boolean} isFinal - true iff this is the final merge
             */
