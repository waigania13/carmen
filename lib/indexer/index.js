        ...
        function setText(textArray, variants) {
            ...
            for (let i = 0; i < textArray.length; i++) {
                if ((textArray[i] !== null) && (textArray[i].trim().length > 0)) {
                    //true if
                    // TODO: insert into the fuzzyphrasesetbuilder https://github.com/mapbox/node-fuzzy-phrase/blob/master/test/all.js#L19
            ...
            if (variants) {
                for (const v of Object.keys(variants)) {
                    if (v !== null && variants[v] !== null && v.length && variants[v].length) {
                        //maps to the normalised version and calls setText with false
                        // TODO: remove(?)
