...
const feature = require('../util/feature'),
    ...
    fuzzy = require('node-fuzzy-phrase'),
    ...
    function updateCache(err, patch) {
        ...
        for (let i = 0; i < patch.text.length; i++) {
            if ((patch.text[i] !== null) && (patch.text[i].trim().length > 0)) {
                dictcache.insert(patch.text[i].split(' '));
            }
        }
        q.awaitAll(callback);
...
function store(source, callback) {
    ...
    const types = ['freq', 'grid'];
    q.defer((callback) => {
        source._dictcache.finish();
        const fuzzySetFile = source.getBaseFilename() + '.fuzzy';
        source._dictcache = new fuzzy.FuzzyPhraseSet(fuzzySetFile);
        callback();
    });
