...
function runChecks(doc, zoom) {
    if (!doc.id) {
        ...
        return 'doc has no carmen:text on id:' + doc._id;
...
function loadDoc(patch, doc, freq, zoom, token_replacer) {
    ...
    if(!doc.properties["carmen:center"] || !verifyCenter(doc.properties["carmen:center"], tiles)) {
        ...
        if(!verifyCenter(doc.properties["carmen:center"], tiles)) {
            return 'Invalid doc.properties.carmen:center provided, and unable to calculate corrected centroid. Verify validity of doc._geometry for doc id:' + doc.id;
        } else {
            console.warn('new: doc._center: ', doc.properties["carmen:center"]);
