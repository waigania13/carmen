...
function runChecks(doc, zoom) {
    ...
    var objectProps = Object.keys(doc.properties);
    for (var propertyIndex = 0; propertyIndex < objectProps.length; propertyIndex++) {
        var prop = objectProps[propertyIndex];
        if ((doc.properties[prop] !== null) && /^carmen:text_/.test(prop) && (doc.properties[prop].trim().length === 0))
            delete doc.properties[prop];
    }
