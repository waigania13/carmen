        ...
        for (var i = 0; i < data.length; i++) {
            ...
            if (data[i].properties['carmen:addressnumber']) {
                for (var addr_it = 0; addr_it < data[i].properties['carmen:addressnumber'].length; addr_it++) {
                    ...
                    feat = point(feat.geometry.coordinates[i], feat.properties);
                ...
                for (var addr_it = 0; addr_it < data[i].geometry.coordinates.length; addr_it++) {
                    ...
                    if (feat.properties['carmen:rfromhn']) feat.properties['carmen:rfromhn'] = feat.properties['carmen:lfromhn'][addr_it]
                    if (feat.properties['carmen:ltohn']) feat.properties['carmen:ltohn'] = feat.properties['carmen:lfromhn'][addr_it]
                    if (feat.properties['carmen:rtohn']) feat.properties['carmen:rtohn'] = feat.properties['carmen:lfromhn'][addr_it]
                    feat.properties.id = feat.id;
                    feat = linestring(feat.geometry.coordinates[i], feat.properties);
                    feat.id = feat.properties.id;
                    patch.vectors.push(feat);
                }
            } else {
