    ...
    var queueJobs = function() {
        ...
        while (toMerge.length >= 2) {
            ...
            var enqueueTask = function(from1File, from2File, mergeToFile, cleanup, isFinal) {
                console.log('%%% fromFile:', from1File);
                q.defer(function(from1File, from2File, mergeToFile, cb) {
                    ...
                    worker.on('exit', function exit(code) {
                        if (code == 0) {
                            ...
                            if (cleanup) {
                                console.log('%%% clean up merged mbtiles');
                                // clean up fromfiles
                                // hard to keep DRY w/ isFinal because option won't always be used
                            }

                            // where we usually clean up
                            if (isFinal) {
                                // we're done
