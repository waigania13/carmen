...
/**
 * Merges files by enqueueing tasks for merge-worker,
 * then cleans up the intermediate files once merged.
 *
 * @param {array} of file paths of files to be merged
 * @param {string} filepath of final file
 * @param {Object[]} config for final file
 * @param {boolean} delete intermediate files as they're merged
                ...
                q.defer(function(from1File, from2File, mergeToFile, cb) {
                    ...
                    worker.on('exit', function exit(code) {
                        if (code == 0) {
                            ...
                            if (cleanup) {
                                // make delete file function to do this, and combine with isFinal?
                                fs.unlink(tmpDir + '/' + from1File, cb);
                                fs.unlink(tmpDir + '/' + from2File, cb);
                            }
