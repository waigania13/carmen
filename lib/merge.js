...
var pairwiseGeocoderIterator = function(from1, from2, type) {
    ...
    var advance = function(num) {
        fetchq.defer(function(cb) { iterators[num].asyncNext(function(row) {
            nexts[num] = row;
            cb();
        })});
    }
            ...
            fetchq.awaitAll(function() {
                ...
                if (nexts[0].done && nexts[1].done) {
                    ...
                    if (!nexts[0].done && (nexts[1].done || nexts[0].value.shard < nexts[1].value.shard)) {
                        ...
                        var out = nexts[0];
                        advance(0);
                        callback({value: {shard: out.value.shard, data1: out.value.data, data2: undefined}, done: false});
                    } else if (!nexts[1].done && (nexts[0].done || nexts[1].value.shard < nexts[0].value.shard)) {
                        // return and advance next[0]
                        var out = nexts[1];
                        advance(1);
                        callback({value: {shard: out.value.shard, data2: out.value.data, data1: undefined}, done: false});
                    } else if (nexts[0].value.shard == nexts[1].value.shard) {
                        var out1 = nexts[0], out2 = nexts[1];
                        advance(0);
                        advance(1);
                        callback({value: {shard: out1.value.shard, data1: out1.value.data, data2: out2.value.data}, done: false});
                    } else {
...
var mergeType = function(from1, from2, to, type, mergeOp, completeCallback) {
    ...
    var next = function() {
        nextq.defer(function(cb) {
            iterator.asyncNext(function(row) {
                if (row.done) {
...
function merge(geocoder, from1, from2, to, options, callback) {
    ...
    q.defer(function(cb) {
        mergeType(from1, from2, to, "grid", function(shard, data1, data2) {
            ...
            [from2, to].forEach(function(source) { source._geocoder.unload("grid", shard); });
