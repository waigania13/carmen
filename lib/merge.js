        ...
        nextq.defer(function(cb) {
            iterator.asyncNext(function(err, row) {
                ...
                if (row.done) {
                    ...
                    if (row.value.data1 !== undefined && row.value.data2 !== undefined) {
                        mergeOp(row.value.shard, row.value.data1, row.value.data2, write);
                    } else {
                        write(null, row.value.data1 || row.value.data2);
                    }
...
function merge(geocoder, from1, from2, to, options, callback) {
    ...
    q.defer(function(cb) {
        ...
        mergeType(from1, from2, to, "grid", function(shard, data1, data2, callback) {
            callback(null, from1._geocoder.merge(data1, data2));
        }, function() {
