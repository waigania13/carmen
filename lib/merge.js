...
function merge(geocoder, from1, from2, to, options, callback) {
    ...
    q.defer(function(cb) {
        ...
        if (use_normalization_cache) {
            console.log('[carmen/lib/merge.js 217] entering use_normalization_cache block');
            // preload normalization maps
                    ...
                    normCache.writeBatch(Array.from(
                        iterTools.map(function(normalization) {
                            ...
                            if (normInBoth.has(mergedKey)) {
                                ...
                                // Array.from() is not used here because es6-native-set does not support this
                                mergedVals = [];
                                combined.forEach((item) => { mergedVals.push(item); });
                                mergedVals.sort();
                            } else {
