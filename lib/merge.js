...
function merge(geocoder, from1, from2, to, options, callback) {
    ...
    q.defer(function(cb) {
        var start = +(new Date());
        if (TIMER) console.time('merge freq');
        mergeType(from1, from2, to, "freq", function(shard, data1, data2) {
            ...
            if (TIMER) console.timeEnd('merge freq');
    ...
    q.defer(function(cb) {
        var start = +(new Date());
        if (TIMER) console.time('merge grid');
        mergeType(from1, from2, to, "grid", function(shard, data1, data2) {
            ...
            if (TIMER) console.timeEnd('merge grid');
    ...
    q.defer(function(cb) {
        var start = +(new Date());
        if (TIMER) console.time('merge feature');
        mergeType(from1, from2, to, "feature", function(shard, data1, data2) {
            ...
            if (TIMER) console.timeEnd('merge feature');
    ...
    // merge dawg
    q.defer(function(cb) {
        if (TIMER) console.time('merge dawg');
        var start = +(new Date());
        ...
        to.putGeocoderData("stat", 0, mergedDawg.toCompactDawgBuffer(), function() {
            if (TIMER) console.timeEnd('merge dawg');
