...
function multimerge(fromFiles, toFile, options, cleanup, callback) {
    // if cleanup, delete mbtiles as merged (but not before; queued)
    console.log('%%% cleanup:', cleanup);
    // make list of files to be merged
    var toMerge = [], inProgress = 0;
    fromFiles.forEach(function(fromFile) { toMerge.push(fromFile); });

    // make a directory to stash files (this stays at 4.0K the whole process)
    ...
    var queueJobs = function() {
        ...
        while (toMerge.length >= 2) {
            ...
            inProgress += 2;
            // pass in cleanup
            var enqueueTask = function(from1File, from2File, mergeToFile, cleanup, isFinal) {
                q.defer(function(from1File, from2File, mergeToFile, cb) {
                    ...
                    worker.on('exit', function exit(code) {
                        if (code == 0) {
                            ...
                            if (cleanup) {
                                console.log('%%% cleaning up', from1File, 'and', from2File);
                                var mergedFiles = [from1File, from2File];
                                for (i = 0; i < mergedFiles.length; i++) {
                                    
                                }
                                // make delete file function to do this
                                fs.unlink(tmpDir + '/' + from1File, function(err) {console.log('file1 deleted')});
                                fs.unlink(tmpDir + '/' + from2File, function(err) {console.log('file2 deleted')});
                            }
                            ...
                            if (isFinal) {
                                ...
                                fs.readdir(tmpDir, function(err, files) {
                                    ...
                                    for (var i = 0; i < files.length; i++) deleteQ.defer(function(file, cb) {
                                        // delete name in filesystem, delete file if not being used.
                                ...
                                // add new file to end of files to be merged list
            ...
            if (inProgress == 2 && toMerge.length == 0) {
                ...
                enqueueTask(from1File, from2File, toFile, cleanup, true);
            } else {
                ...
                (function(from1File, from2File) {
                    ...
                    tmpCounter += 1;
                    enqueueTask(from1File, from2File, filename, cleanup, false);
                })(from1File, from2File);
            }
