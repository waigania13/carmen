...
function merge(geocoder, from1, from2, to, options, callback) {
    ...
    q.defer(function(cb) {
        ...
        var use_normalization_cache = from1.use_normalization_cache || from1._info.use_normalization_cache;
        ...
        if (use_normalization_cache) {
            ...
            norm1Set = new Set([...norm1.keys(), ...norm1.values()]);
            norm2Set = new Set([...norm2.keys(), ...norm2.values()]);
        }
        ...
        while (!remainingNext.done) {
            ...
            if (remainingMembershipSet.has(next1.value[0])) remainingMergeMap.set(remainingNext.value[0], mergedCount);
            mergedCount += 1;
        ...
        fs.writeFile(dawgFile, mergedData, function(err) {
            ...
            if (use_normalization_cache) {
                ...
                normCache.writeBatch(Array.from(iterTools.map(function(normalization) { return [norm1ToMerged.get(normalization[0]), norm1ToMerged.get(normalization[1])] }, norm1)));
                normCache.writeBatch(Array.from(iterTools.map(function(normalization) { return [norm2ToMerged.get(normalization[0]), norm2ToMerged.get(normalization[1])] }, norm2)));
