...
function merge(geocoder, from1, from2, to, options, callback) {
    ...
    ['grid', 'freq'].forEach(function(type) {
        q.defer(function(cb) {
            ...
            var rocks = {
                from1: (from1.getBaseFilename ? from1.getBaseFilename() : '/tmp/rocks.' + _randomChars(9)) + '.' + type + '.rocksdb',
                from2: (from2.getBaseFilename ? from2.getBaseFilename() : '/tmp/rocks.' + _randomChars(9)) + '.' + type + '.rocksdb',
                to: (to.getBaseFilename ? to.getBaseFilename() : '/tmp/rocks.' + _randomChars(9)) + '.' + type + '.rocksdb'
            }
