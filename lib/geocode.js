...
function toFeatures(geocoder, contexts, options) {
    ...
    for (var i = 0; i < contexts.length; i++) {
        ...
        if (options.shard) {
            ...
            // strip out geometry of features to reduce child message size?
            for (var j=0; j < contexts[i].features.length; j++) {
                if (contexts[i].features[j].geometry) {
                    contexts[i].features[j].geometry = null;
                }
            }
