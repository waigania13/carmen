...
function reverseGeocode(geocoder, tokenized, options, callback) {
    ...
    context(geocoder, queryData.query[0], queryData.query[1], { full:true }, function(err, context) {
        if (err) return callback(err);
        ...
        try {
            while (context.length) {
                ...
                if (options.types) {
                    var type = geocoder.byidx[context[0].properties['carmen:dbidx']]._geocoder.name;
                ...
                var contextIndexes = [];
                for (var context_i = 0; context_i < context.length; context_i++)
                    contextIndexes.push(geocoder.byidx[context[context_i].properties['carmen:dbidx']]._geocoder.id)
                queryData.indexes.push(contextIndexes);
...
function forwardGeocode(geocoder, query, options, callback) {
    ...
    function spatialmatchComplete(err, matched) {
        ...
        verifymatch(queryData.query, stats, geocoder, matched, options, function(err, contexts) {
            ...
            try {
                for (var i = 0; i < contexts.length; i++) {
                    ...
                    for (var context_i = 0; context_i < contexts[i].length; context_i++)
                        contextIndexes.push(geocoder.byidx[contexts[i][context_i].properties['carmen:dbidx']]._geocoder.id)
                    queryData.indexes.push(contextIndexes);
                }
            } catch (err) {
                return callback(err);
            ...
            queryData.indexes = queryData.indexes.slice(0, options.limit);
