...
module.exports = function(geocoder, query, options, callback) {
    ...
    if (options.shard) {
        // shard geocode
        return forwardShardGeocode(geocoder, query, options, callback);
    } else {
        // Forward geocode.
        ...
        verifymatch(queryData.query, stats, geocoder, matched, options, function(err, contexts) {
            ...
            try {
                for (var i = 0; i < contexts.length; i++) {
                    ...
                    // Process context array into tidy Carmen geojson feature
...
function forwardShardGeocode(geocoder, query, options, callback) {
    console.log('shard geocoding!');
    options.limit = options.limit ? (options.limit > 10 ? 10 : options.limit) : 5;
    query = token.replaceToken(geocoder.replacer, query);
    var queryData = {
        type: 'FeatureCollection',
        ...
        verifymatch(queryData.query, stats, geocoder, matched, options, function(err, contexts) {
            ...
            try {
                for (var i = 0; i < contexts.length; i++) {
                    ...
                    // add formats to each feature as a property
                    contexts[i][0].properties['carmen:formats'] = formats;
