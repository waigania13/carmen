        ...
        try {
            while (context.length) {
                ...
                for (var context_i = 0; context_i < context.length; context_i++)
                    contextIndexes.push(geocoder.byidx[context[context_i].properties['carmen:dbidx']].data.name.id)
            ...
            try {
                for (var i = 0; i < contexts.length; i++) {
                    ...
                    for (var context_i = 0; context_i < contexts[i].length; context_i++)
                        contextIndexes.push(geocoder.byidx[contexts[i][context_i].properties['carmen:dbidx']].data.name.id)
                    queryData.indexes.push(contextIndexes);
                }
            } catch (err) {
                return callback(err);
