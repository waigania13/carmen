    ...
    context(geocoder, queryData.query[0], queryData.query[1], null, true, function(err, context) {
        ...
        try {
            while (context.length) {
                queryData.features.push(ops.toFeature(context, geocoder.indexes[context[0]._extid.split('.')[0]]._geocoder.geocoder_address));
                context.shift();
            }
        } catch(err) {
            return callback(err);
...
function forwardGeocode(geocoder, query, options, callback) {
    ...
    q.awaitAll(function(err, results) {
            ...
            if (options.debug) {
                for (var j = 0; j < results[i].grids.length; j++) {
                    if (ops.grid(results[i].grids[j]).id === options.debug.id) {
                        ...
                        options.debug.phrasematch.count++;
                    }
                }
            }
        ...
        if (options.debug) options.debug.phrasematch.relev = new Relev(features[options.debug.id]).relev;
    ...
    function spatialmatchComplete(err, matched) {
        if (err) return callback(err);
        if (options.debug) {
            if (matched.sets[options.debug.id])
                options.debug.spatialmatch = { count: 1, relev: options.debug.phrasematch.relev }
            else
                options.debug.spatialmatch = { count: 0, relev: 0};
        }
            ...
            if (options.debug) {
                ...
                for (var x = 0; x < contexts.length; x++) {
                    for (var y = 0; y < contexts[x].length; y++) {
                        if (contexts[x][y]._id === options.debug.id) {
