...
function reverseGeocode(geocoder, tokenized, options, callback) {
    ...
    for (var type in geocoder.bytype) {
        if (options.types && options.parentTypes.indexOf(type) === -1) continue;
    ...
    if (options.limit > 1) {
        context.nearest(geocoder, queryData.query[0], queryData.query[1], options.types[0].split('.')[0], options.limit, function(err, feats) {
            if (err) return callback(err);
        ...
        try {
            while (context.length) {
                ...
                if (options.parentTypes) {
                    ...
                    console.log(type, options.parentTypes);
