    ...
    function stackContext(err, contexts) {
        ...
        for (var contexts_it = 0; contexts_it < contexts.length; contexts_it++) {
            ...
            if (context.length) {
                ...
                queryData.features.push(ops.toFeature(context, index.geocoder_format, options.language, options.languageMode, options.debug));

                // record index names
                if (options.indexes) contextIndexes[context[0].properties['carmen:index']] = true;
            }
        }
    ...
    function splitContext(err, context) {
        ...
        try {
            while (context.length) {
                ...
                var index = geocoder.indexes[context[0].properties['carmen:index']];
                if (!filter.featureAllowed(index, context[0], options)) {
                    context.shift();
                    continue;
                }
                queryData.features.push(ops.toFeature(context, index.geocoder_format, options.language, options.languageMode, options.debug));
        ...
        verifymatch(queryData.query, stats, geocoder, matched, options, function(err, contexts) {
            ...
            try {
                for (var i = 0; i < contexts.length; i++) {
                    var index = geocoder.indexes[contexts[i][0].properties['carmen:index']];
                    if (!filter.featureAllowed(index, contexts[i][0], options)) continue;
                    queryData.features.push(ops.toFeature(contexts[i], index.geocoder_format, options.language, options.languageMode, options.debug));
                }
            } catch (err) {
                return callback(err);
            }
            ...
            if (options.indexes) {
                ...
                for (var k = 0; k < queryData.features.length; k++) {
                    for (var context_k = 0; context_k < contexts[k].length; context_k++)
                        contextIndexes[contexts[k][context_k].properties['carmen:index']] = true;
                }
