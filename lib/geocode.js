...
module.exports = function(geocoder, query, options, callback) {
    ...
    if (options.types) {
        ...
        while (l--) {
            if (!geocoder.bytype[options.types[l]] && !geocoder.bysubtype[options.types[l]]) {
                return callback(errcode('Type "' + options.types[l] + '" is not a known type. Must be one of: ' + Object.keys(geocoder.bytype).join(', ') + ' or ' + Object.keys(geocoder.bysubtype).join(', '), 'EINVALID'));
            } else {
    ...
    if (options.limit > 1) {
        context.nearest(geocoder, queryData.query[0], queryData.query[1], options.types[0], options.limit, function(err, feats) {
            ...
            q.awaitAll(function(err, contexts) {
                ...
                // it's possible for context() to return duplicate contexts
                // from options.limit > 1 reverse queries. dedupe using tmpid.
                var observedIds = [];
                var dedupedContext = [];
                for (var context_i = 0; context_i < contexts.length; context_i++) {
                    ...
                    if (feat.properties && feat.properties['carmen:tmpid']) {
                        if (observedIds.indexOf(feat.properties['carmen:tmpid']) !== -1)
                            continue;
                        observedIds.push(feat.properties['carmen:tmpid']);
                    }
                    dedupedContext.push(contexts[context_i]);
                }

                stackContext(null, dedupedContext);
            });
        });
    } else {
