...
module.exports = function(geocoder, query, callback) {
    ...
    if (queryData.query.length === 2 && _(queryData.query).all(_.isNumber)) {
        return geocoder.context(queryData.query[0], queryData.query[1], null, function(err, context) {
    ...
    function searchComplete(err, feats, grids, zooms) {
        if (err) return callback(err);
        sortByRelevance(indexes, types, queryData, geocoder, feats, grids, zooms, function(err, contexts, sets) {
            if (err) return callback(err);
            ...
            for (var j = 0, cl = contexts.length; j < cl; j++) {
                ...
                for (var i = 0, cjl = contexts[j].length; i < cjl; i++) {
                    var a = contexts[j];
                    if (sets[a[i].id]) subsets.push(sets[a[i].id]);
                }
