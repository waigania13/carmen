...
function forwardGeocode(geocoder, query, options, callback) {
    ...
    q.awaitAll(function(err, results) {
        ...
        if (options.proximity) {
            for (layers_it = 0; layers_it < grids.length; layers_it++ ) {
                var proximityXY = sm.px(options.proximity, zooms[layers_it]).map(function(px) {
                    return Math.floor(px / 256);
                });
                grids[layers_it].sort(function (a, b) {
                    var proxDist = [a, b].map(function (ele) { return Math.sqrt(Math.pow(ops.grid(ele).x - proximityXY[0],2) + Math.pow(ops.grid(ele).y - proximityXY[1],2)); });
                    if (proxDist[0] < proxDist[1]) return -1;
                    else if (proxDist[1] < proxDist[0]) return 1;
                    else return 0;
                });
            }
        }
