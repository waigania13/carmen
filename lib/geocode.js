...
function reverseGeocode(geocoder, tokenized, options, callback) {
    if (options.limit && options.types && options.types.length === 1) {
        ...
        return callback('limit must be combined with a single type paramater when reverse geocoding');
    }
    ...
    if (options.limit > 1) {
        context(geocoder, queryData.query[0], queryData.query[1], {
            ...
            for (var context_it = 0; context_it < contexts.length; context_it++) {
                var coords = JSON.parse(contexts[context_it]['carmen:center']);
        ...
        context(geocoder, queryData.query[0], queryData.query[1], {
            ...
            if (err) return callback(err);
            afterContext(null, context);
        });
    }

    function afterContext(err, context) {
