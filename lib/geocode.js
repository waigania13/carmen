...
module.exports = function(geocoder, query, options, callback) {
    ...
    function loadType(dbid, callback) {
        phrasematch(geocoder.indexes[dbid], queryData.query.join(' '), searchLoaded);
    ...
    function searchComplete(err, feats, grids, zooms) {
        if (err) return callback(err);
        spatialmatch(queryData.query, stats, geocoder, feats, grids, zooms, options, function(err, contexts) {
            if (err) return callback(err);
