    ...
    function stackContext(err, contexts) {
        ...
        for (var contexts_it = 0; contexts_it < contexts.length; contexts_it++) {
            ...
            if (context.length) {
                ...
                // record index names
                if (options.indexes) contextIndexes[geocoder.indexes[context[0].properties['carmen:index']].id] = true;
            }
        }
            ...
            if (options.indexes) {
                ...
                for (var k = 0; k < queryData.features.length; k++) {
                    for (var context_k = 0; context_k < contexts[k].length; context_k++)
                        contextIndexes[geocoder.indexes[contexts[k][context_k].properties['carmen:index']].id] = true;
                }
