...
module.exports = function(carmen, query, callback) {
    ...
    function loadType(dbname, pos) {
        carmen.search(indexes[dbname], queryData.query.join(' '), null, searchLoaded);
    ...
    function searchComplete(err, feats, grids, zooms) {
        if (err) return callback(err);
        sortByRelevance(indexes, types, queryData, carmen, feats, grids, zooms, function(err, contexts, relevd) {
            if (err) return callback(err);
            ...
            for (var j = 0; j < contexts.length; j++) {
                ac = [];
                for (var i = 0; i < contexts[j].length; i++) {
                    var a = contexts[j];
                    if (relevd[a[i].id]) {
                ...
                contexts[j]._relevance = getSetRelevance(queryData.query, ac);
            }

            contexts.sort(sortRelev);

            for (j = 0; j < contexts.length; j++) {
                delete contexts[j]._relevance;
            }
