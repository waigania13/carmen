...
function reverseGeocode(geocoder, tokenized, options, callback) {
    if (options.limit && options.types && options.types.length === 1) {
        ...
        return callback(errcode('limit must be combined with a single type paramater when reverse geocoding', 'EINVALID'));
    }
    ...
    //If multiple reulst are being returned, do not split context array
