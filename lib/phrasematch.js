    ...
    loadall(getter, 'freq', [1], function(err) {
        ...
        while (l--) {
            ...
            if (autocomplete && subqueries[l].ender) {
                ...
                subqueries[l].phrasesText = source._dictcache.getPhrasesFromDegen(subqueries[l].text);
                var m = subqueries[l].phrasesText.length;
                while (m--) {
                    subqueries[l].phrases.push(termops.encodePhrase(subqueries[l].phrasesText[m], false, true));
                }
