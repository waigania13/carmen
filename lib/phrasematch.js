...
module.exports = function phrasematch(source, query, options, callback) {
    ...
    while (l--) {
        ...
        if (text) {
            ...
            // probably set fuzzyFlag here to pass whether or not we want to check fuzzy
            // var fuzzyFlag = false unless we are searching for latin letters or we do normalization
            // if (!latinLetters || tryNormalization) {
            //     var fuzzyFlag = true;
            // }
            ...
            console.log('hasPhrase', dawg.hasPhrase(text, scanPrefix));
