...
module.exports = function phrasematch(source, query, options, callback) {
    ...
    loadall(getter, 'freq', ['__MAX__'], function(err) {
        ...
        while (l--) {
            ...
            subqueries[l].loadallshards = loadallshards;
            ...
            subqueries[l].prefix = (options.autocomplete && subqueries[l].ender);

            if (!subqueries[l].prefix) {
                subqueries[l].shards = [+Cache.shard(subqueries[l].phrase)];
                if (subqueries[l].phrase.length < Cache.SHARD_PREFIX_LENGTH) {
                    subqueries[l].shards.push(+Cache.shard(subqueries[l].phrase + "."));
                }
            } else if (subqueries[l].phrase.length >= Cache.SHARD_PREFIX_LENGTH) {
                subqueries[l].shards = [+Cache.shard(subqueries[l].phrase)];
            } else {
                var prefixes = source._dictcache.prefixContinuations(subqueries[l].text, Cache.SHARD_PREFIX_LENGTH);
                var shards = {};
                for (var i = 0; i < prefixes.length; i++) {
                    shards[Cache.shard(prefixes[i])] = true;
                    if (prefixes[i].length < Cache.SHARD_PREFIX_LENGTH) {
                        shards[Cache.shard(prefixes[i] + ".")] = true;
                    }
                }
                subqueries[l].shards = Object.keys(shards).map(function(s) { return +s; });
            }
