...
module.exports = function phrasematch(source, query, options, callback) {
    ...
    var loadallshards = source._geocoder.loadallshards.bind(source._geocoder);

    // if not in bbox, skip
    if (options.bbox) {
        var intersects = bb.intersect(options.bbox, source.bounds);
        if (!intersects) return callback(null, new PhrasematchResult([], getter, loadallshards, source));
    }
    ...
    loadall(getter, 'freq', ['__MAX__'], function(err) {
        ...
        while (l--) {
            ...
            if (text) {
                ...
                var shards = [];
                if (!prefix) {
                    shards = [+Cache.shard(phrase)];
                    if (phrase.length < Cache.SHARD_PREFIX_LENGTH) {
                        shards.push(+Cache.shard(phrase + "."));
                    }
                } else if (phrase.length >= Cache.SHARD_PREFIX_LENGTH) {
                    shards = [+Cache.shard(phrase)];
                } else {
                    var prefixes = source._dictcache.prefixContinuations(text, Cache.SHARD_PREFIX_LENGTH);
                    var shardSet = {};
                    for (var i = 0; i < prefixes.length; i++) {
                        shardSet[Cache.shard(prefixes[i])] = true;
                        if (prefixes[i].length < Cache.SHARD_PREFIX_LENGTH) {
                            shardSet[Cache.shard(prefixes[i] + ".")] = true;
                        }
                    }
                    shards = Object.keys(shardSet).map(function(s) { return +s; });
                }

                phrasematches.push(new Phrasematch(subquery, weight, subquery.mask, phrase, scorefactor, source.idx, source._geocoder, source.zoom, prefix, shards));
            }
        }

        return callback(null, new PhrasematchResult(phrasematches, getter, loadallshards, source));
    });
};

module.exports.PhrasematchResult = PhrasematchResult;
function PhrasematchResult(phrasematches, getter, loadallshards, source) {
    this.phrasematches = phrasematches;
    this.getter = getter;
    this.loadallshards = loadallshards;
    this.idx = source.idx;
...
function Phrasematch(subquery, weight, mask, phrase, scorefactor, idx, cache, zoom, prefix, shards) {
...
Phrasematch.prototype.clone = function() {
    return new Phrasematch(this.subquery.slice(), this.weight, this.mask, this.phrase, this.scorefactor, this.idx, this.cache, this.zoom, this.prefix, this.shards);
};
