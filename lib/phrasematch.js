...
module.exports = function phrasematch(source, query, autocomplete, callback) {
    ...
    // console.log('subqueries:', subqueries); //still has mfs
    ...
    loadall(getter, 'freq', [1], function(err) {
        ...
        while (l--) {
            if (!source._dictcache.hasPhrase(subqueries[l])) continue; // This is where it drop queries not already stored in the cache
            // Augment permutations with matched grids,
            // index position and weight relative to input query.
            console.log('\n ## subquery[l]:', subqueries[l]);
