...
module.exports = function phrasematch(source, query, callback) {
    ...
    for (var l = 0; l < subqueries.length; l++) {
        ...
        toLook.push(termops.look(phrase));
    }

    source._geocoder.loadall(getter, 'look', toLook, function(err) {
        if (err) return callback(err);
        var l = subqueries.length;
        var toLoad = [];
        for (var l = 0; l < subqueries.length; l++) {
            if (source._geocoder.exists('look', toLook[l])) {
                toLoad.push(subqueries[l].phrase);
            }
        }
