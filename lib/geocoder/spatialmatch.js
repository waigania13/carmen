...
function spatialmatch(query, phrasematchResults, options, stats_data, debug_data, callback) {
    let stacks;
    let run_times;

    if (options.stats) {
        run_times = {
            'collapseToArchetypes': null,
            'stackable': null,
            'allowed': null,
            'sortByZoomIdx': null,
            'sortByRelevLengthIdx': null,
            'expandFromArchetypes': null,
            'rebalance': null,
            'coalesceStacks': null
        }
    }
    ...
    if (phrasematchResults.length) {
        ...
        if (options.stats) { run_times.collapseToArchetypes = +new Date; }
        const archetypes = collapseToArchetypes(phrasematchResults);
        if (options.stats) { run_times.collapseToArchetypes = +new Date - run_times.collapseToArchetypes; }

        if (options.stats) { run_times.stackable = +new Date; }
        let arch_stacks = stackable(archetypes, options.stackable_limit);
        if (options.stats) { run_times.stackable = +new Date - run_times.stackable; }

        if (options.stats) { run_times.allowed = +new Date; }
        arch_stacks = allowed(arch_stacks, options);
        if (options.stats) { run_times.allowed = +new Date - run_times.allowed; }

        if (options.stats) { run_times.sortByZoomIdx = +new Date; }
        arch_stacks.forEach((arch_stack) => { arch_stack.sort(sortByZoomIdx); });
        if (options.stats) { run_times.sortByZoomIdx = +new Date - run_times.sortByZoomIdx; }

        if (options.stats) { run_times.sortByRelevLengthIdx = +new Date; }
        arch_stacks.sort(sortByRelevLengthIdx);
        if (options.stats) { run_times.sortByRelevLengthIdx = +new Date - run_times.sortByRelevLengthIdx; }

        arch_stacks = arch_stacks.slice(0, options.spatialmatch_stack_limit);

        if (options.stats) { run_times.expandFromArchetypes = +new Date; }
        stacks = expandFromArchetypes(arch_stacks, options.spatialmatch_stack_limit);
        if (options.stats) { run_times.expandFromArchetypes = +new Date - run_times.expandFromArchetypes; }
    } else {
        stacks = [];
    }

    // Rebalance weights, relevs of stacks here.
    if (options.stats) { run_times.rebalance = +new Date; }
    for (let i = 0; i < stacks.length; i++) {
        stacks[i] = rebalance(query, stacks[i]);
    }
    if (options.stats) { run_times.rebalance = +new Date - run_times.rebalance; }
    ...
    function coalesceStacks() {
        if (options.stats) { run_times.coalesceStacks = +new Date; }
        const q = queue();
    ...
    function coalesceFinalize(err, results) {
        ...
        if (options.stats) {
            run_times.coalesceStacks = +new Date - run_times.coalesceStacks;
            stats_data.spatialmatch.run_times = run_times;
        }

        return callback(null, { results: filteredSpatialmatches, sets: sets, waste: waste }, stats_data, debug_data);
    }
}
...
function stackable(phrasematchResults, limit, memo, idx, mask, nmask, stack, relev, adjRelev) {
    ...
    for (let i = 0; i < phrasematches.length; i++) {
        ...
        // do these two phrasematches overlap?
        if (mask & next.mask) continue;
        // are the two phrasematches from the same geocoder_name?
