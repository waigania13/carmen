...
function sortByRelevLengthIdx(a, b) {
    ...
    if (Math.abs(a.length - b.length) > 1) {
        // if the difference in stack lengths is > 1, then it's worth considering
        // before looking at adjusted relevance
        first = (b.relev - a.relev) ||
            (a.length - b.length) ||
            (b.adjRelev - a.adjRelev) ||
            (b[b.length - 1].scorefactor - a[a.length - 1].scorefactor);
    } else {
        // if the difference in stack lengths is <= 1, then adjusted relevance
        // should be considered before difference in length
        first = (b.adjRelev - a.adjRelev) ||
            (a.length - b.length) ||
            (b[b.length - 1].scorefactor - a[a.length - 1].scorefactor);
    }
