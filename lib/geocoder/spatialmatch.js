...
const coalesce = require('@mapbox/carmen-core').coalesce;
...
function spatialmatch(query, phrasematchResults, options, callback) {
    ...
    for (let i = 0; i < stacks.length; i++) {
        ...
        stacks[i] = stackToTrees(stacks[i]);
    }
     ...
     * @returns {undefined}
     */
    function coalesceStack(trees, callback) {
        let stack = Array.from(trees);
        ...
        const partialNumber = stack.length > 0 ? stack[stack.length - 1][0].partialNumber : false;
        ...
        for (const match of stack) {
            if (match.address) {
                addressDataByIdx.set(match[0].idx, match[0].address);
            }
            maxZoom = Math.max(maxZoom, match[0].zoom);
        }
        ...
        coalesce(trees, coalesceOpts, (err, cacheSpatialmatches) => {
            if (err) callback(err);
...
function stackToTrees(stack) {
    let tree = new Map();
    let visitedNode = [tree];
    for (let i = 0; i < stack.length; i++) {
        let nodeLen = visitedNode.length;
        for (var j = 0; j < nodeLen; j++) {
            visitedNode[j].set(stack[i], new Map());
            visitedNode.push(visitedNode[j].get(stack[i]));
        }
    }
    return tree;
}
