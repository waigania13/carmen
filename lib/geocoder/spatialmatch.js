...
function spatialmatch(query, phrasematchResults, options, callback) {
    ...
    if (phrasematchResults.length) {
        ...
        quickselect(arch_stacks, (options.spatialmatch_stack_limit < 10) ? 30 : options.spatialmatch_stack_limit, 0, arch_stacks.length - 1, sortByRelevLengthIdx);
        arch_stacks = arch_stacks.filter(Boolean);
        arch_stacks = arch_stacks.slice(0, (options.spatialmatch_stack_limit < 10) ? 30 : options.spatialmatch_stack_limit);
