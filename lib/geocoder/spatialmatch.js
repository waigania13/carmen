...
const parallelCoalesce = require('@mapbox/carmen-core').parallelCoalesce;
...
function spatialmatch(query, phrasematchResults, options, callback) {
    ...
    function coalesceStacks() {
        const stacksWithOpts = [];
        for (let i = 0; i < stacks.length; i++) {
            const opts = getCoalesceOpts(stacks[i]);
            if (opts) {
                stacksWithOpts.push({
                    stack: stacks[i],
                    match_opts: opts
        ...
        parallelCoalesce(stacksWithOpts, (err, allCacheSpatialmatches) => {
            if (err) callback(err);
            const results = [];
            for (let i = 0; i < stacksWithOpts.length; i++) {
                const stack = stacksWithOpts[i].stack;
                const cacheSpatialmatches = allCacheSpatialmatches[i] || [];
                const opts = stacksWithOpts[i].match_opts;
                ...
                for (let i = 0; i < cacheSpatialmatches.length; i++) {
                    spatialmatches.push(new Spatialmatch(cacheSpatialmatches[i], stackByIdx, opts.addressDataByIdx, opts.partialNumber));
                }
                results.push(spatialmatches);
            }
            coalesceFinalize(null, results);
        });
    }
     ...
     * @returns {undefined}
     */
    function getCoalesceOpts(stack, callback) {
        ...
        coalesceOpts.partialNumber = stack.length > 0 ? stack[stack.length - 1].partialNumber : false;
        // see if we have address metadata on any of the entries
        coalesceOpts.addressDataByIdx = new Map();
        let maxZoom = 0;
        for (const match of stack) {
            if (match.address) {
                coalesceOpts.addressDataByIdx.set(match.idx, match.address);
            }
            maxZoom = Math.max(maxZoom, match.zoom);
        }
        ...
        if (options) {
            ...
            if (coalesceOpts.partialNumber && options.proximity) {
                ...
                if (options.bbox) {
                    ...
                    if (!pnBbox) return null;
                }
                coalesceOpts.bbox = bbox.insideTile(pnBbox, maxZoom).slice(1);
            } else if (options.bbox) {
                coalesceOpts.bbox = bbox.insideTile(options.bbox, maxZoom).slice(1);
            }
        }
    ...
    for (const inResult of phrasematchResults) {
        ...
        for (const group of collapsed) {
            if (
                ...
                group.prefix !== ENDING_TYPE.nonPrefix &&
