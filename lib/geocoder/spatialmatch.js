...
module.exports.rebalance_new = rebalance_new;
...
function spatialmatch(query, phrasematchResults, options, callback) {
    ...
    function done(err, results) {
        ...
        results = results || [];
        const rebalancedResults = results.map((result) => rebalance(query, result, preparedPhrasematches.flattenedPhrasematches));
        const combined = rebalancedResults.map((result) => new Spatialmatch(result, preparedPhrasematches.flattenedPhrasematches));
        combined.sort(sortByRelevLengthIdx);
...
function sortByRelevLengthIdx(a, b) {
    const first = (b.adjRelev - a.adjRelev) ||
        (a.length - b.length) ||
        (b.relev - a.relev) ||
        (b.covers[b.covers.length - 1].scorefactor - a.covers[a.covers.length - 1].scorefactor);
