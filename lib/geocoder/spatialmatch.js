...
function spatialmatch(query, phrasematchResults, options, callback) {
    ...
    if (phrasematchResults.length) {
        ...
        // quickselect only if the number of stacks is greater than 30 or atleast more than 10
        if (arch_stacks.length > 10 && arch_stacks.length > options.spatialmatch_stack_limit) {
            quickselect(arch_stacks, options.spatialmatch_stack_limit, 0, arch_stacks.length - 1, sortByRelevLengthIdx);
