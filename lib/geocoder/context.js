...
module.exports = function(geocoder, position, options, callback) {
    options = options || {};
    const maxidx = typeof options.maxidx === 'number' ? options.maxidx : geocoder.byidx.length;
    const matched = options.matched || {};
    const language = options.language || false;

    const indexes = geocoder.byworldview.get(options.worldview).filter((index) => {
        return index.idx < maxidx && bbox.amInside(position, index.bounds);
    });

    if (options.stacks && options.forwardFeature) {
        // if we're going to filter by geocoder_stack, we might end up looking
        // at just the top-level feature, or possibly also the bottom-level feature,
        // depending on their respective worldviews. Ideally let's not load the full
        // context before we make up our minds; let's instead either decide right away
        // if the worldviews agree, or load just enough info from the bottom-level
        // feature to decide that way before we load everything else
        if (filter.featureMatchesStacks(options.forwardFeature, options)) {
            // we're good, just based on the feature itself
            return queueIndexQueries(geocoder, position, options, indexes, callback);
        } else if (indexes.length) {
            const featWorldview = geocoder.indexes[options.forwardFeature.properties['carmen:index']].geocoder_worldview;
            const baseWorldview = indexes[0].geocoder_worldview;
            if (featWorldview !== baseWorldview) {
                // the base index has a different worldview than the top index; fetch just the base to check
                contextVector(indexes[0], position[0], position[1], false, matched, language, false, options.reverseMode, options.routing, (err, results) => {
                    if (results && filter.featureMatchesStacks(results, options)) {
                        // the original feature failed, but the worldview override worked
                        return queueIndexQueries(geocoder, position, options, indexes, callback);
                    } else {
                        // the worldview override still failed
                        callback(null, false);
                    }
                });
            } else {
                // the bottom index is the same worldview, so the original match failure stands
                callback(null, false);
            }
        } else {
            // we can't check the base element because there isn't one, so the original failure stands
            return callback(null, false);
        }
    } else {
        // there was no stack filter; proceed as usual
        return queueIndexQueries(geocoder, position, options, indexes, callback);
    }
};

/**
 * If the filters passed above, actually do the context fetching.
 *
 * @access public
 *
 * @param {Object} geocoder: geocoder instance
 * @param {Array} position: [lon, lat]
 * @param {Object} options: optional options object
 * @param {Object} indexes: the indexes identified above as containing maybe-relevant features
 * @param {Function} callback method that expects err, contexts
 * @return {undefined}
 */
function queueIndexQueries(geocoder, position, options, indexes, callback) {
    ...
    for (const source of indexes) {
        ...
        if (options.targetFeature && (source.type === indexes[indexes.length - 1].type)) {
...
/**
 * contextVector loads a tile and looks around for features. The results
 * of that are then passed here. This function sorts and filters the results.
 * @param {Object} results - Results from a VTQuery call
 * @param {Object} source - a geocoding datasource
 * @param {Number} lon - value of longitude
 * @param {Number} lat - value of latitude
 ...
 */
function processVtQueryResults(results, source, lon, lat,  full, matched, language, scoreFilter,reverseMode, routing, callback) {
    ...
    if (reverseMode === 'score' && full && source.geocoder_reverse_mode) {
        mapped = results.map((el, i) => {
            return {
                ...
                distance: el.tilequery.distance
            };
        });

        mapped.sort(reverseSortByDistScore);
...
function contextVector(source, lon, lat, full, matched, language, scoreFilter, reverseMode, routing, callback) {
    ...
    /**
     * Provides the callback function for the tileCover call. This
     * takes a loaded vector tile and queries it for features near
     * the given lat and lon.  It then passes the data on for post
     * processing in processVtQueryResults.
     * @param {Error} err - Error
     * @param {Object} vt - Vector Tile
    ...
    function query(err, vt) {
        ...
        // Enable basic filtering when in distance-based reverse mode and not querying against a specific feature
        // to avoid being overwhelmed by low scored features
        if (reverseMode === 'distance') {
            vtquery([vt], [lon, lat], {
                radius: 1000,
                layer: source.geocoder_layer,
                limit: 5,
                ...
                return processVtQueryResults(results, source, lon, lat,  full, matched, language, scoreFilter,reverseMode, routing, callback);
            });
        } else {
            vtquery([vt], [lon, lat], {
                radius: 1000,
...
function lightFeature(source, feat, callback) {
    ...
    loaded.properties['carmen:idx'] = source.idx;
...
/**
 * Sort method - Prioritize Higher distscore, then lower distance for tiebreaker
 * @param {object} a - object with `distscore` parameter and `distance` parameter
 * @param {object} b - object with `distscore` parameter and `distance` parameter
 * @return {number} sort order
 */
function reverseSortByDistScore(a, b) {
    // Sort by higher distscore then lower distance
    return (b.distscore - a.distscore) || (a.distance - b.distance);
}
