...
function stackFeatures(geocoder, loaded, options) {
    ...
    for (let i = 0; i < loaded.length; i++) {
        ...
        const stack = feature.properties['internal:stack'];

        for (let l = feature.properties['carmen:types'].length - 1; l >= 0; l--) {
            const type = feature.properties['carmen:types'][l];
            const conflict = feature.properties['internal:conflict'] || type;
            ...
            if (memo[type] === undefined) {
                ...
                feature.properties['internal:extid'] = type + '.' + feature.properties['internal:extid'].split('.').pop();
                break;
            } else if (memo[type] && feature.properties['internal:geomtype'] !== 3) {
                ...
                if (feature.properties['internal:vtquerydist'] >= memo[type].properties['internal:vtquerydist']) continue;
    ...
    for (let k = 0; k < types.length; k++) {
        ...
        delete toAdd.properties['internal:stack'];
        delete toAdd.properties['internal:conflict'];
        ...
        vt.query(lon, lat, {
            ...
            for (let i = 0; i < results.length; i++) {
                ...
                if (resultsSorted) {
                    ...
                    attr['internal:vtquerydist'] = results[i].distance;
                    attr['internal:geomtype'] = results[i].geometry().type();
                    feat = attr;
                    break;
                }
...
function lightFeature(source, feat, callback) {
    ...
    loaded.properties['internal:extid'] = source.type + '.' + feat.id;
    loaded.properties['internal:tmpid'] = (source.idx * mp25) + termops.feature(feat.id);
    loaded.properties['internal:index'] = source.id;
    loaded.properties['internal:stack'] = source.stack;
    loaded.properties['internal:conflict'] = source.name !== source.type ? source.name : undefined;
...
function fullFeature(source, feat, query, routing, callback) {
    feature.getFeatureById(source, feat.id, (err, loaded) => {
        ...
        loaded.properties['internal:extid'] = source.type + '.' + feat.id;
        loaded.properties['internal:tmpid'] = (source.idx * mp25) + termops.feature(feat.id);
        loaded.properties['internal:vtquerydist'] = feat['internal:vtquerydist'];
        loaded.properties['internal:geomtype'] = feat['internal:geomtype'];
        loaded.properties['internal:stack'] = source.stack;
        loaded.properties['internal:conflict'] = source.name !== source.type ? source.name : undefined;
        loaded.properties['internal:reverseMode'] = source.geocoder_reverse_mode;
