...
function getPlaceName(context, formatString, language, languageMode, matched) {
    ...
    if (!formatString || formatString === true || formatString === 1) {
        place_name = ((feat.properties['internal:address'] ? feat.properties['internal:address'] + ' ' : '') +
            context
        ...
        for (let i = 0; i < template.length; i++) {
            ...
            for (let x = 0; x < context.length; x++) {
                ...
                const carmenProp = f.properties['internal:extid'].split('.')[0]; // ex. 'place', 'postcode', 'region'
                ...
                if (templateProp === carmenProp) {
                    if (templateSubprop === '_name') {
                        ...
                        // `address` is a hardcored subproperty that maps to internal:address
                        val = f.properties['internal:address'];
...
function toFeature(context, format, languages, languageMode, debug, geocoder, clipBBox, routing) {
    ...
    if (!feat.properties['internal:extid'])  throw new Error('Feature has no internal:extid');

    languages = languages && languages.length ? languages : [''];

    const feature = {
        id: feat.properties['internal:extid'],
        type: 'Feature',
    ...
    languages.reduce((memo, language, i) => {
        ...
        if (i === 0) {
            ...
            if (geocoder) {
                for (let k = 0; k < context.length; k++) {
                    const matched = !!context[k].properties['internal:query_text'] && getMatchingText(context[k], geocoder, language);
                    if (matched) {
    ...
    if (feat.properties['internal:address']) feature.address = String(feat.properties['internal:address']);
    ...
    if (context.length > 1) {
        ...
        for (let c = 1; c < context.length; c++) {
            if (!context[c].properties['internal:extid']) throw new Error('Feature has no internal:extid');
            if (!featureMatchesLanguage(context[c], { language: languages, languageMode: languageMode })) continue;

            const contextFeat = { id: context[c].properties['internal:extid'] };
...
function getMatchingText(item, geocoder, requestedLanguage) {
    ...
    if (item.properties['internal:matches_language'] && (requestedLanguage ? closest === requestedLanguage : true) && !/,/.exec(item.properties[textKey])) return;

    // now we get to play the came of figuring out which phrase this was originally
    const source = geocoder.byidx[item.properties['internal:idx']];
    ...
    for (const key of Object.keys(item.properties)) {
        ...
        item.properties[key].split(',').forEach((sourceText) => {
            ...
            if (texts.length) {
                const phrases = termops.getIndexablePhrases(texts[0].tokens, { '__COUNT__': [1] });
    ...
    const query = termops.encodableText(item.properties['internal:query_text']);
    const regex = new RegExp('^' + query + (item.properties['internal:prefix'] ? '' : '$'));
    ...
    if (!Object.keys(matches).length) {
        const regex = new RegExp('^' + query.replace(/^[0-9]*#+ /, '') + (item.properties['internal:prefix'] ? '' : '$'));
        for (const candidate of allPhrases.keys()) {
            if (regex.exec(candidate)) {
                for (const phraseKey of allPhrases.get(candidate)) {
                    if (!matches.hasOwnProperty(phraseKey)) matches[phraseKey] = candidate;
