...
function forwardPrefixFiltered(feat, address, options, cover) {
    let matchingAddressPoints = forwardPrefix(feat, query);
    ...
    if (matchingAddressPoints.length > 0) {
        ...
        feat.properties['carmen:address'] = firstMiddleLast[0].number;
        feat.properties['carmen:distance'] = firstMiddleLast[0].distance;
        feat.geometry = firstMiddleLast[0].geometry;
