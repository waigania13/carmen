...
module.exports = function phrasematch(source, query, options, callback) {
    ...
    if (!source.geocoder_address) {
        ...
        for (const match of windowMatches) {
            ...
            const maskSpan = 1 + (maskEnd - maskBegin);
            ...
            // same owner as our end we are in the middle of a user
            // entered token and need to discard this match.
            if (endPos !== (normalized.owner.length - 1) && normalized.owner[endPos + 1] === maskEnd) continue;

            subquery.mask = buildMask(maskBegin, maskSpan);
        ...
        while (l--) {
            ...
            for (let i = 0; i < phraseSetMatches.length; i++) {
                ...
                // Then, as with the non-address code we lookup the build a new
                // mask.
                const maskBegin = normalized.owner[lim[0]];
                const maskSpan = 1 + (normalized.owner[lim[1]] - maskBegin);
                sq.mask = buildMask(maskBegin, maskSpan);
                ...
                for (let i = 0; i < gapExpansionMasks.length; i++) {
                    if (gapExpansionMasks[i] & sq.mask) {
                        const m = (gapExpansionMasks[i] | sq.mask);
    ...
    for (const subquery of subqueries) {
        ...
        let weight;
        if (query.tokens.length === normalized.tokens.length) {
            weight = subquery.length / query.tokens.length;
        } else {
            const b = findMaskBounds(subquery.mask, 20);
            weight = (b[1] - b[0] + 1) / query.tokens.length;
        }
...
/**
 * Detect the start and end of a continuour block of `1` bits in a bitmask.
 *
 * @param {number} mask
 * @param {number} limit
