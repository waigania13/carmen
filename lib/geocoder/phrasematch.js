...
module.exports = function phrasematch(source, query, options, callback) {
    ...
    // TODO move upstream, maybe into encodeableText
    ...
    if (!source.geocoder_address) {
        ...
        let allSubqueries = termops.permutations(query.tokens);
        allSubqueries = termops.uniqPermutations(allSubqueries);
        // TODO not sure the mask's we've generated here will not work if the cardinality has changes, need to confirm.

        if (query.tokens.length > 1) {
            // Include housenum tokenized permutations
            // but only if there are multiple words so we know the number part
            // is complete
            const numTokenized = termops.numTokenize(query.tokens, source.version);
            for (let i = 0; i < numTokenized.length; i++) {
                allSubqueries = allSubqueries.concat(termops.permutations(numTokenized[i]));
            }
        } else if (query.tokens.length === 1 && query.tokens[0].match(/^\d+$/)) {
            if (proxMatch) {
                ...
                const numTokenized = termops.numTokenizePrefix(query.tokens, source.version);
                for (let i = 0; i < numTokenized.length; i++) {
                    allSubqueries = allSubqueries.concat(termops.permutations(numTokenized[i]));
                }
                partialNumber = true;
