...
module.exports = function phrasematch(source, query, options, callback) {
    ...
    if (!source.geocoder_address) {
        ...
        const windowMatches = phraseSet.fuzzy_match_windows(tokenized, maxDistance, maxDistance, options.autocomplete);
        ...
        let l = allSubqueries.length;
        while (l--) {
            const subquery = allSubqueries[l];
            if (subquery) {
                const scanPrefix = subquery.ender && options.autocomplete;

                const phraseSetMatches = scanPrefix ?
                    phraseSet.fuzzy_match_prefix(subquery, maxDistance, maxDistance) :
                    phraseSet.fuzzy_match(subquery, maxDistance, maxDistance);
