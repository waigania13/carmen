...
module.exports = function phrasematch(source, query, options, callback) {
    ...
    while (l--) {
        ...
        if (text) {
            ...
            const dawgMatch = dawg.hasPhrase(text, scanPrefix);

            if (!dawgMatch) continue;

            // TODO: fuzzyphraseset doesn't need the subquery to be joined <2018-06-21, boblannon> //
            const variants = [{ text: subquery.join(' '), prefix: scanPrefix }];
