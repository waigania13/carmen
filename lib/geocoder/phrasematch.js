...
module.exports = function phrasematch(source, query, options, callback) {
    ...
    const maxDistance = (options.fuzzyMatch && tokenized.tokens.length <= options.max_correction_length) ? 1 : 0;
    ...
    if (!source.geocoder_address) {
        ...
        const windowMatches = phraseSet.fuzzyMatchWindows(tokenized.tokens, maxDistance, maxDistance, endingType);

        subqueries = [];
        for (const match of windowMatches) {
            const subquery = match.phrase;
            subquery.original_phrase = tokenized.tokens.slice(
        ...
        let allSubqueries = termops.permutations(tokenized.tokens);
        ...
        if (tokenized.tokens.length > 1) {
            ...
            // is complete
            const numTokenized = termops.numTokenize(tokenized.tokens, source.version);
            for (let i = 0; i < numTokenized.length; i++) {
                allSubqueries = allSubqueries.concat(termops.permutations(numTokenized[i]));
            }
        } else if (tokenized.tokens.length === 1 && tokenized.tokens[0].match(/^\d+$/)) {
            if (proxMatch) {
                ...
                const numTokenized = termops.numTokenizePrefix(tokenized.tokens, source.version);
                for (let i = 0; i < numTokenized.length; i++) {
                    allSubqueries = allSubqueries.concat(termops.permutations(numTokenized[i]));
                }
                partialNumber = true;
    ...
    for (const subquery of subqueries) {
        ...
        const weight = subquery.length / tokenized.tokens.length;
