...
module.exports.contextMatchesStacks = contextMatchesStacks;
...
function contextMatchesStacks(context, options, indexes) {
    if (featureMatchesStacks(context[0], options)) return true;

    if (context.length > 1) {
        const bookends = [context[0], context.slice(-1)[0]];
        const worldviews = bookends.map((feature) => indexes[feature.properties['carmen:index']].geocoder_worldview);
        if (worldviews[0] !== worldviews[1]) {
            // the top and bottom of the stack are from different worldviews, so
            // consider the bottom as well
            if (featureMatchesStacks(bookends[1], options)) return true;
        }
    }
    return false;
}
