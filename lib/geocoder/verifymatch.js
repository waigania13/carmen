...
function verifyFeatures(query, geocoder, spatialmatches, loaded, options) {
    ...
    for (let pos = 0; pos < loaded.length; pos++) {
        ...
        if (feats[0].intersections) address = {
            addr: feats[0].properties['carmen:addressnumber'].slice(-1)[0]
        };
        ...
        if (source.geocoder_address) {
            if (address && (feats[0].properties['carmen:addressnumber'] || feats[0].properties['carmen:rangetype'])) {
                ...
                if (addressPoints.length) {
                    ...
                    if (feats[0].intersections) feats[0].properties['carmen:address'] = feats[0].properties['carmen:addressnumber'].slice(-1)[0];
                    // The feature is an address cluster or range but does not match this cover, return a street result
                    else feats[0].properties['carmen:address'] = false;
                }
