...
* @param {Object} stats_data - an object for capturing detailed statistics (only used if options.stats = true)
* @param {Object} debug_data - an object for capturing debug info (only used if options.debug = true)
* @param {Function} callback - callback function which is called with the verified indexes in the correct hierarchical order
*/
function verifymatch(query, geocoder, matched, options, stats_data, debug_data, callback) {
    ...
    if (options.stats) { run_times.loadFeatures = +new Date; }
    ...
    function afterFeatures(err, loaded) {
        ...
        if (options.types || options.stacks || options.languageMode === 'strict') {
            ...
            if (options.stats) {
                run_times.loadFeatures = +new Date - run_times.loadFeatures;
                run_times.verifyFeatures = +new Date;
            }
        ...
        if (options.stats) {
            run_times.verifyFeatures = +new Date - run_times.verifyFeatures;
            run_times.loadContexts = +new Date;
            stats_data.verifymatch.run_times = run_times;
        }
        loadContexts(geocoder, verified, sets, options, stats_data, debug_data, callback);
    }
}
...
*/
function loadContexts(geocoder, features, sets, options, stats_data, debug_data, callback) {
    const q = queue(5);
    ...
    q.awaitAll((err, contexts) => {
        if (err) return callback(err);
        let verifiedContexts = verifyContexts(contexts, sets, geocoder.indexes);
        if (options.stats) { stats_data.verifymatch.run_times.loadContexts = +new Date - stats_data.verifymatch.run_times.loadContexts; }
        callback(null, verifyContexts(contexts, sets, geocoder.indexes), stats_data, debug_data);
    });
}
