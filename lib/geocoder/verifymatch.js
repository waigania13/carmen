...
function verifyFeatures(query, geocoder, spatialmatches, loaded, options) {
    ...
    for (let pos = 0; pos < loaded.length; pos++) {
        ...
        // this is passed-through information from phrasematch about where we
        // what and where we think the address number is in this query
            ...
            if (feats[0].properties['carmen:addressnumber'] || feats[0].properties['carmen:rangetype']) {
                ...
                if (spatialmatch.partialNumber) {
                    ...
                    if (feats[0].properties['carmen:addressnumber']) {
                        ...
                        const matchingAddressPoints = addressCluster.forwardPrefix(feats[0], query[0]);
                        ...
                        if (matchingAddressPoints.length > 0) {
                            ...
                            addressPoints = [{
                                type:'Point',
                                coordinates: [
                                    Math.round(firstMiddleLast[0].geometry.coordinates[0] * 1e6) / 1e6,
                                    Math.round(firstMiddleLast[0].geometry.coordinates[1] * 1e6) / 1e6
                                ]
                            }];
                            feats[0].properties['carmen:address'] = firstMiddleLast[0].number;
                            feats[0].properties['carmen:distance'] = firstMiddleLast[0].distance;
                        } else {
                            feats = [];
                        }
                    } else {
...
/**
* sortContext - sort contexts based on the relevance, then score, layer type, address.positionition, spatialmatch position and then id
*/
