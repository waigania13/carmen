        ...
        if (source.geocoder_address) {
            if (address && (feats[0].properties['carmen:addressnumber'] || feats[0].properties['carmen:rangetype'])) {
                ...
                if (feats[0].properties['carmen:addressnumber']) {
                    addressFeats = addressCluster.forward(feats[0], address.addr);
                }

                // If feature did not match a point and has interpolation - interpolate a match
                if (!addressFeats.length && feats[0].properties['carmen:rangetype']) {
                    const itpPoint = addressItp.forward(feats[0], address.addr);
                    addressFeats = itpPoint ? [itpPoint] : [];
                }

                // If there was an address match then set up a new result
                if (addressFeats.length) {
                    feats = addressFeats.slice(0,10).map((addressFeat) => {
