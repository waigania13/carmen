                ...
                if (addressPoints.length) {
                    const newFeats = addressPoints.slice(0,10).map((addressPoint) => {
                        ...
                        if (options.routing && source.geocoder_routable) {
                            const routPoints = routablePoints(addressPoint.geometry, feat);
                            if (routPoints) {
                        ...
                        feat.geometry = addressPoint.geometry;
                        feat.properties['carmen:center'] = feat.geometry && feat.geometry.coordinates;

                        for (const prop of Object.keys(addressPoint.properties)) {
                            feat.properties[prop] = addressPoint.properties[prop];
                        }
