...
function verifyContext(context, peers, strict, loose, indexes, options, geocoder) {
    ...
    for (let c = 0; c < context.length; c++) {
        let backy = false;
        let backyExempt = false;
        if (c > 0) {
            backyExempt = indexes[context[c].properties['carmen:index']].geocoder_backy_exempt ||
                          indexes[context[c - 1].properties['carmen:index']].geocoder_backy_exempt;
        }
