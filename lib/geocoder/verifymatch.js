...
function verifyContextChunk(geocoder, verifiedContexts, sets, options, callback) {
    loadContexts(geocoder, verifiedContexts.batch, sets, options, (err, contextResults) => {
        ...
        if (verifiedContexts.backfill.length === 0) {
            let sortedResult = verifiedContexts.results.sort(sortContext);
            if (sortedResult.length > options.limit_verify) {
                const partialNumberLimit = 0.8 * options.limit_verify;
                let partialNumberCount = 0;
                sortedResult = sortedResult.filter((context) => {
                    if (context[0].properties['carmen:spatialmatch'.partialNumber]) {
                        partialNumberCount++;
                        return partialNumberCount < partialNumberLimit;
                    } else {
                        return true;
                    }
                }).slice(0, options.limit_verify);
            }

            return callback(null, sortedResult);
        }
