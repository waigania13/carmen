...
*/
function verifymatch(query, stats_data, geocoder, matched, options, callback) {
    ...
    function afterFeatures(err, loaded) {
        ...
        loadContexts(geocoder, verified, sets, options, stats_data, callback);
    }
}
...
*/
function loadContexts(geocoder, features, sets, options, stats_data, callback) {
    const q = queue(5);
    ...
    q.awaitAll((err, contexts) => {
        if (err) return callback(err);
        callback(null, verifyContexts(contexts, sets, geocoder.indexes), stats_data);
    });
}
