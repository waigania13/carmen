    ...
    function afterFeatures(err, loaded) {
        ...
        if (options.types || options.stacks || options.languageMode === 'strict') {
            ...
            loaded.forEach((feature, i) => {
                ...
                const source = geocoder.indexes[feature.properties['internal:index']];
...
function verifyFeatures(query, geocoder, spatialmatches, loaded, options) {
    ...
    for (let pos = 0; pos < loaded.length; pos++) {
        ...
        if (source.geocoder_address) {
            if (address && (feats[0].properties['carmen:addressnumber'] || feats[0].properties['carmen:rangetype'])) {
                feats[0].properties['internal:address'] = address.addr;
                ...
                if (addressPoints.length) {
                    ...
                    feats[0].properties['internal:address'] = false;
                }

            } else {
                feats[0].properties['internal:address'] = null;
            }
        ...
        for (const feat of feats) {
            ...
            feat.properties['internal:score'] = feat.properties['carmen:score'] || 0;
            feat.properties['internal:extid'] = lastType + '.' + feat.id;
            feat.properties['internal:tmpid'] = cover.tmpid;

            feat.properties['internal:distance'] = proximity.distance(options.proximity, feat.properties['carmen:center'], cover);
            feat.properties['internal:position'] = pos;
            feat.properties['internal:spatialmatch'] = spatialmatch;
            ...
            if (feat.properties['internal:address'] === false) cover.relev *= 0.99;

            feat.properties['internal:relev'] = cover.relev;
            feat.properties['internal:geocoder_address_order'] = source.geocoder_address_order;
            feat.properties['internal:zoom'] = cover.zoom;
            result.push(feat);
        }
    }
    ...
    for (let k = 0; k < result.length; k++) {
        ...
        if (options.proximity) {
            feat.properties['internal:scoredist'] = proximity.scoredist(
                feat.properties['internal:score'],
                geocoder.minScore,
                geocoder.maxScore,
                feat.properties['internal:distance'],
                feat.properties['internal:zoom']
            );
            ...
            feat.properties['internal:scoredist'] = feat.properties['carmen:score'];
        }
    }
    ...
    for (let i = 0; i < result.length; i++) {
        ...
        if (feat.properties['internal:scoredist'] >= 0 || !byText[text]) {
            filtered.push(feat);
            byText[text] = true;
        }
    }
...
function loadContexts(geocoder, features, sets, options, callback) {
    ...
    for (let i = 0; i < features.length; i++) q.defer((f, done) => {
        const name = geocoder.indexes[f.properties['internal:index']].name;
        const firstidx = geocoder.byname[name][0].idx;
        context(geocoder, f.properties['carmen:center'], {
            maxtype: f.properties['internal:extid'].split('.')[0],
            maxidx: firstidx,
            matched: sets,
...
function verifyContexts(contexts, sets, indexes, options, geocoder) {
    ...
    for (let c = 0; c < contexts.length; c++) {
        peers[contexts[c][0].properties['internal:tmpid']] = contexts[c][0];
    }

    for (let a = 0; a < contexts.length; a++) {
        const context = contexts[a];
        context._relevance = 0;
        context._typeindex = indexes[context[0].properties['internal:index']].ndx;

        // Create lookup for covers by tmpid.
        const verify = {};
        let cover;
        const covers = context[0].properties['internal:spatialmatch'].covers;
        ...
        for (const ctx of context) {
            if (verify[ctx.properties['internal:tmpid']]) {
                cover = verify[ctx.properties['internal:tmpid']];
                ctx.properties['internal:matches_language'] = cover.matches_language;
                ctx.properties['internal:prefix'] = cover.prefix;
                ctx.properties['internal:query_text'] = cover.text;
                ctx.properties['internal:idx'] = cover.idx;
            }
        }
 ...
 * @param {Object} peers - A mapping from `internal:tmpid`s to features, used when applying the "squishy" logic for nested, identically-named features.
 * @param {Object} strict - A mapping from `internal:tmpid`s to covers matched by some substring of the query
 * @param {Object} loose - A mapping from `internal:tmpid`s to the cover with that tmpid whose `relev` value is greatest (across all result contexts).
...
function verifyContext(context, peers, strict, loose, indexes, options, geocoder) {
    const addressOrder = context[0].properties['internal:geocoder_address_order'];
    let usedmask = 0;
    let lastmask = -1;
    let lastgroup = -1;
    ...
    if (indexes[context[0].properties['internal:index']].geocoder_inherit_score)
    ...
    for (let c = 0; c < context.length; c++) {
        ...
        if (lastMatchedIndex) {
            ...
            ignoreOrder = indexes[context[c].properties['internal:index']].geocoder_ignore_order ||
                          indexes[lastMatchedIndex].geocoder_ignore_order;
        }
        const feat = context[c];

        const matched = strict[feat.properties['internal:tmpid']] || loose[feat.properties['internal:tmpid']];
        ...
        if (
            squishyTarget &&
            (c > 0) &&
            peers[feat.properties['internal:tmpid']] &&
            textAlike(squishyTarget, feat.properties) &&
            indexes[feat.properties['internal:index']].geocoder_grant_score
        ) {
            squishy += Math.max(peers[feat.properties['internal:tmpid']].properties['carmen:score'] || 0, 0);
        }
        ...
        lastgroup = indexes[feat.properties['internal:index']].ndx;
        lastMatchedIndex = feat.properties['internal:index'];
    ...
    if (squishy > 0) {
        ...
        // Recalculate internal:scoredist and internal:relevance for smallest squishy feature
        ...
        if (options.proximity) {
            ...
            feat.properties['internal:scoredist'] += squishy;
        }
    }
...
function sortFeature(a, b) {
    return ((b.properties['internal:relevance'] || 0) - (a.properties['internal:relevance'] || 0)) ||
        (b.properties['internal:spatialmatch'].relev - a.properties['internal:spatialmatch'].relev) ||
        ((a.properties['internal:address'] === null ? 1 : 0) - (b.properties['internal:address'] === null ? 1 : 0)) ||
        ((a.geometry && a.geometry.omitted ? 1 : 0) - (b.geometry && b.geometry.omitted ? 1 : 0)) ||
        ((b.properties['internal:scoredist'] || 0) - (a.properties['internal:scoredist'] || 0)) ||
        ((a.properties['internal:position'] || 0) - (b.properties['internal:position'] || 0)) ||
        0;
}
...
function sortContext(a, b) {
    ...
    const ar = a[0].properties['internal:relevance'] || 0;
    const br = b[0].properties['internal:relevance'] || 0;
    if (ar > br) return -1;
    if (ar < br) return 1;

    // sort by scoredist
    const as = a[0].properties['internal:scoredist'] || 0;
    const bs = b[0].properties['internal:scoredist'] || 0;
    if (as > bs) return -1;
    if (as < bs) return 1;
    ...
    if (a[0].properties['internal:address'] && b[0].properties['internal:address']) {
        // Prefer addresses with an earlier position in the query
        // so if you have a query "70 St. #501" it will prioritize 70
        if (a[0].properties['internal:addresspos'] < b[0].properties['internal:addresspos']) return -1;
        if (b[0].properties['internal:addresspos'] < a[0].properties['internal:addresspos']) return 1;
    ...
    if (a[0].properties['internal:position'] < b[0].properties['internal:position']) return -1;
    if (a[0].properties['internal:position'] > b[0].properties['internal:position']) return 1;
