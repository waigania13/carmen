    ...
    for (let k = 0; k < result.length; k++) {
        ...
        if (options.proximity && feat.properties['carmen:score'] >= 0) {
            feat.properties['carmen:scoredist'] =
                Math.max(1, feat.properties['carmen:score']) *
                Math.max(1, proximity.scoredist(feat.properties['carmen:distance'], feat.properties['carmen:zoom'], constants.PROXIMITY_RADIUS, geocoder.scoreConstant));
        } else {
