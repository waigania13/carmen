...
function forwardGeocode(geocoder, query, options, callback) {
    ...
    q.awaitAll((err, phrasematchResults) => {
        ...
        if (options.debug) {
            ...
            for (let idx = 0; idx < phrasematchResults.length; idx++) {
                ...
                for (let x = 0; x < phrasematchResults[idx].phrasematches.length; x++) {
                    ...
                    options.debug.phrasematch[id][phraseText] = {
                        w: matched.weight,
        ...
        spatialmatch(queryData.query, phrasematchResults, options, stats_data, spatialmatchComplete);
    });

    function spatialmatchComplete(err, matched, stats_data) {
        if (err) return callback(err);
        ...
        verifymatch(queryData.query, stats_data, geocoder, matched, options, finalize);
    }

    function finalize(err, contexts, stats_data) {
        if (err) return callback(err);
