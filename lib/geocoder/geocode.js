...
module.exports = function(geocoder, query, options, callback) {
    ...
    if (options.language) {
        ...
        if ((new Set(options.language)).size !== options.language.length) return callback(errcode('options.language should be a list of unique language codes', 'EINVALID'));
    }
    ...
    const asReverse = termops.asReverse(query);
    if (asReverse) return reverseGeocode(geocoder, asReverse, options, callback);
...
* @param {callback} callback - of the accepts err, queryData
* @return {undefined}
*/
function reverseGeocode(geocoder, tokenized, options, callback) {
            ...
            q.awaitAll((err, contexts) => {
                ...
                for (let context_i = 0; context_i < contexts.length; context_i++) {
                    ...
                    if (feat.properties && feat.properties['carmen:tmpid']) {
                        const dedupeId = feat.properties['carmen:tmpid'] + '-' + (feat.properties['carmen:address'] || '');
                        if (observedIds.has(dedupeId)) continue;
                        observedIds.add(dedupeId);
                    }
    ...
    /**
     * # Assigns feature to queryData
     * @param {Error} err - error
     * @param {Array<object>} contexts - context list
     * @returns {undefined}
     */
 ...
 * @returns {undefined}
 */
function forwardGeocode(geocoder, text, options, callback) {
    options.limit = options.limit ? (options.limit > 10 ? 10 : options.limit) : 5;
    const query = termops.tokenize(token.replaceGlobalTokens(geocoder.replacer, text));

    if (query.tokens.length > constants.MAX_QUERY_TOKENS) {
        return callback(errcode('Query too long - ' + query.tokens.length + '/' + constants.MAX_QUERY_TOKENS + ' tokens', 'EINVALID'));
    }
    ...
    /**
     * Local callback that captures debug info before calling verifymatch
     * @param {Error} err - error
     * @param {Object} matched - Object with spatial match results
     * @returns {undefined}
     */
    ...
    /**
     * Callback envoked after verify match. Adds results to queryData and collects
     * debug infomation.
     *
     * @param {Error} err - error
     * @param {Array<Array<object>>} contexts - Array of context arrays
     * @returns {undefined}
     */
    function finalize(err, contexts) {
        if (err) return callback(err);
...
function toFeatures(geocoder, contexts, options) {
    ...
    for (let i = 0; i < contexts.length; i++) {
        ...
        const feature = ops.toFeature(context, index.geocoder_format, options.language, options.languageMode, options.debug, geocoder, options.clipBBox, options.routing);

        if (!options.allow_dupes) {
            // Dedupe by place name, prefer feature with non-omitted geom
            const keys = [feature.place_name];
            if (context[0].properties['carmen:address'] && context[0].properties['carmen:spatialmatch']) {
                keys.push(uniqueAddressId(geocoder, context));
            }

            const previous = keys.reduce((m ,v) => {
                return m || by_place_name[v] || m;
            }, false);

            if (previous) {
                if (features[previous.index].geometry.omitted && !feature.geometry.omitted) {
 ...
 */
function uniqueAddressId(geocoder, context) {
    const zoomThreshold = geocoder.indexes[context[0].properties['carmen:index']].zoom;

    const coverText = new Map();
    const covers = context[0].properties['carmen:spatialmatch'].covers;
    for (let i = 0; i < covers.length; i++) {
        coverText.set(geocoder.byidx[covers[i].idx].id, covers[i].text);
    }
    for (let j = 1; j < context.length; j++) {
        const indexName = context[j].properties['carmen:index'];
        // If we already have an string to represent this context, likely from the cover text, skip it.
        if (coverText.has(indexName)) continue;

        // We set a maximum zoom threshold to prevent insignificant differences,
        // likely informal neighborhoods, from interferring with deduplication.
        if (geocoder.indexes[indexName.zoom] <= zoomThreshold) continue;
