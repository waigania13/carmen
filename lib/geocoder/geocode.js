...
function forwardGeocode(geocoder, query, options, callback) {
    ...
    const stats_data = {};
    if (options.stats) {
        stats_data.time = +new Date();
        stats_data.phrasematch = {};
        stats_data.spatialmatch = {};
        stats_data.verifymatch = {};
        stats_data.phrasematch.time = +new Date();
    }
    const debug_data = {};
    if (options.debug) {
        debug_data.id = termops.feature(options.debug);
        debug_data.extid = options.debug;
    }
    ...
    q.awaitAll((err, phrasematchResults) => {
        if (err) return callback(err);
        if (options.stats) {
            stats_data.spatialmatch.time = +new Date;
            stats_data.phrasematch.time = +new Date - stats_data.phrasematch.time;
        }
        if (options.debug) {
            debug_data.phrasematch = {};
            for (let idx = 0; idx < phrasematchResults.length; idx++) {
                const id = geocoder.byidx[idx].id;
                debug_data.phrasematch[id] = {};
                for (let x = 0; x < phrasematchResults[idx].phrasematches.length; x++) {
                    ...
                    debug_data.phrasematch[id][phraseText] = {
                        w: matched.weight,
                        m: matched.mask.toString(2),
                        e: matched.editMultiplier
                    }
                }
            }
        }

        spatialmatch(queryData.query, phrasematchResults, options, stats_data, debug_data, spatialmatchComplete);
    });

    function spatialmatchComplete(err, matched, stats_data, debug_data) {
        if (err) return callback(err);

        if (options.stats) {
            stats_data.spatialmatch.time = +new Date - stats_data.spatialmatch.time;
            stats_data.spatialmatch.count = matched.results.length;
            stats_data.verifymatch.time = +new Date;
        }
        if (options.debug) {
            debug_data.spatialmatch = null;
            for (let x = 0; x < matched.results.length; x++) {
                const spatialmatch = matched.results[x];
                if (spatialmatch.covers[0].id !== debug_data.id) continue;
                debug_data.spatialmatch = spatialmatch;
                debug_data.spatialmatch_position = x;
            }
        }
        ...
        verifymatch(queryData.query, geocoder, matched, options, stats_data, debug_data, finalize);
    }

    function finalize(err, contexts, stats_data, debug_data) {
        if (err) return callback(err);

        if (options.stats) {
            stats_data.verifymatch.time = +new Date - stats_data.verifymatch.time;
            stats_data.verifymatch.count = contexts.length;
        }

        if (options.debug) {
            debug_data.verifymatch = null;
            for (let x = 0; x < contexts.length; x++) {
                if (contexts[x][0].id !== debug_data.extid) continue;
                debug_data.verifymatch_info = contexts[x][0].properties;
                debug_data.verifymatch_context = contexts[x];
                debug_data.verifymatch_position = x;
            }
        }
        ...
        if (options.stats) {
            stats_data.relev = contexts.length ? contexts[0]._relevance : 0;
            stats_data.time = (+new Date()) - stats_data.time;
            queryData.stats = stats_data;
        }

        if (options.debug) queryData.debug = debug_data;
