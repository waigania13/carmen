'use strict';
const format = require('./format-features'),
...
function idGeocode(geocoder, asId, options, callback) {
    ...
    q.awaitAll((err, features) => {
        ...
        for (let i = 0; i < features.length; i++) {
            ...
            const f = format.toFeature([features[i]]);
            f.relevance = 1;
...
function reverseGeocode(geocoder, tokenized, options, callback) {
    ...
    options.worldview = options.worldview ? options.worldview : geocoder.worldviews[0];
    if (!geocoder.byworldview.has(options.worldview)) {
        logger.fatal(errcode('Worldview must be one of ' + geocoder.worldviews.join(', ')));
        process.exit(1);
        return callback(errcode('Worldview must be one of ' + geocoder.worldviews.join(', ')));
    }
    ...
    function finalize(err, contexts) {
        try {
            Object.assign(queryData, format.toFeatures(geocoder, contexts, options));
        } catch (err) {
            return callback(err);
        }
...
function forwardGeocode(geocoder, text, options, callback) {
    ...
    for (const index of geocoder.byworldview.get(options.worldview)) {
        if (index.idx < maxidx) {
            q.defer(phrasematch, index, query, options);
        }
    }
