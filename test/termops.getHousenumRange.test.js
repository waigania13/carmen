...
test('termops.getHousenumRange', function(assert) {
    ...
    }), { type:'range', min:0, max:1000 }, 'parses JSON _cluster');

    assert.deepEqual(termops.getHousenumRange({
        _cluster:{ 0: {}, 10: {} }
    }), { type:'range', min:0, max:1000 }, '_cluster => 0,10');
    ...
    assert.deepEqual(termops.getHousenumRange({
        _cluster:{ 5: {}, 10: {}, 1: {}, 13: {} }
    }), { type:'range', min:0, max:1000 }, '_cluster => [1,13]');
    ...
    }), { type:'range', min:0, max:1000 }, '_cluster => [1,10]');
    ...
    assert.deepEqual(termops.getHousenumRange({
        _rangetype:'tiger',
        _lfromhn:['5','11','0','100']
    }), { type: 'range', min:0, max:1000 }, '_rangetype + _lfromhn => [0,100]');

    assert.deepEqual(termops.getHousenumRange({
        _rangetype:'tiger',
        _lfromhn:['100'],
        _ltohn:['10']
    }), { type: 'range', min:0, max:1000 }, '_rangetype + _lfromhn,_ltohn => [10,100]');

    assert.deepEqual(termops.getHousenumRange({
        _rangetype:'tiger',
        _rfromhn:['5','11','0','100']
    }), { type: 'range', min:0, max:1000 }, '_rangetype + _rfromhn => [0,100]');

    assert.deepEqual(termops.getHousenumRange({
        _rangetype:'tiger',
        _rfromhn:['100'],
        _rtohn:['10']
    }), { type: 'range', min:0, max:1000 }, '_rangetype + _rfromhn,_rtohn => [10,100]');
