...
tape('winding river rd springfield', function(t) {
    ...
    c.geocode('winding river rd  springfield', {}, function(err, res) {
        ...
        t.deepEqual(c.indexes.place1._original.logs.getGeocoderData, ['grid,40616'], 'place1: loads 1 grid');
        t.deepEqual(c.indexes.place1._original.logs.getTile, ['6,32,32'], 'place1: loads 1 tile');

        t.deepEqual(c.indexes.street1._original.logs.getGeocoderData.sort(), ['feature,1', 'feature,2', 'grid,11905', 'grid,53620'], 'street1: loads 1 grid, 1 feature per result');
...
tape('springfield', function(t) {
    ...
    c.geocode('springfield', {}, function(err, res) {
        ...
        t.deepEqual(c.indexes.place1._original.logs.getGeocoderData.sort(), ['feature,1','grid,40616'], 'place1: loads 1 grid');
        t.deepEqual(c.indexes.place1._original.logs.getTile, ['6,32,32'], 'place1: loads 1 tile');

        t.deepEqual(c.indexes.street1._original.logs.getGeocoderData.sort(), ['feature,3','grid,40616'], 'street1: loads 1 grid, 1 feature per result');
        t.deepEqual(c.indexes.street1._original.logs.getTile, [], 'street1: loads no tiles (most specific index)');
        t.end();
    });
});
