...
test('index - streaming interface', (t) => {
    ...
    t.test('ensure merged index features and original features are identical', (q) => {
        ...
        const percentage = (count / 202) * 100;
    ...
    t.test('normalizer comparison', (q) => {
        const cNormalizations = memObjectC._dictcache.normalizationCache.getAll().map((row) => {
            return [
        ...
        const normData = tmpfile();
        const buf = memObjectD._dictcache.dumpWithNormalizations(normData);

        const loaded = new DawgCache(buf);
        loaded.loadNormalizationCache(normData);

        const dNormalizations = loaded.normalizationCache.getAll().map((row) => {
            return [
